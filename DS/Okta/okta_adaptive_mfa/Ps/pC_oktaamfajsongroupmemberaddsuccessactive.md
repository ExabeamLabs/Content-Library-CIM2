...
Fields = [
   ...
   """exa_json_path=$.profile.email,exa_regex=^(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))$""",
   """exa_json_path=$.profile.displayName,exa_regex=^((({domain}[^\s\\"]+)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?)|({full_name}[^\s,]+([\s,]+\s*[^\s"]+)+?))$""",
]
