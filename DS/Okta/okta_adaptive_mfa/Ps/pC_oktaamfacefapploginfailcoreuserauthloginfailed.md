...
```Java
{
Name = "okta-amfa-cef-app-login-fail-coreuserauthloginfailed"
  ...
  Fields = ${OktaParserTemplates.json-okta-auth.Fields}[
    ...
    """({result}FAILURE|INVALID|failed|fail)""",
    """"browser":"(UNKNOWN|({browser}[^"]+))"""",
    """"deviceFingerprint":"({fingerprint}[^"]+)"""",
    """"actor":\{[^\}]+?"type":"User","alternateId":"((({user}[\w\.\-\!\#\^\~]{1,40}\$?)@({domain}[^\s",]+?\.corp))|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,.;\|]+\.[^\]\s"\\,;\|]+)))""""
    """exa_regex="actor":\{[^\}]+?"type":"User","alternateId":"((({user}[\w\.\-\!\#\^\~]{1,40}\$?)@({domain}[^\s",]+?\.corp))|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,.;\|]+\.[^\]\s"\\,;\|]+)))""""
    """exa_regex=({result}(?i)FAILURE|(?i)INVALID|(?i)failed|(?i)fail)""",
