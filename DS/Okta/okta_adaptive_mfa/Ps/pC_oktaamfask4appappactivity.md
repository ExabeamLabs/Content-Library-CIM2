...
""""(targets|actors)":[^\]]+?"objectType":"User"[^\]\}]+?"displayName":"(Unknown|RSA-OKTA Admin|AD-OKTA Admin|({full_name}[^"]+))"""
""""(targets|actors)":[^\]]+?"displayName":"(Unknown|RSA-OKTA Admin|AD-OKTA Admin|({full_name}[^"]+))[^\]\}]+?"objectType":"User""""
"""(s|d)?user\\*=({email_address}[^\s@,]+@({email_domain}[^\s@,]+))"""
"""(s|d)?user\\*=(anonymous|({user}[\w\.\-\!\#\^\~]{1,40}\$?))(\s|\||,)"""
""""(targets|actors)":[^\]]+?"objectType":"User"[^\]\}]+?"login":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))"""
""""(targets|actors)":[^\]]+?"login":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))[^\]\}]+?"objectType":"User""""
...
""""target(s)?":[^\]]+?"objectType":"User"[^\]\}]+?"displayName":"(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?)|({dest_user_full_name}\w+(\s+\w+)+))"""
""""target(s)?":[^\]]+?"displayName":"(({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?)|({dest_user_full_name}\w+(\s+\w+)+))","login":"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))","objectType":"User""""
...
"""\Wsuid=(anonymous|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))(\s+\w+=|\s*$)"""
