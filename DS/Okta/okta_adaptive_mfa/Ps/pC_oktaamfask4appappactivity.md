...
""""(targets|actors)":[^\]]+?"objectType":"User"[^\]\}]+?"login":"({email_address}[^"]+)"""
""""(targets|actors)":[^\]]+?"login":"({email_address}[^@]+@({email_domain}[^"]+))[^\]\}]+?"objectType":"User""""
...
""""actors":[^\]]+?"objectType":"Client"[^\]\}]+?"ipAddress":"(|({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)"""
""""actors":[^\]]+?"ipAddress":"(|({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)[^\]\}]+?"objectType":"Client""""
""""target(s)?":[^\]]+?"objectType":"User"[^\]\}]+?"displayName":"({dest_user}[^"]+)"""
""""target(s)?":[^\]]+?"displayName":"({dest_user}[^"]+)[^\]\}]+?"objectType":"User""""
...
"""\Wsuid=(anonymous|({email_address}[^@=]+@[^@=]+?)|({user}[^\s=]+?))(\s+\w+=|\s*$)"""
