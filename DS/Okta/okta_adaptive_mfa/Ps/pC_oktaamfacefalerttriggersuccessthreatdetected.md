...
Name = okta-amfa-cef-alert-trigger-success-threatdetected
  ...
  Fields = ${OktaParserTemplates.json-okta-auth.Fields}[
    """suser=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|(anonymous|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))""",
    """"severity"+:"+({alert_severity}[^",]+)""",
    """({alert_type}application-action)"""
    """"action"+:.+?"+message"+:"+({alert_name}[^",]+)"""
    """"displayMessage"\s*:\s*"({alert_name}[^",]+)""",
    """exa_json_path=$..displayMessage,exa_field_name=alert_name""",
  ]
