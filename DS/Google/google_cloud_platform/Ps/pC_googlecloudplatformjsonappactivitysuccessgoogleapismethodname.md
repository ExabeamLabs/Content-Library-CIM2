...
Name = google-cloudplatform-json-app-activity-success-googleapismethodname
  ...
  Fields = [
    ...
    """"principalEmail":\s*"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\s@"]+))"""",
    ...
    """exa_json_path=$.jsonPayload.access.principalEmail,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\s@"]+))""",
