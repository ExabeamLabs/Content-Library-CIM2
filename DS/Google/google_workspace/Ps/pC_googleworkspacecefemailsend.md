...
""""service":"gmail-ui"""" 
  ...
  Fields = ${GoogleParsersTemplates.json-google-email-activity.Fields} [
     ...
     """"destination":\[\{"address[":]*({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""",
     """"source":\{"address":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
...
json-google-email-activity = {
  ...
  Fields = [
    ...
    """exa_json_path=$.message_info.source.address,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
    """exa_json_path=$.message_info.source.from_header_displayname,exa_field_name=log_source""",
    """exa_json_path=$.message_info.destination[*].address,exa_regex=({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""",
