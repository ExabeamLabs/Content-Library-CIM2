...
Fields = [
    ...
    ""","parameters":[^=]*?name":"doc_id","value":"({file_id}[^"]+)"[^=]*?name":"doc_type","value":"((?i)unknown|({file_type}[^"]+))"[^=]*?name":"doc_title","value":"\s*({file_name}[^"]+?(\.\s*({file_ext}[a-zA-Z]+?))?)\s*"[^=]*?name":"visibility","value":"({operation}({privileges}[^"]+))"""",
