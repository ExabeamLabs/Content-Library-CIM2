...
"""Client:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s*(\((\()?({src_host}[\w\d\\]+)\))?"""
"""Server:\s*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?\s*(\(({dest_host}[\w\d\\]+)\))?"""
"""Server Type:\s*({server_group}.+?)\s([#\d\\n]+)?Client:"""
"""DB User:\s*(({domain}\w+)\\)?({account}({db_user}[\w\d]+))(\s+\(.+?\))?([#\d\\n]+)?([\w\s]+:)"""
"""OS User:\s*({user}[\w\.\-\!\#\^\~]{1,40}\$?)\s*([#\d\\n]+)?([\w\s]+:)"""
