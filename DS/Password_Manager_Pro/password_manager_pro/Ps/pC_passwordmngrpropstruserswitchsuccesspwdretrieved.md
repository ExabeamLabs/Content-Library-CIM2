...
Fields = [
"""\s+(ResourceAudit:)?(({first_name}[^\s:_]+)(_({last_name}[^:\s]+))?):(?:({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?|({src_host}[^\s]+))\s+Password_Retrieved"""
