...
"""attempts="""
   ...
   Fields = [
"""host=({host}(({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})))|({src_host}[\w\-\.]+))\s\w+="""
...
"""exa_regex=host=({host}(({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})))|({src_host}[\w\-\.]+))\s\w+="""
"""exa_regex=\w{3}\s\d\d\s\d\d:\d\d:\d\d\s({host}\S+)\sinfo"""
