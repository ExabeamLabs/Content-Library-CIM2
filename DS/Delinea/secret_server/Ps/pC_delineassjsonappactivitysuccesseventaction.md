...
Fields = [
  ...
  """exa_regex="Target"+:\{.+?"Type":"USER","Name":"(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
  """exa_regex="Notes":.+?"byUser\\*":\\*"({user}[\w\.\-\!\#\^\~]{1,40}\$?)"""
  """exa_regex="Actor":\s*\{[^\}]*"Type":\s*"user"[^\}]+"Name":\s*"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
  """exa_regex="Actor":\s*\{[^\}]*"Id":\s*"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))[^\}]+"IdType":\s*"email""""
