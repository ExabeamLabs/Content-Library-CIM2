...
Fields = [
  ...
  """exa_json_path=$.message,exa_regex=({additional_info}({time}\d\d\d\d\d\d\d\d \d\d:\d\d:\d\d),({host}[^,]+),({event_name}[^,]+),({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})),([^,]+,){2}({connection_status}[^,]+),[^\$"]+)"""
