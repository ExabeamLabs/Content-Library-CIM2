...
```Java
{
Name = vmware-airwatch-kv-app-activity-success-appremoved
  Conditions = [ """AirWatch""", """Event Timestamp:""", """ConsoleEvent: ApplicationRemoveFromDeviceRequested""" ]
...
airwatch-app-activity = {
    ...
    Fields = [
      ...
      """User:\s*((({domain}[^\\]+?)\\+)?(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\s*Event Source:"""
      """"Application=({app}[^;=]+);\w+="""
    airwatch-app-activity = {
    Vendor = VMware
    Product = VMware AirWatch
