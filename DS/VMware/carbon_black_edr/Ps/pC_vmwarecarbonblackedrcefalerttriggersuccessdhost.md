...
"""\srt=({time}\d{13})"""
"""\sdhost=({src_host}[^\s]+)"""
"""\s(dst|local_ip)=({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
"""\sdeviceSeverity=({alert_severity}\d+)"""
"""\sdirection:({direction}(Inbound|Outbound))"""
"""\sremote_ip:({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
