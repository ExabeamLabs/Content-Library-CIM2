...
```Java
{
Name = vmware-carbonblackedr-json-app-authentication-success-autheventlogonop
  ParserVersion = "v1.0.0"
  Conditions = [ """"type":"auth.event.logonop"""", """"auth_username":"""", """"auth_event_action":"""" ]
  Fields = ${CarbonBlackParsersTemplates.carbonblack-endpoint.Fields}[
    """"clientIp":"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""""
    """exa_json_path=$.auth_event_action,exa_field_name=action"""   
    """exa_json_path=$.sensor_action,exa_field_name=result"""
    """exa_json_path=$.device_internal_ip,exa_regex=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
    """exa_json_path=$.auth_privileges,exa_field_name=privileges"""    
    """exa_json_path=$.device_id,exa_field_name=device_id"""
    """exa_json_path=$.auth_user_id,exa_field_name=user_sid"""    
    ...
    """exa_json_path=$.device_os_version,exa_field_name=os"""
    """exa_json_path=$.auth_domain_name,exa_field_name=domain"""    
    """exa_json_path=$.auth_username,exa_regex=^(({domain}[^\\"]+)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?)$"""
