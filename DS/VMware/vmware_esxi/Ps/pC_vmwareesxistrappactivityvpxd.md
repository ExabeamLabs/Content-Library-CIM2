...
Name = vmware-esxi-str-app-activity-vpxd
  ...
  Fields = [
    ...
    """Event\s*(\[[^\]]*\]\s+){8}\[({event_name}[^\]]+)\s(({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.-])*[A-Za-z0-9]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)(?<!local)(?<!loc)(?<!localdomain))|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@({domain}[^,]+))?)\sfrom\s({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s"""
  ]
