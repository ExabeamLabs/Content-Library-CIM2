...
Name = vmware-esxi-str-app-login-loggedin
  ...
  Fields = [
    """({time}\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d.\d+Z)\s+({dest_host}({host}[\w\-\.]+))\sHostd:""",
    """User\s+((({domain}[^\\\s@]+)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))[^$]+?\s*logged""",
    """({event_name}logged in)""",
    """logged in as ({user_agent}\S+)""",
    """user agent:\s+({user_agent}[^)]+)""",
    """: User\s\w+@({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
  ]
