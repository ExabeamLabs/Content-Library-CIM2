...
```Java
{
Name = vmware-carbonblack-sk4-alert-trigger-success-watchlist-2
  Vendor = VMware
  Product = Carbon Black CES
  TimeFormat = "yyyy-MM-dd'T'HH:mm:ss.SSSZ"
  Conditions = [ """"type":"WATCHLIST"""", """"reason_code"""", """"alert_url":""", """"threat_id":""", """"device_username":""" ]
  Fields = [
    ...
    """"device_username"+:"+(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|(({domain}[^"]+?)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""",
    """"process_username"+:"+(NT AUTHORITY\\+(SYSTEM|NETWORK SERVICE|LOCAL SERVICE)|(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|(({domain}[^"]+?)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?)))"""",
    """"device_os"+:"+({os}[^"]+)""""
  ]
