...
```Java
{
Name = oracle-oci-json-app-activity-auditlogevent
  Vendor = Oracle
  ...
  Fields = [
    ...
    """exa_json_path=$.data.identity.ipAddress,exa_regex=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))"""
    """exa_json_path=$.data.identity.principalId,exa_field_name=principal_id"""
    """exa_json_path=$.data.identity.userAgent,exa_field_name=user_agent"""
    ...
    """exa_json_path=$.data.resourceName,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({resource_name}[\w\.\-\!\#\^\~]{1,40}\$?))"""
