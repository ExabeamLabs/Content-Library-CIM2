...
```Java
{
Name = oracle-pc-json-configuration-modify-success-sessionmodifysuccess-1
...
oracle-public-cloud-events-1 = {
    ...
    Fields = [
          ...
          """exa_regex=actorName\":\"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
          """exa_regex=ssoPlatform\":\"({os}[^"\\]+)""",
          """exa_regex=idcsLastModifiedBy\":.+?display\":\"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({full_name}[^,"\\]+,[^"\\]+))""",
