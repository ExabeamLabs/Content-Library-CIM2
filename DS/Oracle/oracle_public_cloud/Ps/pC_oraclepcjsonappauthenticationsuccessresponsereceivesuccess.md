...
```Java
{
Name = oracle-pc-json-app-authentication-success-responsereceivesuccess
  ParserVersion = "v1.0.0"
  Conditions = [ """"ssoAuthnLevel":""", """"serviceName":"SAML"""", """"eventId":"saml.sp.sso.response.received.success"""" ]
...
oracle-public-cloud-events = {
    ...
    Fields = [
          ...
          """exa_json_path=$..actorName,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
          """exa_json_path=$..ssoPlatform,exa_field_name=os""",
          """exa_json_path=$..idcsLastModifiedBy.display,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({full_name}[^,"]+,[^"]+))""",
