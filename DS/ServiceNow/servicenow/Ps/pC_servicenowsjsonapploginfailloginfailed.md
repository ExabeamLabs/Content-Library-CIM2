...
```Java
{
Name = servicenow-s-json-app-login-fail-loginfailed
  ParserVersion = v1.0.0
  Conditions = [ """"instance":""", """"name":"login.failed"""", """"sys_created_on":""", """"sys_created_by":""" ]
...
servicenow-login-template = {
    ...
    Fields = [
      ...
      """({app}ServiceNow)""",
      """"user(_name)?":"((anonymous)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)@({domain}[^"\s@]+)|({=user}[^"\s@]+))"""",
      """"name":"({object}[^"]+)""",
      """"name":"({event_name}[^"]+)"""",
      """"queue":"({event_name}[^"]+)""",
