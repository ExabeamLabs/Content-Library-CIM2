...
Name = zeek-z-str-file-read-success-fileslog
  ...
  Fields = [
      """({time}\d{10})\.\d{6}\s+({file_id}[^\s]+)\s+(?:-|(({src_ip}(\d{1,3}\.){3}\d{1,3}|([A-Fa-f0-9%.]*:[A-Fa-f0-9%.:]+(th0)?))|[^\s]+))\s+(?:-|(({dest_ip}(\d{1,3}\.){3}\d{1,3}|([A-Fa-f0-9%.]*:[A-Fa-f0-9%.:]+(th0)?))|[^\s]+))\s+(?:-|({connection_uids}[^\s]+))\s+(?:-|({protocol}[^\s]+))\s+(?:-|({depth}[^\s]+))\s+(?:-|({analyzers}[^\s]+))\s+(?:-|({mime}[^\s]+))\s+(?:-|([^\s]+))\s+(?:-|({duration}[^\s]+))\s+(?:-|({local_orig}[^\s]+))\s+(?:-|({is_orig}[^\s]+))\s+(?:-|({bytes}\d+)?)\s+(?:-|({=bytes}\d+))\s+(?:-|({missed_bytes}\d*))\s+(?:-|({overflow_bytes}\d*))\s+(?:-|({timedout}[^\s]+))\s+(?:-|({parent_file_id}[^\s]+))\s+(?:-|({hash_md5}[^\s]+))\s+(?:-|({hash_sha1}[^\s]+))\s+(?:-|({hash_sha256}[^\s]+))\s+(?:-|({extracted}[^\s]+?))\s*""",
