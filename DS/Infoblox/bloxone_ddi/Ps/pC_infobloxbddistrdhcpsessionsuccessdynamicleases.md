...
Name = infoblox-bddi-str-dhcp-session-success-dynamicleases
  ...
  Conditions = [ """ dhcpd[""", """]: BOOTREQUEST from """ ]
  Fields = [
    """\w{3}\s\d{2}\s\d{2}:\d{2}:\d{2}\s+({host}[\w.-]+)\s+({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s+dhcpd\[""",
