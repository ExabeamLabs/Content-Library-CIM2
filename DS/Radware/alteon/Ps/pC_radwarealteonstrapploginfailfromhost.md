...
Name = radware-alteon-str-app-login-fail-fromhost
  ...
  Fields = [
    ...
    """({operation}({result}Failed) login attempt) via ({resource}.+?) from host (({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?|({src_host}\S+))""",
  ]
