...
"""mod=mail cmd=env_from"""
  ...
  Fields = [
    ...
    """\sx=({alert_id}.+?)\s+(\w+=|$)""",
    """\svalue=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))\s+(\w+=|$)""",
    """\shost=\[?(({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?|({src_host}[\w\-.]+?))\]?\s+(\w+=|$)""",
