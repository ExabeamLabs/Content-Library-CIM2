...
""":"""
  ...
  Fields = [
    """"relay"+:\s*"+({dest_host}({host}[\w\-.]+?))\.?\s*\[({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
    """"from"+:\s*\[?"+([^<,]+?<|<)?(\\u\d+)?({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|\>]+\.[^\]\s"\\,;\|\>]+))\s*>?""",
    """"sizeBytes"+:\s*"*({bytes}\d+)""",
    ...
    """"qid"+:\s*"+({alert_id}[^"]+)""",
    """"to":\["<\'?({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@([^\]\s"\\,\|>]+\.[^\]\s"\\,\|>]+)(?<!local))\s*>"""
    """exa_json_path=$.sm.relay,exa_regex=({dest_host}({host}[\w\-.]+?))\.?\s*\[({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
    """exa_json_path=$.sm.from,exa_regex=([^<,]+?<|<)?(\\u\d+)?({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|\>]+\.[^\]\s"\\,;\|\>]+))\s*>?""",
    ...
    """exa_regex="to":\["<\'?({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@([^\]\s"\\,\|>]+\.[^\]\s"\\,\|>]+)(?<!local))\s*>"""
  ]
