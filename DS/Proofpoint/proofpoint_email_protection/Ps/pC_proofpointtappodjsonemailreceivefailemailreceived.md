...
"""rcpts":\[({email_recipients}"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))"[^\]]*)\]"""
"""filter":[^\n]*?"disposition":"({result}[^"]+)""""
...
"""exa_regex=from":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""""
"""exa_regex=subject":\["({email_subject}[^"]+?)\s*"\]"""
"""exa_regex=rcpts":\[({email_recipients}"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))"[^\]]*)\]"""
