...
""":"""
  ...
  Fields = [
    ...
    """exa_json_path=$.sm.to[1:],exa_regex=(\\u\d+)?<(([^<@\]]+<|<|\\"+|\')?({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@([^\]\s"\\,\|>]+\.[^\]\s"\\,\|>]+)(?<!local)))\s*>"""
    """exa_regex="to":\s*\["?<\'?({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|\>]+))"""
    ...
    """"to"+:\s*\["+(\\u\d+)?<(([^<@\]]+<|<|\\"+|\')?({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@([^\]\s"\\,\|>]+\.[^\]\s"\\,\|>]+)(?<!local)))\s*>"""
  ]
