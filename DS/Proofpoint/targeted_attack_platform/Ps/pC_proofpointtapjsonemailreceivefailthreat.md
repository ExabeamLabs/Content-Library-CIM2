...
"""messageID":""""
  ...
  Fields = ${ProofpointParsersTemplates.s-proofpoint-email-in-1.Fields}[
    ...
    """(fromAddress|sender)":\s*\[?"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))([\\]+)?([\\]+)?"\]?""",
    """toAddresses":\s*\[({email_recipients}"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))[^\]]*?)\]""",
    """\scs1=Policy \[id: [^\]]*? ; name: ({alert_type}({alert_subject}({alert_name}[^\]]+?))) ; category: ({category}[^\]]+?)]""",
    """"classification":\s*"({alert_type}({alert_subject}({alert_name}[^"]+)))""",
    """:\sCategory\s\[[^\]]+\]\s,\sName\s\[({alert_type}({alert_subject}({alert_name}[^\]]+)))\]""",
    ...
    """"recipient":\["({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))"],""",
    """proto=({alert_type}({alert_subject}({alert_name}[^=]+)))\s""",
    """msg=.*?\[({alert_source}[^\]]+)\]:""",
...
s-proofpoint-email-in-1 = {
  ...
  Fields = [
    ...
    """suser=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|0-9]+))""",
    """duser=({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""",
    """sender":\s*"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|0-9]+))""",
    """recipient":\s*\[?"({email_recipients}[^",;]+@[^",;]+[^"]*)""",
    """recipient":\s*\[?"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""",
    """GUID":\s*"({alert_id}[^",]+?)\s*(,|")""",
    ...
    """exa_json_path=$.sender,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|0-9]+))""",
    """exa_regex=recipient":\s*\[?"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""",
