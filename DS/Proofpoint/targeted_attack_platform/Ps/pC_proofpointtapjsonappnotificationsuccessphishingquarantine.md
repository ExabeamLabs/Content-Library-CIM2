...
proofpoint-tap-log = {
  ...
  """exa_json_path=$.event.description.incident_id,exa_field_name=alert_id""", 
	"""exa_json_path=$.event.description.sender,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""", 
	...
	"""exa_json_path=$.event.description.recipient,exa_regex=({email_recipients}({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))\\*"\]""", 
