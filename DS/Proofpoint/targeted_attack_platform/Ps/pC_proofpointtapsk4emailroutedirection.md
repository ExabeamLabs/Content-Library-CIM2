...
Fields = [
  ...
  """"msg_normalizedHeader_from_s":"(\[|\s|\\|")*[^\]]+?<({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))>"""
  """"msg_normalizedHeader_from_s":"(\[|\s|\\|")*({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))\\""""
  """"envelope_from_s":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""""
  """"smtp\.mailfrom": "({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""""
  """"filter_verified_rcpts_s":"\[\s*\\*"({email_recipients}.+?)\\*"\s*\]","""
  """"filter_verified_rcpts_s":"\[[^"]*"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))[\s\\"]+"""
