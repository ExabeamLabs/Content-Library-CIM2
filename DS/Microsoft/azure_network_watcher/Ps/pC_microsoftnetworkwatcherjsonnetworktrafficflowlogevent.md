...
Name = microsoft-networkwatcher-json-network-traffic-flowlogevent
    ...
    Fields=[
      ...
      """"(raw-log|flowTuples)":\[?"\d+,({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})),({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})),({src_port}\d+),({dest_port}\d+),({protocol}\d+),({direction}\w),({action}\w),({result_code}[^,]+),({packets_out}\d+),({bytes_out}\d+),({packets_in}\d+),({bytes_in}\d+)""""
    ]
