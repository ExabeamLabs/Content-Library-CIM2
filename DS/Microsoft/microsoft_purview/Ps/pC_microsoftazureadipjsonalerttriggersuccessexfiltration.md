...
Name = microsoft-azureadip-json-alert-trigger-success-exfiltration
  ...
  Fields = [
    ...
    """"userPrincipalName":"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@[^"]+)?)"""",
    """"domainName":\s*"({domain}[^"]+)"""",
    ...
    """exa_json_path=$.evidence[0].userAccount.userPrincipalName,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@[^"]+)?)""",
