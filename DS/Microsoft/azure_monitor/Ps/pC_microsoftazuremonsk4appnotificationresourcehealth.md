...
Name = microsoft-azuremon-sk4-app-notification-resourcehealth
  ...
  Fields = ${LMSMSParsersTemplates.cef-microsoft-app-activity1.Fields}[
    """"properties".*?"cause"\s*:\s*"({result_reason}[^"]+?)"""",
    """"properties".*?"details"\s*:\s*"({additional_info}[^"]+)""",
    """"properties".*?"title"\s*:\s*"({event_name}[^"]+?)"""",
    """"properties".*?"type"\s*:\s*"({event_category}[^"]+)"""",
    """"properties".*?"currentHealthStatus"\s*:\s*"({result}[^"]+)""""
  ]
