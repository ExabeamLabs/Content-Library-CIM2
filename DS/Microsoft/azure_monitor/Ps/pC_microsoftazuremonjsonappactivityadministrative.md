...
```Java
{
Name = microsoft-azuremon-json-app-activity-administrative
  ExtractionType = json
  Conditions = [ """"OperationNameValue"""", """"_ResourceId"""", """"CategoryValue":"Administrative"""" ]
  Fields = ${LMSMSParsersTemplates.cef-microsoft-app-activity-3.Fields}[
    ...
    """exa_regex="_ResourceId":\s*"({resource_id}(\/SUBSCRIPTIONS\/({subscription_id}[^\/]+))?(\/RESOURCEGROUPS\/({resource_group}[^\/]+))?\/[^"]+)"""",
  ]
