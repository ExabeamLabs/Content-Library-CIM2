...
# azure_impact_level is removed
    #"""\Wext_Properties_UserAddress=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
    """"properties".*?"UserAddress":"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""",
...
cef-microsoft-app-activity1 = {
  ...
  Fields = [
    ...
    """"name":"({full_name}[^"]+)"""",
    """action":"({action}[^"]+)""",
    """"(callerIpAddress|CIp)":"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""",
    """"Caller":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""",
    ...
    """exa_regex=category(value)?":"({category}[^"]+)"""",
    ...
    """exa_regex="operationName(Value)?":"({operation}[^"]+)"""
    """exa_regex="Host":"({host}[^"]+)"""",
    ...
    """exa_json_path=$.Caller,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
