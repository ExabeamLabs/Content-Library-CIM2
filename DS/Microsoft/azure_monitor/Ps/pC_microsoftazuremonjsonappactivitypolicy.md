...
```Java
{
Name = microsoft-azuremon-json-app-activity-policy
  ParserVersion = v1.0.0
  ExtractionType = json
  Conditions= [ """"OperationNameValue"""", """"CategoryValue":"Policy"""", """"_ResourceId"""" ]
  Fields = ${LMSMSParsersTemplates.cef-microsoft-app-activity-3.Fields}[
    """exa_json_path=$.TimeGenerated,exa_field_name=time""",
    """exa_json_path=$.CategoryValue,exa_field_name=category""",
    """exa_json_path=$.OperationNameValue,exa_field_name=operation""",
    """exa_json_path=$._ResourceId,exa_field_name=object""",
    """exa_json_path=$._ResourceId,exa_field_name=resource""",
    """exa_regex="_ResourceId":\s*"({resource_id}\/SUBSCRIPTIONS\/({subscription_id}[^\/]+)\/RESOURCEGROUPS\/({resource_group}[^\/]+)(\/PROVIDERS\/({provider_name}[^\/]+))?(\/[^\/"]*)?(\/({resource}[^"]+))?)"""",
  ]
...
cef-microsoft-app-activity-3 = {
  ...
  Fields = [
    ...
    """exa_regex=claims\/(name|upn)":\s*"({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.-])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
    """exa_regex=email":\s*"({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.-])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
    ...
    """exa_regex=userId":\s*"({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.-])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
