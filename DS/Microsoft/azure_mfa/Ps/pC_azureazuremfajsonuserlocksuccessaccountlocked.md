...
```Java
{
Name = azure-azuremfa-json-user-lock-success-accountlocked
  ParserVersion = "v1.0.0"
  Conditions = [ """"additionalDetails":"This error can be returned for two reasons - the sign in could have come from a malicious IP address, or the account was locked""", """"resourceId":""", """"failureReason":""", """"userDisplayName":""" ]
...
microsoft-azuremfa-json-events = {
    ...
    Fields = [
      ...
      #"""exa_json_path=$.userPrincipalName,exa_regex=(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({dest_user}[^@",\s]+))""",
      """exa_json_path=$..userPrincipalName,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@[^"]+)?)""",
