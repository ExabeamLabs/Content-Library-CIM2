...
Name = microsoft-evsecurity-kv-endpoint-login-fail-4625-3
  ParserVersion = v1.0.0
    ...
    Fields = [
      ...
      """,(Audit Failure|Failure Audit|Information),({host}({dest_host}[\w\-.]+)),""",
      """({event_code}4625)""",
      ...
      """\s*Sub Status:\s*({failure_code}({result_code}.+?))[\s;]*Process Information:""",
      """Workstation Name:\s+({src_host_windows}({src_host}[\w\-\.]+))\s+Source Network""",
