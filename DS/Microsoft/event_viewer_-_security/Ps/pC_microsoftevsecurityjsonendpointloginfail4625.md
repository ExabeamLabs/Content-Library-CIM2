...
Name = "microsoft-evsecurity-json-endpoint-login-fail-4625"
  ...
  Fields = [
    ...
    """"(TargetUserName|AccountName)":"\s*(({dest_email_address}({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({dest_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))""""
    ...
    """exa_regex="(Hostname|MachineName)\\?":\\?"({dest_host}({host}[\w\-.]*))"""
    ...
    """exa_regex="(TargetUserName|AccountName)":"\s*(({dest_email_address}({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({dest_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))""""
    """exa_regex="TargetDomainName\\?":\\?"({dest_domain}({domain}[^."\\]+))"""
    """exa_regex="SubStatus\\?":\\?"({failure_code}({result_code}[^"\\]+))"""
