...
Fields = [
  ...
  """({event_code}5145)"""
  """domain:\s+[^\s:]+:\s+({user_sid}[^\s]+)\s+({user}[^\s]+)\s({domain}[^\s]+)\s+({login_id}[^\s]+)\s+({file_type}[^\s]+)\s+({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s+({src_port}\d+)\s+({share_name}[^\s]+)\s*(({share_path}[^\s]+)\s+({file_path}({file_dir}.*?[\\\/]+)?({file_name}[^\\\/]+?(\.({file_ext}\w+))?)?)\s+({access_mask}0x\d+)\s+({access}[^:]+?)\s+({access_reason}[\%\d]+:.+?)\s*)($|\w+=|"|')"""
]
DupFields = [
  "host->dest_host"
]
