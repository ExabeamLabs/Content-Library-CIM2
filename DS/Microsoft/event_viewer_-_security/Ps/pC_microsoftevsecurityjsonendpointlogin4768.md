...
Name = "microsoft-evsecurity-json-endpoint-login-4768"
  ...
  Fields = [
    ...
    """({event_code}4768)"""
    """"(TargetUserName|AccountName)":"\s*(({user_upn}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+(\.[^\]\s"\\,\|]+)?)|({dest_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))"""
    ...
    """exa_json_path=$.TargetUserName,exa_regex=\s*(({user_upn}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+(\.[^\]\s"\\,\|]+)?)|({dest_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))"""
