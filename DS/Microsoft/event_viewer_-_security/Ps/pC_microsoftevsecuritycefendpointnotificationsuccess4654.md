...
Name = microsoft-evsecurity-cef-endpoint-notification-success-4654
  ...
  """"SourceSystem":"({app}[^"]+)""""
  """<Data Name\\?=\\?"RemoteAddress\\?">({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))"""
  """<Data Name\\?=\\?"FailureReason\\?">({failure_reason}[^<]+)\s+?"""
  """<Data Name\\=\\?"LocalAddress\\?">({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))"""
