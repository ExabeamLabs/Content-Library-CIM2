...
Name = microsoft-evsecurity-csv-endpoint-login-success-4648
  ...
  Fields = [ 
    ...
    """サブジェクト:\s+セキュリティ ID:\s+({user_sid}[^\s]+)\s+アカウント名:\s+({user}.+?)\s+>アカウント ドメイン:\s+({domain}[^\s]+)\s+ログオン ID:\s+({login_id}[^\s]+)\s+""",
    ...
    """ネットワーク アドレス:\s+(?:-|({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)"""
  ]
