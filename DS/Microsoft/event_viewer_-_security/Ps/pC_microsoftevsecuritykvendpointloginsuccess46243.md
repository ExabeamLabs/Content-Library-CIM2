...
Name = "microsoft-evsecurity-kv-endpoint-login-success-4624-3"
  ...
  Fields = [
    ...
    """\sAccount Name:\s*(\\[ntr])*(-|(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))(\s|(\\[ntr])*)\s+Account Domain:"""
    """New Logon:[^"]*?Account Name(:|=)\s*(\\[nrt])*(-|SYSTEM|(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\s*"""
    ...
    """Source Network Address(:|=)\s*(\\[nrt])*(?:-|(::ffff:)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)([\s;]*|(\\[nrt])*)Source Port(:|=)"""
    ...
    """\s*Process ID:\s*(\\[nrt])*({process_id}[^\s\\]+)(\s*|(\\[nrt])*)"""
