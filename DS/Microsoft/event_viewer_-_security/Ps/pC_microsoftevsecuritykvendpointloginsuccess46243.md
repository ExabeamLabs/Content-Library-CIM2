...
Name = "microsoft-evsecurity-kv-endpoint-login-success-4624-3"
  ...
  Fields = [
    ...
    """Account Name:\s*(\\+[ntr])*(-|(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({account}({user}[\w\.\-\!\#\^\~]{1,40}\$?))))(\s|(\\+[ntr])*)\s*Account Domain:"""
    """New Logon:[^"]*?Account Name(:|=)\s*(\\+[nrt])*(-|SYSTEM|(({user_upn}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+(\.[^\]\s"\\,\|]+)?)|({account}({user}[\w\.\-\!\#\^\~]{1,40}\$?))))\s*"""
    ...
    """:\d+:\d+\s+(\d\d\d\d|(AM|PM)|(::ffff:)?(({dest_ip}[a-fA-F0-9.:]+)|({dest_host}[\w\-.]+)))\s"""
