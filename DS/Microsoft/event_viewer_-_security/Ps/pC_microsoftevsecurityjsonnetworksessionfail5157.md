...
"""({event_code}5157)"""
"""Computer(Name)?:\s*({host}[\w.\-]+)"""
"""Process ID:\s*(\\t|\\r|\\n)*(|({process_id}\d+))\s*(\\t|\\r|\\n)*Application Name:\s*(\\t|\\r|\\n)*(|({process_path}({process_dir}.*?[\\\/]+)?({process_name}[^\\\/]+?)))\s*(\\t|\\r|\\n)*Network Information:"""
"""Direction:\s*(\\t|\\r|\\n)*({direction}[^:]+?)(\\t|\\r|\\n)*Source Address:"""
"""Source Address:\s*(\\t|\\r|\\n)*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?(\\t|\\r|\\n)*"""
"""Source Port:\s*(\\t|\\r|\\n)*({src_port}\d+)(\\t|\\r|\\n)*"""
"""Destination Address:\s*(\\t|\\r|\\n)*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?(\\t|\\r|\\n)*"""
