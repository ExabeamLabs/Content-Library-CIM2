...
```Java
{
Name = microsoft-evsecurity-cef-user-delete-success-4726
    ParserVersion = v1.0.0
    Conditions = [ """CEF:""", """|Microsoft-Windows-Security-Auditing:4726|""", """A user account was deleted""" ]
    Fields = ${WindowsParsersTemplates.windows-events-3.Fields} [
      ...
      """suser=({user}[\w\.\-\!\#\^\~]{1,40}\$?)\s\w+=""",
	    """duser=({account_name}({dest_user}[^=]+))\s\w+=""",
	    """sntdom=({domain}[^=\\\s]+)""",
	    """dntdom=({dest_domain}[^=\\\s]+)"""
	    """duid=({dest_user_sid}[^=]+)\s\w+="""
    ]
