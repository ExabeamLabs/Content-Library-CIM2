...
Name = "microsoft-evsecurity-kv-endpoint-login-4776-6"
  ...
  Fields = [
    ...
    """Logon (?:a|A)ccount(:|=)\s*(({email_address}[^@\s]+?@[^\s]+?\.[^\s]+?)|((({full_name}[^:]+?\s[^\s]+?)|({user}[^\:]+?))(?:@({domain}[^\s.;,@=]+).*?)?))[\s;]*Source Workstation(:|=)([\s\\]+|(\s*\\*((({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})?)(:({dest_port}\d+))?)|({dest_host}.+?))[\s;]*))Error Code(:|=)"""
