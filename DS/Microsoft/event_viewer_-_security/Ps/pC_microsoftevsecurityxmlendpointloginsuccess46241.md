...
Name = "microsoft-evsecurity-xml-endpoint-login-success-4624-1"
  ...
  """<Data Name(\\)?=('|")TargetUserName('|")>(({dest_email_address}({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({dest_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?))(@({dest_domain}({domain}[^@<=]+)))?)</Data>"""
  """<Data Name\\*=('|")TargetUserName('|")>(-|({user_upn}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_domain}({domain}[^\]\s"\\,;\|]+(\.[^\]\s"\\,;\|]+))?))|({full_name}[^\s<]+\s[^<]+)|(SYSTEM|({dest_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?))))<\/Data>"""
  ...
  """<Data Name =('|")TargetUserName('|")>({src_host}[\w\-\.]+)\$<"""
  ]
