...
Name = microsoft-evsecurity-kv-endpoint-login-4776-1
  ParserVersion = v1.0.0
    ...
    Fields = [
      ...
      """Logon (?:a|A)ccount(:|=)[\s\\\\t]*(({email_address}[^@\s]+?@[^\s]+?\.[^\s]+?)|(({user}[\w\.\-]{1,40}\$?)(?:@({email_domain}[^\s.;,@=]+).*?)?))[\\rnt]*Source Workstation(:|=)([\s\\]+|(\s*\\*[\\\\t]*((({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})?)(:({dest_port}\d+))?)|({dest_host}[\w\-.]+?)[\\\\n]*)[\s;]*))Error Code(:|=)""",
