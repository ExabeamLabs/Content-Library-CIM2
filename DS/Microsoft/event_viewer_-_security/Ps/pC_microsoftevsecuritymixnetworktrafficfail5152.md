...
Name = microsoft-evsecurity-mix-network-traffic-fail-5152
  ...
  Fields = [
    ...
    """\s*Direction:\s*(|-|({direction}.+?))\s*Source Address:\s*(|-|0.0.0.0|(::ffff:)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)\s*Source Port:\s*(|-|({=src_port}\d+))\s*Destination Address:\s*(|-|0.0.0.0|(::ffff:)?({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?)\s*Destination Port:\s*(|-|({=dest_port}\d+))\s*Protocol:\s*(|-|({protocol}.+?))\s*Filter Information:""",
  ]
