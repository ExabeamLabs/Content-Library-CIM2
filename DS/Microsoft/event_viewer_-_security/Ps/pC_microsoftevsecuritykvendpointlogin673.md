...
Fields = [
  ...
  """Computer=({host}[^\s]+)"""
  """User Name:\s+({user}[^@\s]+).+?Service Name:\s+({service_name}\S+).+?Client Address:\s+({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?.+?Failure Code:\s+({result_code}[^\s]+)"""
