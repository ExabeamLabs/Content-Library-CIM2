...
Name = microsoft-evsecurity-kv-network-session-fail-5157
  ...
  Fields = [
    ...
    """({event_code}5157)""",
    """\s*Computer(Name)?=({host}[\w\-.]+)\s*""",
    """\s*Process ID:\s*({process_id}[^\s]+)\s*""",
    """\s*Source Address:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s*""",
    """\s*Source Port:\s*({src_port}\d+)\s*""",
    """\s*Destination Address:\s*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?\s*""",
    ...
    """Message=({failure_reason}[^=]+?)\s+Application Information:"""
  ]
