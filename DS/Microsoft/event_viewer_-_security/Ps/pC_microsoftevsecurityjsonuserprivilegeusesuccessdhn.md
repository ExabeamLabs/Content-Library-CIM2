...
```Java
{
Name = "microsoft-evsecurity-json-user-privilege-use-success-dhn"
Vendor = "Microsoft"
...
Fields = [
  ...
  """\s*Account Name(:|=)\s*(?:-|({user}[\w\.\-]{1,40}\$?)|({full_name}[^",]+?))[\s;]*Account Domain(:|=)\s*({domain}.+?)[\s;]*Logon ID(:|=)\s*({login_id}.+?)[\s;]*Object(:|=)"""
