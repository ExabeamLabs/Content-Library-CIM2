...
Name = microsoft-evsecurity-csv-endpoint-login-success-528
  ...
  Fields = [
    ...
    """({event_code}528)""",
    """Information\s+({dest_host}({host}[\w.\-]+))\s+""",
    """(?:Success|Audit)\s+\w+\s+({dest_host}({host}[^\s]+))""",
    """Security\s+(rn=)?({event_id}\d+)""",
    """User Name:\s+({user}[\w\.\-\!\#\^\~]{1,40}\$?)\s+Domain:\s+({domain}.+?)\s+Logon ID:\s+\([^,\s]+[,\s]({login_id}[^\)]+)\)\s+Logon Type:\s+({login_type}\d+)\s+Logon Process:""",
