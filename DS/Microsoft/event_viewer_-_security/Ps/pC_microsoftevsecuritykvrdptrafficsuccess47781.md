...
Name = "microsoft-evsecurity-kv-rdp-traffic-success-4778-1"
    ...
    Fields = [
      ...
      """Client Address=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""""
      ...
      """Key\[0\]=({user}[\w\.\-\!\#\^\~]{1,40}\$?)\s""",
      """Key\[5\]=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
      """Key\[4\]=({host}[^\s]*)\s""",
      """Key\[2\]=({login_id}[^\s]*)\s""",
    ]
