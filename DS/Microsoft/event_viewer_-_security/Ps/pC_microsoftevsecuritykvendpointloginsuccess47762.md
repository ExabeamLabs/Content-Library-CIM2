...
Name = "microsoft-evsecurity-kv-endpoint-login-success-4776-2"
  ...
  Fields = [
    ...
    """Logon (?:a|A)ccount(:|=)\s*(({email_address}[^@\s]+?@[^\s]+?\.[^\s]+?)|(({user}[\w\.\-\!\#\^\~]{1,40}\$?)(?:@({domain}[^\s.;,@=]+).*?)?))[\s;]*Source Workstation(:|=)([\s\\]+|(\s*\\*((({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})?)(:({src_port}\d+))?)|({src_host}[\w\-.]+?))[\s;]*))Error Code(:|=)"""
    """Error Code(:|=)\s*({result_code}[\w\-]+)"""
    """Computer_name\s*:\s*({host}[\w\-.]+)"""
    """<\d+>\w+ \d+ \d\d:\d\d:\d\d ({host}[\w_\-\.]+)"""
    """Source Workstation(:|=)([\s\\]+|(\s*\\*((({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})?)(:({src_port}\d+))?)|({src_host}[\w\-\.]+))[\s;]*))\w+\s*\w+(:|=)"""
