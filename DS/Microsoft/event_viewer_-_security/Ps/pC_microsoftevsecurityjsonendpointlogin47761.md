...
Name = microsoft-evsecurity-json-endpoint-login-4776-1
  ...
  Fields = [
    ...
    """exa_json_path=$..event_data.Workstation,exa_regex=^(({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?|(?:(?!NULL)(\\*({src_host}[^\s"]+))))$""",
    """exa_json_path=$..event_data.Status,exa_field_name=result_code""",
    """exa_json_path=$..TargetUserName,exa_regex=(({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-]{1,40}\$?)(@({domain}[^\s"@]+))?)""",
