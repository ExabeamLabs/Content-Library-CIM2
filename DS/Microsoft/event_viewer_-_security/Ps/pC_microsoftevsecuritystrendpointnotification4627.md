...
Name = microsoft-evsecurity-str-endpoint-notification-4627
  ...
  Fields = [
    ...
    """New Logon:[^"]+?Account Name:\s*(SYSTEM|ANONYMOUS|(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\s*\w+\s\w+:""",
...
# DL Fields are removed
    ...
    """EVENT_TYPE="({result}[^"]+)""""
    """Account Name(:|=)\s*(-|SYSTEM|(({user_upn}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\s*Account Domain"""
  ]
