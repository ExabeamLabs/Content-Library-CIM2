...
Name = "microsoft-evsecurity-kv-endpoint-login-4768-3"
  ...
  TimeFormat = ["yyyy-MM-dd'T'HH:mm:ss", "yyyy-MM-dd HH:mm:ss", "epoch_sec", "MMM dd HH:mm:ss yyyy","yyyy-MM-dd'T'HH:mm:ss.SSSSSSSSSZ"]
  Conditions = [  
  ...
  Fields = [  
    ...
    """Supplied Realm Name(:|=)\s*.*?User ID(:|=)\s*(\\t|\\r|\\n)*(?:NULL SID|({user_sid}[^\s]+?))[\s;]*(\\t|\\r|\\n)*Service Information"""  
    """Pre-Authentication Type:[\\t\s;]*({auth_type}[^\s]+)"""
