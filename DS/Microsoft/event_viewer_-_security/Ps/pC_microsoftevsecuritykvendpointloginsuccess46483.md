...
Fields = [
  ...
  """({event_code}4648)""",
  """Account Whose Credentials Were Used:.+?Account Name(:|=)[\\t\s]*(-|SYSTEM|(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+(?<!local)(?<!loc))\s|({dest_user}({account}[\w\.\-\!\#\^\~]{1,40}\$?))))[\\r\\t\\n]*\s*(\w+\s\w+:|\w+=|[\w\s]+?\.)"""
  """Subject(:|=).+?Security ID(:|=)[\\t\s]*({user_sid}NULL SID|S-[^\:]+?)[;\s\\n\\r\\t]*\w+\s\w+(:|=)""",
  """Subject(:|=).+?Account Name(:|=)\s*(\\t|\\r|\\n)*(?:-|SYSTEM|(({user_upn}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^<\]\s"\\,\|]+(?<!local)(?<!loc))|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))[\s;]*(\\t|\\r|\\n)*(Account Domain(:|=)|\w+=|[\w\s]+?\.)""",
