...
"""\w+\s+\d+\s\d+:\d+:\d+\s+(::ffff:)?(am|pm|\d{4}|({host}[\w\-.]+))\s*Account Expires:"""
...
"""\w+\s+\d+\s+\d+:\d+:\d+\s+(::ffff:)?(({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?|(am|pm|\d{4}|({dest_host}[\w\-.]+)))\s*Account Expires:"""
""""agent.id":"({agent_id}\d+)"""
