...
Name = "microsoft-evsecurity-str-group-member-remove-success-memberwasremoved"
  ...
  Fields = [
      ...
      """({dest_host}({host}[\w\-.]+))\sMicrosoft-Windows-Security-Auditing""",
      """\s({time}\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d\.\d\d\d\d\d\d[+-]\d\d:\d\d)\s+({dest_host}({host}[\w.-]+))\s""",
      ...
      """"MemberName":"(-|({member}({user_dn}({account_id}[^"]+))))"""",
      """A member was removed from a security-enabled ({group_type}[^\s]+) group.+?Account Name:\s+({src_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)).+?Account Domain:\s+({src_domain}({domain}[^\s]+)).+?Logon ID:\s+({login_id}[^\s]+)\s+""",
