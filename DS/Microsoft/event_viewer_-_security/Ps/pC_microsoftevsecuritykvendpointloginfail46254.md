...
Name = microsoft-evsecurity-kv-endpoint-login-fail-4625-4
  ...
  Fields = [
   ...
   """Estado:\s*(?:[^\s]+)\s*Subestado:\s*({result_code}({failure_code}[^\s]+))\s*Información de proceso:""",
   """Nombre de estación de trabajo:\s*({src_host_windows}({src_host}[\w\-\.]+))\s*Dirección de red de origen:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s*Puerto de orig""",
   """Dominio de cuenta:\s*(\\t)?\$?(-|\$|({domain}[^\s]+))\s*(\\r\s\\r\s\\n)*?Información de error:"""
   """Subestado:\s*(\\t)*?({result_code}({failure_code}[^\\\s]+))\s*(\\r\s)*?(\\r\s\\r\s\\n)*?Información de proceso:"""
   """Dirección de red de origen:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
  ]
