...
Name = microsoft-evsecurity-kv-endpoint-login-fail-4625-4
  ...
  Fields = [
   ...
   """Nombre de estación de trabajo:\s*({src_host_windows}[^\s]+)\s*Dirección de red de origen:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s*Puerto de orig""",
