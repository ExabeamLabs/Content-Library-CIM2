...
Name = microsoft-evsecurity-str-user-lock-success-4740
  ...
  Fields = [
    ...
    """サブジェクト.+?\s*アカウント名:\s*({src_user}.*?)\s*アカウント ドメイン:\s*({src_domain}.*?)\s*ログオン ID:\s*({login_id}.*?)\s*ロックアウトされたアカウント.+?セキュリティ ID:\s*({user_sid}.*?)\s*アカウント名:\s*({user}.*?)\s*追加情報:.+?呼び出し元コンピューター名:\s*({src_host}.*?)\s*("|$|,)"""
    ]
