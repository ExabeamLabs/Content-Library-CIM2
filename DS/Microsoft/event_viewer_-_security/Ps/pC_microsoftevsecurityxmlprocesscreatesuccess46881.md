...
"""Creator Subject:"""
  ...
  Fields = [
   ...
   """Account Name:\s*(|-|(LOCAL SYSTEM|anonymous logon|LOCAL SERVICE|SYSTEM)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s*Account Domain:""",
   ...
   """New Process ID:\s*({process_id}({process_guid}[x\da-f]+))""",
   """<Data Name\\*=('|")NewProcessId('|")>\s*({process_id}({process_guid}[x\da-f]+))<""",
