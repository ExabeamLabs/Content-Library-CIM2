...
"""<Event xmlns"""
  ...
  Fields = [
    ...
    """({event_code}5156)""",
    """<Computer>({local_asset}({host}[^<>]+))</Computer>""",
    """<\d+>\w+ \d+ \d\d:\d\d:\d\d ({host}[\w_\-\.]+)""",
    """<Data Name\\*='ProcessID'>({process_id}[^<>]+)<\/Data>""",
    """<Data Name\\*='Application'>({process_path}({process_dir}(?:[^<]+)?[\\\/])?({process_name}[^\\\/]+?))<\/Data>""",
    """<Data Name\\*='SourceAddress'>(::1|({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})))(:({src_port}\d+))?<\/Data>""",
    """<Data Name\\*='SourcePort'>({src_port}\d+)</Data>""",
    """<Data Name\\*='DestAddress'>(::1|({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})))(:({dest_port}\d+))?<\/Data>""",
