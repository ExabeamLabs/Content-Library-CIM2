...
```Java
{
Name = "microsoft-evsecurity-kv-share-access-5145-1"
Vendor = "Microsoft"
...
Fields = [
  ...
  """Object Type:\s*((\\)[rnt])*({file_type}.+?)\s*((\\)[rnt])*\s*Source Address:""",
  """Source Address:\s*((\\)[rnt])*(::1|({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)((\\)[rnt])*\s*Source Port:""",
  """Share Name:\s*((\\)[rnt])*(?:\\\\\*\\)?({share_name}.+?)\s*((\\)[rnt])*\s*Share Path:""",
  """Share Path:\s*((\\)[rnt])*(?:[\\\?]+)?(?:\s*|({share_path}(({d_parent}.+?)\\)?(|({d_name}[^\\]*?)))\\?)\s*((\\)[rnt])*\s*Relative Target Name:""",
