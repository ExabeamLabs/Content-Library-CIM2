...
Name = microsoft-evsecurity-xml-handle-request-4656
  ...
  Fields = [
    ...
    """<Data Name(\\)?=('|")SubjectDomainName('|")>(NT AUTHORITY|({src_domain}({domain}[^<>]+)))<""",
    """<Data Name(\\)?=('|")SubjectUserName('|")>(SYSTEM|({src_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))<""",
    """<Data Name(\\)?=('|")ObjectServer('|")>({object_server}[^<>]+)<""",
    """<Data Name(\\)?=('|")ObjectType('|")>({object_type}({object_class}File))</Data><Data Name(\\)?=('|")ObjectName('|")>(-|({file_path}({file_dir}[^<]*?)({file_name}[^<>\\\/]+?(\.({file_ext}[^<>\\\/\.]+?))?)))<""",
    """<Data Name(\\)?=('|")ObjectType('|")>({object_type}({object_class}Process))</Data><Data Name(\\)?=('|")ObjectName('|")>(-|({process_path}({process_dir}[^<]*?[\\\/]+)?({process_name}[^<\\\/]+?)))<""",
    """<Data Name(\\)?=('|")ObjectType('|")>({object_type}({object_class}Key))<\/Data><Data Name(\\)?=('|")ObjectName('|")>(-|({registry_path}[^<]*?({registry_key}[^<\\\/]+?)))<""",
    """<Data Name(\\)?=('|")ObjectType('|")>(?!(File|Process|Key))({object_type}({object_class}[^<]+?))</Data><Data Name(\\)?=('|")ObjectName('|")>(-|({object}[^<]+?))<""",
