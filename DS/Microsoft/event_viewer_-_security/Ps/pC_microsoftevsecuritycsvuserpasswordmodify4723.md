...
Name = microsoft-evsecurity-csv-user-password-modify-4723
   ...
   Fields = [
     ...
     """アカウント名:\s*({user}[\w\.\-\!\#\^\~]{1,40}\$?)\s*アカウント ドメイン:\s*({domain}.*?)\s*ログオン ID:\s*({login_id}.*?)\s*ターゲット.+?セキ>ュリティ ID:\s*({dest_user_sid}.*?)\s*アカウント名:\s*({dest_user}.*?)\s*>アカウント ドメイン:\s*({dest_domain}.*?)\s*追加情報"""
   ]
