...
"""Subject:[^=]+?Account Name:\s*([\\t]*)({src_user}[\w\.\-\!\#\^\~]{1,40}\$?)\s*([\\t]*)Account Domain:\s*(?=\w|([\\t]*)|NA)({src_domain}[^:]+?)\s*([\\t]*)Logon ID:\s*({login_id}[^:]+?)\s*Account That Was"""
...
"""<Data Name\\*=('|")TargetDomainName('|")>(?:\\+)?({domain}[^<=\s]+)(<|\s)"""
"""<Level>({run_level}[^<]+)<"""
]
