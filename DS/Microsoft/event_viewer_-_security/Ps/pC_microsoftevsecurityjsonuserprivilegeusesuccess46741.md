...
Fields = [
  ...
  """\"(SubjectUserName)\"\s*:\s*\"(-|({src_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\s*\""""
  """\"(SubjectDomainName)\"\s*:\s*\"(-|({src_domain}({domain}.+?)))\s*\""""
  ...
  """exa_json_path=$.host,exa_regex=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?$"""
  ...
  """exa_json_path=$.event_data.SubjectUserName,exa_regex=(?:-|(?i)(LOCAL SYSTEM|anonymous logon|LOCAL SERVICE|SYSTEM)|({src_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\\?$"""
  """exa_json_path=$.event_data.SubjectDomainName,exa_regex=(|-|NT Service|NT AUTHORITY|({src_domain}({domain}[^\"]+)))\\?$"""
