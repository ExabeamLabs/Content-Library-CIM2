...
```Java
{
Name = microsoft-azuremon-json-app-activity-success-compliance
  Product = Microsoft Intune
  ParserVersion = v1.0.0
  Conditions = [ """"tenantId":""", """"category":"OperationalLogs"""", """"operationName":""", """"IntuneAccountId":""" ]
...
microsoft-azuremon-json-events = {
  ...
  Fields = [
    ...
    """exa_json_path=$..UserEmail,exa_regex=(-|system|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
    """exa_json_path=$..UPN,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
    ...
    """exa_json_path=$..UserName,exa_regex=(-|({full_name}[^"\s]+\s[^"]+)(\s*\(\w+\))?|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
