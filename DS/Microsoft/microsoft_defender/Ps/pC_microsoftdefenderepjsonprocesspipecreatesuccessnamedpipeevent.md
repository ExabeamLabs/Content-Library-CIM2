...
```Java
{
Name = "microsoft-defenderep-json-process-pipe-create-success-namedpipeevent"
  ExtractionType = json
  Conditions = [  """DeviceEvents"""", """"ActionType":"NamedPipeEvent"""" ]
  Fields = ${MicrosoftParserTemplates.json-defender-atp.Fields} [
    """exa_regex="DesiredAccess\\?":\\?"?({access}[^\\",]+)""",    
    """exa_regex="FileOperation\\?":\\?"({operation}[^\\"]+)""",    
    """exa_regex="PipeName\\?":\\?"({pipe_name}[^"]+?)\\?"""",        
  ]
