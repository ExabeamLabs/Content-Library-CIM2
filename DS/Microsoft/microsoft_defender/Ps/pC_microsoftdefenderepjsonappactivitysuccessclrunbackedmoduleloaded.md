...
```Java
{
Name = "microsoft-defenderep-json-app-activity-success-clrunbackedmoduleloaded"
  ExtractionType = json
  Conditions = [  """DeviceEvents"""", """"ActionType":"ClrUnbackedModuleLoaded"""" ]
  Fields = ${MicrosoftParserTemplates.json-defender-atp.Fields} [
    """exa_regex="ModuleILPathOrName\\?":\\?"\\?({module_path}({module_dir}[^"]*?)\\+({module_name}[^"]+))\\?"""",
    """exa_regex="ModuleId\\?":\\?"({module_id}[^\\"]+)"""    
  ]
