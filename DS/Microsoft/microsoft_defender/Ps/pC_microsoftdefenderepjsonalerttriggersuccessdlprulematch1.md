...
Name = microsoft-defenderep-json-alert-trigger-success-dlprulematch-1
  ...
  Fields = [
    ...
    """"From":\s*"\s*\|?\s*(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@({domain}[^@"]+))?)""",
    """"To":\s*\[({email_recipients}"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))[^\]]*?")\]""",
    """"To":\s*\[({target}[^\]]+)\]""",
    """src-account-name":"({account_name}[^"]+)""",
    """Operation":\s*"({additional_info}[^"]+)"""",
    """"Workload":\s*"({alert_source}[^",]+)"""",
    """\ssuser=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s""",
    """FileSize":({bytes}\d+),""",
    ...
    """exa_json_path=$.ExchangeMetaData.From,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@({domain}[^@"]+))?)"""
    """exa_regex="To":\s*\[({email_recipients}"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))[^\]]*?")\]"""
    ...
    """exa_json_path=$.SharePointMetaData.From,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@({domain}[^@"]+))?)"""
