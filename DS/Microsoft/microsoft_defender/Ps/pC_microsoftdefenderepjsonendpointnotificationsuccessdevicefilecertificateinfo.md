...
```Java
{
Name = microsoft-defenderep-json-endpoint-notification-success-devicefilecertificateinfo
  ExtractionType = json
  Conditions = [ """"category":""", """"AdvancedHunting-DeviceFileCertificateInfo"""", """"DeviceName":""", """"Issuer":"""]
  Fields = ${MicrosoftParserTemplates.json-defender-atp.Fields} [
    """exa_regex="DeviceId\\?":\\?"({device_id}[^\\"]+)\\?"""",
    """exa_regex="Issuer\\?":\\?"({issuer}[^"]+?)\\?"""",
    """exa_regex="Signer\\?":\\?"({signer}[^"]+?)\\?""""
  ]
