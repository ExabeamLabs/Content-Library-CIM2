...
```Java
{
Name = "microsoft-defenderep-json-endpoint-activity-success-deviceevents"
  ExtractionType = json
  Conditions = [ """DeviceEvents"""", """"ActionType":""", """"DeviceName":"""" ]
  Fields = ${MicrosoftParserTemplates.json-defender-atp.Fields} [
    """exa_regex="FileSize":({bytes}\d+)""",
    """exa_regex="Flags\\?":\\?"({action_flags}[^"\\]+)""",
    """exa_regex="OperationType\\?":\\?"({operation_type}[^"\\]+)""",
  ]
