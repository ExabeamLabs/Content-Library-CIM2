...
```Java
{
Name = "microsoft-defenderep-json-endpoint-notification-advancehuntingdevinfo"
  ExtractionType = json
  ParserVersion = "v1.0.0"
  Product = "Microsoft Defender"
  Conditions = [ """"DeviceCategory":""", """"category":""", """"AdvancedHunting-DeviceInfo"""" ]
  Fields = ${MicrosoftParserTemplates.json-defender-atp.Fields} [
    """exa_regex=ActionType":\s*"({event_name}[^"]+)""",
    """exa_regex=operationName":\s*"({action}[^"]+)""",
    """exa_regex="ClientVersion":"({client_version}[^"]+)""",
    """exa_regex=Sid\\?":\\?"({user_sid}[^"\\]+)""",
    """exa_regex=UserName\\?":\\?"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\\"?"""
    """exa_regex="DomainName\\?":\\?"({domain}[^"\\]+)""",
    """exa_regex="PublicIP":\s*"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
    """exa_regex="OnboardingStatus":"({additional_info}[^"]+)""""
  ]
...
json-defender-atp {
   ...
   Fields = [
     ...
     """"InitiatingProcessAccountName":\s*"(SYSTEM|NETWORK SERVICE|LOCAL SERVICE|Système|system|local service|-|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({full_name}({first_name}[^.\s",]+)[.\s]+({last_name}[^",]+))|({user}[\w\.\-]{1,40}\$?))"""",
     ...
     """AccountUpn":\s*"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""",
     """SHA1":\s*"({hash_sha1}[^"]+)"""",
     ...
     """exa_json_path=$..InitiatingProcessAccountName,exa_regex=(SYSTEM|NETWORK SERVICE|LOCAL SERVICE|Système|system|local service|-|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({full_name}({first_name}[^.\s",]+)[.\s]+({last_name}[^",]+))|({user}[\w\.\-]{1,40}\$?))"""
     ...
     """exa_json_path=$..InitiatingProcessAccountUpn,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
