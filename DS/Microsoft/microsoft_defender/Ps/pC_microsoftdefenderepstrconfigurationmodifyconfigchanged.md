...
```Java
{
Name = "microsoft-defenderep-str-configuration-modify-config-changed"
  Product = Microsoft Defender
  Conditions = [ 
    """Microsoft-Windows-Windows Defender/Operational"""
    """Windows Defender Antivirus Configuration has changed"""
  ]
  Fields = ${WindowsParsersTemplates.windows-defender-1.Fields}[
    """Hostname":"({host}[^"]+?)"""",
    """"+host"+:"+({host}[^"]+)"+""",
    """AccountName":"({user}[\w\.\-\!\#\^\~]{1,40}\$?)"""",
    """({event_name}Windows Defender Antivirus Configuration has changed)"""
  ]
