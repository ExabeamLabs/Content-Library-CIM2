...
"""domainName":"(null|({domain}[^,]+?))\s*""""
""""description":"({additional_info}[^"]+)"""
""""severity":"({alert_severity}[^"]+)"""
""""id":"({alert_id}[^"]+)"""
""""sourceMaterials":\["({malware_url}[^"]+)"""
""""title":"({alert_subject}({alert_name}[^"]+?))(\\u200b)?""""
""""userPrincipalName":"(null|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|(({full_name}[^\s"]+\s+[^"]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))""""
""""userPrincipalName":"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@({domain}[^@"]+))?)\\?""""
