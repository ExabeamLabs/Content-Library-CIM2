...
```Java
{
Name = microsoft-azureadactivity-json-user-mfa-modify-success-adminregister
  Conditions = [ """"activityDisplayName":"Admin registered security info"""", """"resultReason":"Admin registered phone method for user"""" ]
...
azure-ad-activity-json = {
    ...
    Fields = [
      ...
      """exa_json_path=$..initiatedBy.user.userPrincipalName,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
      """exa_json_path=$..initiatedBy..ipAddress,exa_field_name=src_ip""",
      """exa_json_path=$..targetResources[*].userPrincipalName,exa_regex=(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({dest_user}[^@",\s]+))""",
