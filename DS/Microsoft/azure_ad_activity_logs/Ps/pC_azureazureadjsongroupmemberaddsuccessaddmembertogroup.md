...
```Java
{
Name = azure-azuread-json-group-member-add-success-addmembertogroup
  ParserVersion = "v1.0.0"
  Conditions = [ """"activityDisplayName":""", """"Add member to group""", """"operationType":""", """"Assign"""", """"targetResources":""", """"category":""", """"GroupManagement"""" ]
  Fields = ${MicrosoftParserTemplates.microsoft-azuread-json-events.Fields}[
    """exa_regex=({event_name}Self-service password reset)""",
    #"""exa_json_path=$.targetResources[:1].userPrincipalName,exa_field_name=dest_email_address""",
    """exa_json_path=$.targetResources[:1].userPrincipalName,exa_regex=(({external_id}[^_@"]+?\_[^@"]+#(ext|EXT)#@[^"]+)|({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|(({dest_user}[\w\.\-]{1,40}\$?)))""",
