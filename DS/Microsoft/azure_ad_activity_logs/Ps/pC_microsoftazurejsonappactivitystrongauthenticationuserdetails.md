...
Fields = [
  ...
  """exa_json_path=$..initiatedBy.user.userPrincipalName,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)))\s*""",
  """exa_json_path=$..initiatedBy.user.userPrincipalName,exa_regex=({email_address}({user}[\w\.\-\!\#\^\~]{1,40}\$?)@[^\.\"]+\.[^\"]+)"""
  """exa_json_path=$..targetResources[*].userPrincipalName,exa_regex=(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({dest_user}[^@",\s]+))"""
