...
```Java
{
Name = microsoft-azure-cef-dns-request-success-dnsquery
  Product = Azure AD Activity Logs
  ParserVersion = v1.0.0
  Conditions = [ """OperationName":"DNS query"""", """Run command:"""]
  Fields = ${MicrosoftAzureParsersTemplates.cef-azure-app-activity.Fields} [
"""<b>DNS query</b> <b>({dns_query}[^<]+)</b>"""
"""\"SourcePort\",\"value\":\"({src_port}\d+)\""""
"""\"DestinationPort\",\"value\":\"({dest_port}\d+)\""""
  ]

cef-azure-app-activity = {
  Vendor = Microsoft
  Product = Azure
...
"""\Wduser=(anonymous|Unknown|email|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))(\s+\w+=|\s*$)"""
"""\Wsuser=(anonymous|Unknown|email|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))(\s+\w+=|\s*$)"""
...
o365-dlp-email-out = {
  ...
  Fields = [
    ...
    """"+UserId"+:"+((\w{1,5}:\w{1,5}:[^\#]+\#)?({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({full_name}({first_name}[^"\s]+)\s({last_name}[^"]+))|(Unknown|({user_id}[^"]+)))"+""",
