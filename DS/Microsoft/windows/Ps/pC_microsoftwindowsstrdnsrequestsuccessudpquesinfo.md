...
Name = "microsoft-windows-str-dns-request-success-udpquesinfo"
  ...
  Fields = [
    ...
    """({protocol}UDP)\s+({operation}Rcv)\s+({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
    """Remote addr\s*({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})),\s*port\s*({src_port}\d+)""",
