...
# azure_event_hub_name is removed
     ...
     """exa_json_path=$.properties.FolderPath,exa_regex=({file_path}({file_dir}[^"]+[\\\/]+)?({file_name}[^"\\\/\.]+(\.({file_ext}\w+))?))"""
     ...
     """exa_json_path=$.properties.InitiatingProcessIntegrityLevel,exa_field_name=process_integrity"""
  ]
