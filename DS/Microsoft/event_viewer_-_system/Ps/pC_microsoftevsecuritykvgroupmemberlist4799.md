...
Name = microsoft-evsecurity-kv-group-member-list-4799
  ...
  TimeFormat = ["yyyy-MM-dd'T'HH:mm:ss", "MMM dd HH:mm:ss yyyy", "yyyy-MM-dd HH:mm:ss"]
  ...
  Fields = [
    ...
    """exa_json_path=$.Properties.SubjectUserSid,exa_regex=(SYSTEM|({user_sid}\S+))"""
    """exa_json_path=$.Properties.SubjectUserName,exa_regex=(SYSTEM|ANONYMOUS LOGON|({user}\S+))"""
    ...
    """exa_json_path=$.Properties.TargetSid,exa_field_name=group_id"""
    """exa_json_path=$.Properties.TargetUserName,exa_field_name=group_name"""
    """exa_json_path=$.Properties.TargetDomainName,exa_regex=(-|NT AUTHORITY|({group_domain}[^\s\\"]+))"""
    """exa_json_path=$.Properties.CallerProcessId,exa_field_name=process_id"""
    """exa_json_path=$.Properties.CallerProcessName,exa_regex=({process_path}({process_dir}[^,"]*?[\\\/]+)?({process_name}[^\\\/\s"]+?))$"""
  ]
