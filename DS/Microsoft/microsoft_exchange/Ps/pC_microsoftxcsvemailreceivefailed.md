...
""",FAIL,""" 
  ...
  Fields = [
    ...
    """({action}FAIL)""",
    """,\s*(?:'|")?(|MicrosoftExchange.*?|({src_email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@({external_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))(?:'|")?)\s*,(?:[^,]*,){2}Incoming,""",
    ...
    """,\s*"({email_subject}(?:[^"]|"")+?)\s*"\s*,([^,]*,){3}Incoming,""",
    """,\s*(?:'|")?(|MicrosoftExchange.*?|({src_email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)(?:'|")?)\s*,([^,]*,){2}Incoming,""",
    """,\s*(?:'|")?(?:<>|({return_path}[^,]+?))(?:'|")?\s*,([^,]*,)Incoming,""",
    """({direction}Incoming)"""
]
  DupFields = [ "orig_user->email_address" , "src_email_address->external_address"]
