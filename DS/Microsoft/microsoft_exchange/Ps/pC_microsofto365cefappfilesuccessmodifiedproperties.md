...
```Java
{
Name = microsoft-o365-cef-app-file-success-modifiedproperties
  ParserVersion = v1.0.0
  Product = "Microsoft Exchange"
  Conditions= [ """"Workload":"Exchange"""", """"Operation":"Update"""", """"ModifiedProperties"""" ]
  Fields = ${MSParsersTemplates.cef-microsoft-app-activity.Fields} [
    """"Recipients":\[({recipients}"({email_recipients}[^",\]]+)[^\]]*)\]"""
    """"RecordType":\s*"*({object_type}[^,]+?)"*,""",
    """"LogonType":"({login_type}\d+)""",
    """"LogonUserSid":"({user_sid}[^"]+)"""",
    """MailboxOwnerUPN":\s"({owner}({user_upn}[^"]+))",""",
    """"Attachments\\*"+:[\s\\]*"+\s*({attachment}[^"\\]+?)\s*""""
    """InternetMessageId":"({message_id}[^"]+)"""",
    """"ParentFolder":[^=]+?"Path":"\\*({object}[^"]+)"""",
    """"Subject":"({email_subject}[^"]+?)\s*""""
    """"ModifiedProperties"+:[\[]"+\s*({additional_info}[^]]+?)\s*"\]"""    
  ]
