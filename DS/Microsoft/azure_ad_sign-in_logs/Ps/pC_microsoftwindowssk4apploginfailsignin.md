...
"""exa_json_path=$.eventHubsAzureRecord.properties.ipAddress,exa_regex=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
"""exa_json_path=$.eventHubsAzureRecord.properties.userPrincipalName,exa_regex=({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""",
"""exa_json_path=$.eventHubsAzureRecord.properties.conditionalAccessStatus,exa_field_name=result""",
"""exa_json_path=$.eventHubsAzureRecord.properties.appDisplayName,exa_field_name=app""",
"""exa_json_path=$.eventHubsAzureRecord.identity,exa_regex=(({user_id}\w+-\w+-\w+-\w+-\w+)|({full_name}({last_name}[^",\s]+)\s*,?\s*({first_name}[^",]+)))""",
"""exa_json_path=$.eventHubsAzureRecord.properties.userDisplayName,exa_regex=([a-f\d]+(\-[a-f\d]+){4}|({full_name}[^"]+))""",
"""exa_json_path=$.eventHubsAzureRecord.properties.userId,exa_field_name=user_id""",
"""exa_json_path=$.eventHubsAzureRecord.properties.deviceDetail.browser,exa_field_name=browser""",
"""exa_json_path=$.eventHubsAzureRecord.properties.deviceDetail.operatingSystem,exa_field_name=os""",
...
"""exa_json_path=$.eventHubsAzureRecord.properties.location.countryOrRegion,exa_field_name=country_code"""
"""exa_json_path=$.eventHubsAzureRecord.category,exa_field_name=category"""
"""exa_json_path=$.eventHubsAzureRecord.properties.riskLevelAggregated,exa_field_name=severity,exa_match_expr=!Contains($.eventHubsAzureRecord.properties.riskLevelAggregated,"none")"""
...
"""exa_json_path=$.properties.userPrincipalName,exa_regex=({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""",
"""exa_json_path=$.properties.conditionalAccessStatus,exa_field_name=result""",
"""exa_json_path=$.properties.appDisplayName,exa_field_name=app""",
"""exa_json_path=$.identity,exa_regex=(({user_id}\w+-\w+-\w+-\w+-\w+)|({full_name}({last_name}[^",\s]+)\s*,?\s*({first_name}[^",]+)))""",
"""exa_json_path=$.properties.userDisplayName,exa_regex=([a-f\d]+(\-[a-f\d]+){4}|({full_name}[^"]+))""",
"""exa_json_path=$.properties.userId,exa_field_name=user_id""",
"""exa_json_path=$.properties.deviceDetail.browser,exa_field_name=browser""",
"""exa_json_path=$.properties.deviceDetail.operatingSystem,exa_field_name=os""",
"""exa_json_path=$.operationName,exa_field_name=operation""",
"""exa_json_path=$.identity,exa_regex=({last_name}[^",]+),\s*({first_name}[^",\/]+)(\/[^"]*)?("|$)""",
"""exa_json_path=$.properties.location.countryOrRegion,exa_field_name=country_code""",
...
"""exa_regex="identity"+:\s*"+(({user_id}\w+-\w+-\w+-\w+-\w+)|({full_name}({last_name}[^",\s]+)\s*,?\s*({first_name}[^",]+)))""""
