...
Fields = [
      ...
      """exa_json_path=$.userDisplayName,exa_regex=(({full_name}({first_name}[^"\s,]+)\s({last_name}[^\s"]+)))""",
      """exa_json_path=$.userDisplayName,exa_regex=({last_name}[^,"\(]+?)\s*(\([^\)]*\))?,\s*({first_name}[^"\(]+?)\s*(\([^\)]*\))?""",
      """exa_json_path=$.status.failureReason,exa_field_name=failure_reason,exa_match_expr=!Contains(toLower($.status.failureReason),"other")""",
      ...
      """exa_json_path=$.properties.createdDateTime,exa_field_name=time""",
      ...
      """exa_json_path=$.properties.userDisplayName,exa_regex=(({full_name}({first_name}[^"\s,]+)\s({last_name}[^\s"]+)))""",
      """exa_json_path=$.properties.userDisplayName,exa_regex=({last_name}[^,"\(]+?)\s*(\([^\)]*\))?,\s*({first_name}[^"\(]+?)\s*(\([^\)]*\))?""",
      """exa_json_path=$.properties.status.failureReason,exa_field_name=failure_reason,exa_match_expr=!Contains(toLower($.properties.status.failureReason),"other")""",
      """exa_json_path=$.properties.status.errorCode,exa_field_name=error_code""",
      """exa_json_path=$.properties.appDisplayName,exa_field_name=app""",
      """exa_json_path=$.properties.location,exa_field_name=additional_info""",
      """exa_json_path=$.properties.location.city,exa_field_name=location_city""",
      """exa_json_path=$.properties.location.state,exa_field_name=location_state""",
      """exa_json_path=$.properties.location.countryOrRegion,exa_field_name=location_country""",
