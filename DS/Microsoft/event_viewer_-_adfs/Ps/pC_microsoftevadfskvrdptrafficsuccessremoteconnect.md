...
Name = microsoft-evadfs-kv-rdp-traffic-success-remoteconnect
  ...
  Fields = [
    ...
    """Information\s+({dest_host}({host}[^\s]+))""",
    """({event_name}User authentication succeeded)""",
    """User:\s+(({user_upn}[^\s@]+@[^\.\s]+\.[^\s]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)@({domain}[^\s]+?)\.?|({=user}[^\s]+))\s""",
    """Domain:\s+(|({domain}[^\s]+))\s+Source Network Address:"""
    """Source Network Address:\s+({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s+\d+\s"""
  ]
