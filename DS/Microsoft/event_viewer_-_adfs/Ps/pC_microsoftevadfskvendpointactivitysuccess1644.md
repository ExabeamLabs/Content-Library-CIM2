...
Name = microsoft-evadfs-kv-endpoint-activity-success-1644
  ...
  Fields = [
    ...
    """Client:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})?):({src_port}\d+)\s""",
    ...
    """User:\s*(({domain}[^\\\s"]+)\\+)?\s*(CN\s*=|\s*)(({user_upn}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
    """\sUser:\s*(({user_upn}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s"""
