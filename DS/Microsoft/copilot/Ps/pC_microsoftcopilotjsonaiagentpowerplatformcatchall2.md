...
Name = microsoft-copilot-json-ai-agent-powerplatform-catchall-2
  ...
  Fields = [
    ...
    """exa_json_path=$..UserId,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user_id}([\w\.\-\!\#\^\~]{1,40}\$?)))"""
    """exa_json_path=$..UserKey,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user_upn}([\w\.\-\!\#\^\~]{1,40}\$?)))"""
    """exa_json_path=$..PropertyCollection[?(@.Name == 'enduser.principal_name')].Value,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))"""
    ...
    """"UserId"\s*:\s*"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user_id}[\w\.\-\!\#\^\~]{1,40}\$?))"""
    """"enduser.principal_name\\?"\s*:\s*\\?"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user_id}[\w\.\-\!\#\^\~]{1,40}\$?))"""
