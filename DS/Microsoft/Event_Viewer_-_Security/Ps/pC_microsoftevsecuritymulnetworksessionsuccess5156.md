...
```Java
{
Name = microsoft-evsecurity-mul-network-session-success-5156
...
"""The Windows Filtering Platform has permitted a connection"""
  ...
  Fields = [
    ...
    """Direction:\s*({direction}Inbound).*Source Address:\s*(::ffff:)?({dest_ip}[a-fA-F:\d.]+?)\s*Source Port:\s*({dest_port}\d*)\s*Destination Address:\s*(::ffff:)?({src_ip}[a-fA-F:\d.]+?)\s*Destination Port:\s*({src_port}\d*)"""
    """Direction:\s*({direction}Outbound).*Source Address:\s*(::ffff:)?({src_ip}[a-fA-F:\d.]+?)\s*Source Port:\s*({src_port}\d*)\s*Destination Address:\s*(::ffff:)?({dest_ip}[a-fA-F:\d.]+?)\s*Destination Port:\s*({dest_port}\d*)"""
