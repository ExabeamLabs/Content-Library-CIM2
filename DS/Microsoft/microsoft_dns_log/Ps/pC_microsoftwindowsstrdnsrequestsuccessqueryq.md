...
"""   Q ["""
  ...
  Fields = [
    ...
    """\sPACKET\s+\S+\s+({protocol}\S+)\s+({operation}\S+)\s+({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s+\S+\s+Q\s+\[\S+\s+(\s|({dns_query_flags}[^\s]+))\s+\S+\]\s+({dns_query_type}\S+)\s+({dns_query}[^\",\s]+)\"*"""
  ]
