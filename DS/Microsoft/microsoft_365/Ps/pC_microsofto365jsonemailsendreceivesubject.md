...
Name = microsoft-o365-json-email-send-receive-subject
  ...
  ExtractionType = json
  TimeFormat = ["yyyy-MM-dd'T'HH:mm:ss.SSSSSSS","yyyy-MM-dd'T'HH:mm:ss.SSSSSS","yyyy-MM-dd'T'HH:mm:ss.SSSSS","yyyy-MM-dd'T'HH:mm:ss.SZ","yyyy-MM-dd'T'HH:mm:ss.SS","yyyy-MM-dd'T'HH:mm:ss.S","yyyy-MM-dd'T'HH:mm:ssZ","yyyy-MM-dd'T'HH:mm:ss"]
  Conditions = [ 
    """MessageTrace"""
    """Id":"""",
    """"SenderAddress":"""",
  ...
  Fields = [
    ...
    """"receivedDateTime":"({time}\d\d\d\d\-\d\d\-\d\dT\d\d:\d\d:\d\d\.\d\d)"""
    """"Subject":"\s*({email_subject}.+?)\s*"}""",
    """"Subject":"\s*({email_subject}.+?)\s*",""",
    """"Direction":"({direction}[^"]+)"""",
    """"SenderAddress":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)"""",
    """"RecipientAddress":"({email_recipients}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)"""",
    """"RecipientAddress":"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
    """"Size":"?({bytes}\d+)""",
    ...
    """"triggered-by":\{"user-email":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)"""",
    """Category\s+\[({category}[^\]]+)\]""",
    """"Action":"({action}[^",]+)"""",
    """"ToIP":"({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
    """dproc=({log_source}[^"\s]+)""", 
    ...
    """exa_regex="SenderAddress":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)"""",
    """exa_regex="RecipientAddress":"({email_recipients}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)"""",
    """exa_regex="RecipientAddress":"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
    ...
    """exa_regex="triggered-by":\{"user-email":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)"""",
    """exa_json_path=$.Category,exa_field_name=category"""
    """exa_json_path=$.Action,exa_field_name=action"""
    """exa_regex="ToIP":"({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
    """exa_json_path=$.MalwareName,exa_field_name=malware_name"""
    """exa_json_path=$.BulkComplaintLevel,exa_field_name=spam_score"""
    """exa_regex="receivedDateTime":"({time}\d\d\d\d\-\d\d\-\d\dT\d\d:\d\d:\d\d\.\d\d)"""
    """exa_regex=dproc=({log_source}[^"\s]+)"""
  ]
