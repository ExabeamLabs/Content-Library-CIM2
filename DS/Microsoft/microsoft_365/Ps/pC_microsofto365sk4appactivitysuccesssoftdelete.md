...
Name = microsoft-o365-sk4-app-activity-success-softdelete
  ...
  Conditions= [ """"MailboxOwnerUPN":"""", """"OriginatingServer":"""", """"Workload":""", """"Operation":"SoftDelete"""", """"UserId":"""" ]
...
cef-microsoft-o365-app-activity = {
  ...
  Fields = [
    ...
    """"OriginatingServer":(\s*|)"({src_host}({host}\w+))\s*(\([^\)]+?\))?(\\r\\n)?"""",
    """CEF:([^\|"]*\|){5}({operation}[^\|"]+)""",
    ...
    """"ClientIP":"(::1|::ffff:|\[?(::ffff:)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})?)(\]:({src_port}\d+))?)"""",
