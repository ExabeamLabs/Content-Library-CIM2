...
Name = microsoft-o365-mix-app-login-success-teamssessionstarted
  ...
  Fields = [
    ...
    """\Wsuser=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@({domain}[^\s"]+))?)""",
    """"Operation":"({operation}[^"]+)"""",
    ...
    """exa_regex=ClientIP":"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""",
    """exa_json_path=$.UserId,exa_field_name=user_upn"""
    """ObjectId"*:\s*"*((Unknown)|({object}[^"]+))"*"""
    """exa_regex=ObjectId"*:\s*"*((?i)(Unknown)|({object}[^"]+))"*"""
    """exa_regex="+UserId"+:"+((\w{1,5}:\w{1,5}:[^\#]+\#)?({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({full_name}({first_name}[^"\s]+)\s({last_name}[^"]+))|(Unknown|({user_sid}[^"]+)))"+"""
