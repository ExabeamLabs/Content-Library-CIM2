...
Name = microsoft-o365-cef-app-activity-success-inboxrule
  ...
  Fields = ${MicrosoftAzureParsersTemplates.cef-microsoft-app-activity-1.Fields}[
    ...
    """"ForwardTo":"+(smtp:)?({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""""
...
cef-microsoft-app-activity-1 = {
  ...
  Fields = [
    ...
    """"+UserId"+:"+((\w{1,5}:\w{1,5}:[^\#]+\#)?({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({full_name}({first_name}[^"\s]+)\s({last_name}[^"]+))|(Unknown|({user_sid}[^"]+)))"+""",
