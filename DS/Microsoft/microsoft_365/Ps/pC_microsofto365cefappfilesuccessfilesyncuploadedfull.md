...
```Java
{
Name = microsoft-o365-cef-app-file-success-filesyncuploadedfull
  ParserVersion = v1.0.0
  Product = Microsoft 365
  Conditions= [ """"Operation":""", """"FileSyncUploadedFull""", """"Workload":""", """"ItemType":""", """"File"""" ]
  Fields = ${MSParsersTemplates.cef-microsoft-app-activity.Fields} [
    """filePath=\{"ObjectUrl":"({file_path}({file_dir}[^"]+\/)?({file_name}[^"]+(\.({file_ext}[^"\\\/\.]+))?)?)""""
    """"ObjectId":\s*"({object}[^"]+)"""
    """"ObjectId":"({file_path}({file_dir}[^"]+[\/])({file_name}[^"]+?(\.(?!(_|-|\{))({file_ext}[^\\\.\s)"]+))?))""""
    ]
