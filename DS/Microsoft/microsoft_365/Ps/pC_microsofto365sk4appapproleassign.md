...
```Java
{
Name = microsoft-o365-sk4-app-approleassign
  Conditions = [ """"Workload""", """"ResultStatus""", """"Operation""", """"Add app role assignment grant to user""" ]
...
o365-activity-template = {
  ...
  Fields = [
    ...
    """"UserId\\*"+:[\s\\]*"+({user_upn}[^",]+)",""",
    """"MailboxOwnerUPN\\*"+:[\s\\]*"+({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))>?\s*"+""",
