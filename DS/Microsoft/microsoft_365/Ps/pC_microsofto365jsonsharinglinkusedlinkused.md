...
Name = microsoft-o365-json-sharing-link-used-linkused
  ...
  Fields = ${MSO365ParsersTemplates.m365-activity.Fields}[
    ...
    """SiteUrl":"({url}[^"]+)"""",
    """"(?i:c)orrelationId":\s*"({correlation_id}[^"]+)"""",
    """"SourceRelativeUrl":"({src_file_dir}[^"]+)"""",
    """"EventSource":"({log_source}[^"]+)"""",
    """"Platform":"({os}[^"]+)"""",
    """"AuthenticationType":"(None|({auth_type}[^"]+))"""",
    """"ApplicationDisplayName":"(Unknown|({app}[^"]+))""",
    ...
    """exa_json_path=$.SourceRelativeUrl,exa_field_name=src_file_dir""",
    """exa_json_path=$.SiteUrl,exa_field_name=url""",
    """exa_json_path=$.EventSource,exa_field_name=log_source""",
    """exa_json_path=$.Platform,exa_field_name=os""",
    """exa_json_path=$.AuthenticationType,exa_regex=(None|({auth_type}[^"]+))""",
    """exa_json_path=$.ApplicationDisplayName,exa_regex=(Unknown|({app}[^"]+))""",
    """exa_json_path=$.UserKey,exa_regex=(anonymous|({user_id}[^@\"]+))""",
    """exa_json_path=$.UserAgent,exa_field_name=user_agent""",
    """exa_json_path=$.Permission,exa_field_name=permissions"""
  ]
