...
```Java
{
Name = microsoft-o365-kv-file-write-success-filesyncuploadedfull
Conditions = [
  """SESSID="""
  """RESULTCODE="""
  """WORKLOAD=OneDrive"""
  """COMMAND=FileSyncUploadedFull"""
  """OBJECT="""
]
...
azure-app-activity-skyfromation= {
  ...
  Fields = [
    ...
    """"caller"+:"+((\w+?_)?(\w+-)?\w+-\w+-\w+-\w+|(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))"""
    """"channels"+:"+({channels}[^"]+)""",
    """"claims".+?aud"+:"+({aud}[^"]+)""",
    """"claims".+?iss"+:"+({iss}[^"]+)""",
    """"claims".+?iat"+:"+({iat}[^"]+)""",
