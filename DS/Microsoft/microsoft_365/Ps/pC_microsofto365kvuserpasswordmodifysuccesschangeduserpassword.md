...
"""USER=(Unknown|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@({domain}[^\s]+))?)\s+\w+="""
"""DOMAIN=(|({domain}[^\s]+?))\s+\w+="""
"""WORKLOAD=({app}[^=]+?)\s+\w+="""
"""COMMAND=({event_name}[^=]+?)\s+\w+="""
"""OBJECT=(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({dest_user}[^=]+?))\s+\w+="""
