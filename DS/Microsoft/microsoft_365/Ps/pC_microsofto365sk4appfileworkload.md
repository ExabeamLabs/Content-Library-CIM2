...
""""UserId\\*"+:[\s\\]*"+(({domain}[^"\\]+)\\+)?(({email_address}[^@"]+@[^."]+\.[^"]+?)|(SecurityComplianceAlerts|(Unknown|Sync|AirInvestigation|(\w+?_)?(\w+-)?\w+-\w+-\w+-\w+|({user}[^"@\\\s]*))))"""",
...
"""\ssuser=(anonymous|SecurityComplianceAlerts|({email_address}[^\s]+@[^\s]+\.[^\s]+)|(Unknown|Sync|AirInvestigation|(\w+?_)?(\w+-)?\w+-\w+-\w+-\w+|({user}[^"\s@]+?)))\s""",
