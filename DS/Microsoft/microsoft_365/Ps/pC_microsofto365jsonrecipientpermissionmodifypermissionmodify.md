...
```Java
{
Name = microsoft-o365-json-recipient-permission-modify-permissionmodify
  ParserVersion = v1.0.0
  Conditions = [ """"Operation":"""", """RecipientPermission"""", """"Workload":"Exchange"""" ]
  Fields = ${MSO365ParsersTemplates.m365-activity.Fields}[
    """/"Name":"Trustee","Value":"(({dest_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.-])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|(system|({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?)))"\}"""",
    """"Value":"(({dest_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.-])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|(system|({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?)))","Name":"Trustee"""",
    """"Name":"AccessRights","Value":"({permissions}[^"]+)"""",
    """"Value":"({permissions}[^"]+)","Name":"AccessRights"""",
