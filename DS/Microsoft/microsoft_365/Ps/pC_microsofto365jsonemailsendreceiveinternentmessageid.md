...
Name = microsoft-o365-json-email-send-receive-internentmessageid
  ...
  Fields = [
    ...
    """"P2Sender":"((([^@]+?\\=)+)?({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({email_user}[^"]+))"""",
    """"P1Sender":"((([^@]+?\\=)+)?({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({email_user}[^"]+))""""
    """AttachmentData\\?"*:\s*({attachment}\[\{\\?"*FileName\\?"+:\s*\\?"+({file_name}[^"\\]+)?\\?"+\,.+?FileType\\?"+:\s*\\?"+({file_ext}[^"\\;]+)?(;[^"\\;]+)?\\?"+\,.+?\])"""
    """"DeliveryAction":"({action}[^"]+)"""",
    """"Workload":\s*"({app}[^"]+)"""",
    """"UserType":"*({user_type}[^,}"]+)"*""",
    """"PhishConfidenceLevel":"({confidence_level}[^"]+)"""",
    """"PolicyAction":"({malware_action}[^"]+)"""",
    """"OriginalDeliveryLocation":"({src_email_folder}[^"]+)"""",
    """"LatestDeliveryLocation":"({dest_email_folder}[^"]+)""""
]
