...
```Java
{
Name = microsoft-o365-cef-app-file-success-memberadded
  ParserVersion = v1.0.0
  Product = Microsoft 365
  Conditions= [ """"Operation":""", """"MemberAdded""", """"Workload":""" ]
  Fields = ${MSParsersTemplates.cef-microsoft-app-activity.Fields}[
    """"UPN":"({user_upn}[^"]+)""",
    """"TeamName"+:"+({group_name}[^"]+)""",
    """"Workload"+:"+({app}[^"]+)"""
    """"Key"+:"+ClientName"+,"+Value"+:"+({client_name}[^"]+)"""
    """"UPN"+:"+(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({dest_user}[^@",\s]+))""""
    """"Key":"OsName","Value":"({os}[^"]+)""""
]
