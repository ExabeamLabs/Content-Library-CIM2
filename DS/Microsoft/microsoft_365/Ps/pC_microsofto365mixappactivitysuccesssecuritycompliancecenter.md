...
Fields = [
  ...
  """Workload"*:\s*"*({app}[^"]+)"""
  """"ObjectId":\s*"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)",""""
  """"f3u\\?":\\?"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)\\?""""
  """trc\\*"+:\\*"+({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
  """"app-user-displayname":"({full_name}[^"]+)""",
  """"user-email":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
  """"ResultStatus":"({result}[^"]+)"""
  ...
  """exa_json_path=$..['Workload'],exa_field_name=app"""
  """exa_regex="f3u\\?":\\?"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)\\?""""
  """exa_regex=trc\\*"+:\\*"+({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
