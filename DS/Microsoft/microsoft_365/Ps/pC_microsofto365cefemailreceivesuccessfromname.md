...
Name = microsoft-o365-cef-email-receive-success-fromname
  ...
  Fields = [
    ...
    """ValidatedFromAddress\\=({email_address}[^@;]+@({email_domain}[^@\s;]+));""",
    """\Wact=({alert_name}.+?)\s+(\w+=|$)""",
    """Id\\=({alert_id}[^;\s]+)""",
    """({direction}o)"""
  ]
  DupFields = [ "email_address->email_user","email_address->orig_user", "alert_name->alert_type", "to_address->email_recipients", "dest_email_address->external_address" ]
