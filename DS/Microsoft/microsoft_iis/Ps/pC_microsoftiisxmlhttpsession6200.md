...
"""<Data Name ='c-ip'>(::1|127.0.0.1|({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)<"""
"""<Data Name ='s-ip'>(::1|127.0.0.1|({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?)<"""
...
"""<Data Name ='cs-username'>(-|(0#\.f\|ww\|)?({user}[^@.<]+)@({domain}[^.\d<]+)?)<"""
"""<Data Name ='cs-username'>(-|((0#\.f\|ww\|)|(0#\.w\|))?(((({domain}[^\\]+)\\)?({user}[^@<]+?))|({email_address}[^@]+@[^.<]+?\.[^<]+)))<"""
]
