...
Name = microsoft-iis-str-http-session-getapi
  ...
  Fields = [
    ...
    """({time}\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})\s({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s({method}GET)?\s*([^\s]+\s){2}({dest_port}\d+)\s(({domain}[^\\\s]+)\\+)?(-|({user}[\w\.\-]{1,40}))?\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s({user_agent}[^\s]+)\s({url}[^\s]+)\s({http_response_code}\d+)"""
    """"timestamp":"({time}\d{10})"""
  ]
