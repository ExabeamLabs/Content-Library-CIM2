...
Name = "mcafee-wg-kv-http-session-success-mwgaccess3"
    ...
    Fields = [
      ...
      """mwg:\s+\[({time}[^\]]+)\]""",
      """mwg:\s+\[.+?\]\s+"(?:|({user}[^"]+))"""",
      """mwg:\s+\[.+?\]\s+".*?"\s+({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
      """mwg:\s+\[.+?\]\s+".*?"\s+[^\s]+\s+({http_response_code}\d+)""",
      """mwg:\s+\[.+?\]\s+".*?"\s+[^\s]+\s+\d+\s+"({method}[^\s]+)""",
      """mwg:\s+\[.+?\]\s+".*?"\s+[^\s]+\s+\d+\s+"\w+\s+(({protocol}\w+):\/+)?""",
      """mwg:\s+\[.+?\]\s+".*?"\s+[^\s]+\s+\d+\s+"\w+\s+({url}(\w+:\/+)?({web_domain}(?:({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?|[^\/:]+))[^\s"]+)""",
      ...
      """mwg:\s+\[.+?\]\s+".*?"\s+[^\s]+\s+\d+\s+(".*?"\s+){4}(\d+\s+){2}(".*?"\s+){4}({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
    ]
