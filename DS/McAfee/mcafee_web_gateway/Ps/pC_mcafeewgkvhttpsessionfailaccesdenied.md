...
Name = "mcafee-wg-kv-http-session-fail-accesdenied"
    ...
    Fields = [
      ...
      """mwg:\s+Acces Denied\s+\[.+?\]\s+(".*?"\s+){2}({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
      """mwg:\s+Acces Denied\s+\[.+?\]\s+(".*?"\s+){2}[^\s]+\s+({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
