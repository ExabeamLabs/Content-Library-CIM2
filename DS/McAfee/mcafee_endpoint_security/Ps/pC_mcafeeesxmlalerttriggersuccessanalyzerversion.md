...
Name = "mcafee-es-xml-alert-trigger-success-analyzerversion"
	...
	"""<(DetectedUTC|GMTTime)>({time}\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d)""",
        ...
        """<IPAddress>(::1|({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)""",
        """<SourceUserName>((NT-AUTORITÄT|AUTORIDADE NT|NT AUTHORITY|({domain}[^\\\s]+))\\+)?((?i)system|({user}[^<\\\s]+))<\/SourceUserName>""",
       """<DomainName>({domain}[^<]+)""",
       """<UserName>((NT-AUTORITÄT|AUTORIDADE NT|NT AUTHORITY|({domain}[^\\\s]+))\\+)?((?i)SYSTEM|({user}[^<\\\s]+))<\/UserName>""",
       """<TargetUserName>((NT-AUTORITÄT|AUTORIDADE NT|NT AUTHORITY|({domain}[^\\\s]+))\\+)?((?i)SYSTEM|({user}[^<\\\s]+))<\/TargetUserName>""",
       ...
       """<OSName>({os}[^<]+)""",
       """<ThreatActionTaken>((?i)none|({action}[^<]+))<""",
