...
Name = mcafee-nsp-kv-alert-trigger-success-sensorname
  ...
  Fields = [
    ...
    """Direction:\s*Outbound;.*?SIP:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}));\s*SPort:\s*(?:N/A|({src_port}\d+));\s*DIP:\s*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}));\s*DPort:\s*(?:N/A|({dest_port}\d+));""",
    """Direction:\s*Inbound;.*?SIP:\s*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}));\s*SPort:\s*(?:N/A|({dest_port}\d+));\s*DIP:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}));\s*DPort:\s*(?:N/A|({src_port}\d+));""",
