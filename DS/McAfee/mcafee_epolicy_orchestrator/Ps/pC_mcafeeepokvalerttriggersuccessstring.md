...
Name = mcafee-epo-kv-alert-trigger-success-string
  ...
  Fields = [
    ...
    """:epoEventTargetIPV6:\s*'STRING:\s*\\?"({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
    ...
    """:epoEventTargetUserName:\s*'STRING:\s*\\?"(({domain}[^\\\s"']+)\\+)?(SYSTEM|({user}[\w\.\-]{1,40}\$?))\\?"""",
    """:epoEventThreatCategory:\s*'STRING:\s*\\?"({alert_type}[^"]+?)\\?"""",
    """:epoEventThreatSeverity:\s*'STRING:\s*\\?"({alert_severity}[^"']+?)\\?"""",
    """:epoEventThreatActionTaken:\s*'STRING:\s*\\?"(None|({result}[^"']+?))\\?"""",
