...
Name = salesforce-sf-json-app-activity-success-type
  ...
  Fields = [
    ...
    """exa_json_path=$.CreatedBy.Email,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
    ...
    """exa_json_path=$.LastModifiedBy.Email,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
    """exa_json_path=$.Subject,exa_field_name=email_subject"""
    """exa_json_path=$.ToAddress,exa_regex=({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|;]+\.[^\]\s"\\,\|;]+)"""
    """exa_json_path=$.ToAddress,exa_field_name=email_recipients"""
