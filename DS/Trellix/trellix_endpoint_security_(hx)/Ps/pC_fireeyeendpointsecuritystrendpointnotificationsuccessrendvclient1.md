...
```Java
{
Name = fireeye-endpointsecurity-str-endpoint-notification-success-rendvclient-1
  ParserVersion = "v1.0.0"
  Conditions = [ """ rendv_client[""", """]: [rendv_client.""", """trellix""" ]
  Fields = ${FireEyeParsersTemplates.fireeye-endpointsecurity-hx-events.Fields}[
    """\w{3}\s\d\d\s\d\d:\d\d:\d\d\s\S+\s\S+\s.+?to server\s({dest_host}[\w\-\.]+)"""
  ]
...
fireeye-endpointsecurity-hx-events = {
  ...
  Fields = [
    ...
    """\w{3}\s\d\d\s\d\d:\d\d:\d\d\s\S+\s\S+\s\[?(\w+\.)?({severity}(NOTICE|ERR(OR)?|WARNING))\]?:?""",
