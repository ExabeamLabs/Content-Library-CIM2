...
Name = mcafee-dlp-kv-alert-trigger-success-lossprevention
  ...
  Fields = [
    ...
    """\WDLP_FileName ="({file_path}({file_dir}[^<>]*?[\\\/<>]+)?({file_name}[^\\\/<>]+?(\.({file_ext}\w+))?))"""",
    """\Wuser="*(N\/A|\s+|NULL|([^\\]+\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""",
    """\Wdest_nt_host="*({src_host}[^\s"]+)""",
    """\Wsrc_ip="*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
