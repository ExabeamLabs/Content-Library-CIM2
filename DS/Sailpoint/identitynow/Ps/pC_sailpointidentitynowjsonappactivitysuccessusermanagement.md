...
""""hostName":\s*"(\d+|({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?|({src_host}[^",]+))""""
""""ipAddress":\s*"({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""""
""""action":\s*"({operation}[^"]+)""""
""""type":\s*"({operation_type}[^",]+)""""
""""actor":[^\}]+?"name":\s*"(Not Available|unknown|({full_name}[^\s",]+(|,)\s[^\s",]+)|({user}[^\s",]+))""""
