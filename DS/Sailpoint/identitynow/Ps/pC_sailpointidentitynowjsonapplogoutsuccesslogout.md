...
Name = sailpoint-identitynow-json-app-logout-success-logout
  ...
  Fields = [
    ...
    """exa_json_path=$.attributes.hostName,exa_regex=(\d+|({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?|({src_host}[^",]+))""",
    ...
    """exa_json_path=$.actor.name,exa_regex=((?i)Not Available|unknown|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)|(({full_name}[^\s"]+\s[^"]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))""",
