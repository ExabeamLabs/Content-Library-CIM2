...
""" user="""
  ...
  Fields = [
    ...
    """\suser=(({email_address}[^@\s]+@[^\.\s]+\.[^\s]+)|(({domain}[^\\\s]+)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s""",
    ...
    """\- \[({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\]\s+(\w+\\)?([\w\s]+?::)?(({email_address}[^@]+@[^\s]+?)|(({domain}[^\\\(]+)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))\([^\[]+\)\[(?!Machine Cert)""", 
