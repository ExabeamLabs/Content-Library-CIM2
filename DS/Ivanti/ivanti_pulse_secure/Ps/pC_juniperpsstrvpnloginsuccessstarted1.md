...
""": Session started for user"""
    ...
    Fields = [
      ...
      """\- \[({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\]\s+([\w\s]+?::)?(({domain}[^\\\(]+)\\+)?(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|\(]+\.[^\]\s"\\,\|\(]+)|({full_name}[^,\(]+,\s*\w+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s*({realm}[^\[]+)?\[(?!Machine Cert)""",
