...
""" has been terminated."""
  ...
  Fields = [
    ...
    """Session for user\s+(({email_address}[^@\s]+@[^\.\s]+\.[^\s]+)|(({domain}[^\\]+)\\)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s+on host ({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))? has been terminated"""
  ]
