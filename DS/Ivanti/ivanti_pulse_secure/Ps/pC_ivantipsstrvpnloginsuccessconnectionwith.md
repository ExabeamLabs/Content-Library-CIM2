...
"""Established connection with type"""
  ...
  Fields = [
    ...
    """PulseSecure:.+?-\s({time}\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d)\s*-\s*[^\s]+\s*-\s*\[({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\]\s({account}({user}[\w\.\-\!\#\^\~]{1,40}\$?))\(({realm}[^\)]+)\)"""
