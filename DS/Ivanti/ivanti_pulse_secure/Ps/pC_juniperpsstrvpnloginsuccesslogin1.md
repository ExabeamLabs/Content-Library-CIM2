...
"""Agent login succeeded for"""
  ...
  Fields = [
    ...
    """({event_name}Agent login succeeded) for (({domain}[^\\]+)[\\])?({user}[\w\.\-\!\#\^\~]{1,40}\$?)(?:(@|\\)({=domain}[^\/\s]+))?[^=]*? from ({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
    """\(({realm}[^\)]+)\)\[ ]""",
