...
"""Network"""
    ...
    Fields = [
      ...
      """\s+({dest_host}({host}[\w-.]+))\s+PulseSecure:""",
      ...
      """\- \[({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\]\s+([\w\s]+?::)?(({email_address}[^@\s\(]+@[^\.\s\(]+\.[^\s\(]+)|(({domain}[^\\]+)\\)?({account}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\(({realm}[^\[]+)?\)\[(?!Machine Cert)""",
      """with IP(?:v4 address)?\s+({src_translated_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))""",
