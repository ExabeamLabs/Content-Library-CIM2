...
""" (session:"""
    ...
    Fields = [
      ...
      """Login succeeded for (?:({email_address}[^@\s]+@[^@\s\/]+)|({account}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))""",
      ...
      """PulseSecure:.*?\[(::ffff:)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\]\s+(({domain}[^\\]+)\\+)?(?:({email_address}[^@\s]+@[^@\(]+)|({account}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\(({realm}[^\)]+)?""",
      """Mozilla\/\d+.\d\s*\(({os}[^\)]+)\)"""
    ]
