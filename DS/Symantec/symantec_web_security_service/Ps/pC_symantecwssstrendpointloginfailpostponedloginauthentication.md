...
```Java
{
Name = "symantec-wss-str-endpoint-login-fail-postponedloginauthentication"
  ParserVersion = "v1.0.0"
  Conditions = [ """ ProxySG: """, """ SSH: Postponed, login-authentication""" ]
  Fields = ${SymantecParsersTemplates.symantec-wss-template.Fields}[
    """ ProxySG: ({event_code}[^\s]+)\s({event_category}SSH)[^:]*:\s*({event_name}Postponed, login-authentication)"""
    """user "({user}[\w\.\-\!\#\^\~]{1,40}\$?)"""
