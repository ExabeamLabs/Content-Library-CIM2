...
Fields = [
  ...
  """[\s,]sender="+\s*({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""""
  ...
  """[\s,]recipients="+\s*(N\/A|({email_recipients}(?:({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))(?:\s*,\s*[^\s"@,]+@[^\s@",]+?\s*?)*)\s*"*,)"""
