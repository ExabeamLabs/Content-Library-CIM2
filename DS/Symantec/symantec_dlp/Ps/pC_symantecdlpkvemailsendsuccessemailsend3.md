...
Fields = [
  ...
  """Machine_IP:\s*({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
  ...
  """Recipients:\s*(N\/A|Unknown|({email_recipients}({dest_email_address}[^@]+@[^,]+)[^:]*),[^:]+Subject:)"""
  """Subject:\s*(N\/A|({email_subject}[^,]+?))\s*,"""
  """Sender:\s*(N\/A|({src_email_address}[^@]+@[^,]+)),[^:]+Recipients:"""
  """Policy:\s*({alert_name}[^,]+)"""
