...
Name = symantec-dlp-cef-alert-trigger-success-dlp-1
    ...
    Fields = [
      ...
      """Protocol="({protocol}[^"]+)""",
      """Sender="((?i)n\/a|({src_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({src_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))""",
      """Machine_IP="({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
