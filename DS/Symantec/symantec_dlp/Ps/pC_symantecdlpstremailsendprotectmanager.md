...
Fields = [
  ...
  """({host}[\w.\-]+)\s+({alert_name}[^\s\^]+)\^\^({alert_id}\d+)\^\^({additional_info}[^\^]+)\^\^[^\^]*\^\^({email_recipients}({dest_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))[^\^]*)\^\^({src_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({src_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|\^]+))\^\^({alert_severity}[^\^]+)\^\^({email_subject}[^\^]+)\^\^(N/A|({object}[^\^]+))\^\^([^\^]*\^\^){9}({protocol}[^\^]+?)\s*(\^|$)"""
  ...
  """\d\d:\d\d:\d\d\s+[\w.\-]+\s+([^\^]*?\^\^){5}({src_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({src_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|\^]+))"""
