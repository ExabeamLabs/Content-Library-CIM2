...
"""User\s*(Name)?:\s+(SYSTEM|None|NETWORK|LOCAL|(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))),Domain"""
"""({operation}File Write)"""
"""Domain\s*(Name)?:\s+(NT AUTHORITY|None|({domain}.+?)),Action Type"""
