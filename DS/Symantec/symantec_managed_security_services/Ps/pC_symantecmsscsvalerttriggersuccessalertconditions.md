...
Name = symantec-mss-csv-alert-trigger-success-alertconditions
  ...
  Fields = [
    """({alert_id}\d+),"({time}\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d\.\d+)",[^\,]+,"({alert_severity}[^"]+)",[^\,]+,"({alert_type}[^"]+)","({alert_name}[^"]+)","({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?",[^\n]+<Symantec MSS alert Conditions>"""
  ]
  SOAR {
