...
```Java
{
Name = symantec-endpointprotection-cef-alert-trigger-success-sedr
  Product = Symantec Endpoint Protection
  ParserVersion = v1.0.0
  Conditions = ["""|Symantec|SEDR|""", """symcSEDRData="""]
  Fields = ${SymantecParsersTemplates.symantec-epp-cef-alert-1.Fields} [
    """({time}\d\d \d\d \d\d\d\d \d\d:\d\d:\d\d)\s"""
    """\|Symantec\|SEDR\|([^|]*?\|){2}({alert_name}[^|]+?)\|""",
    """act=({action}[^\s]+)""",
    """\|Symantec\|SEDR\|([^|]*?\|){2}({alert_type}[^|(]+?)\s*(\([^|]*?)?\|(Unknown|({alert_severity}[^|]+))\|"""
  ]
...
symantec-epp-cef-alert-1 = {
  ...
  Fields = [
    """\srt=({time}\d{13})""",
    """\scs1=({alert_name}[^=]+?)\s+(\w+=|$)""",
    """\sduser=((system|none)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s+(\w+=|$)""",
    """\ssuser=((system|none)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s+(\w+=|$)""",
    ...
    """\scs5=((\(Unknown\) \[-1\])|({process_name}[^=]+?))\s+(\w+=|$)""",
