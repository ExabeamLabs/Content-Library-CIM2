...
Name = symantec-endpointprotection-kv-alert-trigger-success-denialofservice
  ...
  Fields = [
		...
		"""Remote Host Name:\s*(|({src_host}[\w\-.]+)),(Remote Host IP:\s*(?:0+|({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?),)?.*?,Inbound,({protocol}\w+),"""
		"""Remote Host Name:\s*(|({dest_host}[\w\-.]+),),(Remote Host IP:\s*(?:0+|({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?))?.*?,Outbound,({protocol}\w+),"""
