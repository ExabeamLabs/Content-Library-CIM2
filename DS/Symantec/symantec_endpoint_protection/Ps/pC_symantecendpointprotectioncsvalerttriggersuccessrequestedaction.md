...
Name = symantec-endpointprotection-csv-alert-trigger-success-requestedaction
  ...
  Fields = [
         ...
         """IP 地址:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
         """风险名称:\s*({alert_name}[^,]+)""",
         """出现次数:\s*\d+,({malware_url}[^,]+)""",
         """用户:\s*({user}[\w\.\-\!\#\^\~]{1,40}\$?)""",
         """计算机名:\s*(?:0+|({src_host}[^,]+))""",
         """源计算机:\s*(?:0+|({dest_host}[^,]+))?,""",
         """源 IP:\s*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
