...
Name = symantec-endpointprotection-json-peripheral_storage-insert-fail-blockautoruninf
  ...
  Fields = ${SymantecParsersTemplates.json-symantec-endpoint-protection.Fields}[
    ...
    """({operation}Block)""",
    """exa_regex=({operation}Block)""",
    """"file":[^\}]+?"path":"({file_path}({file_dir}(?:[^";]+)?[\\\/;])?({file_name}autorun\.inf))[^\}]+"size":({bytes}\d+)"""
    """"type":"({alert_name}({alert_type}[^"]+))"""",
    """exa_regex="type":"({alert_name}({alert_type}[^"]+))"""",
    """exa_json_path=$.type_id,exa_field_name=event_code"""
    """"type_id":({event_code}8031)"""
    """exa_regex="file":[^\}]+?"path":"({file_path}({file_dir}(?:[^";]+)?[\\\/;])?({file_name}autorun\.inf))[^\}]+"size":({bytes}\d+)"""
