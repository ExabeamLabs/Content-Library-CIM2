...
"""\WinternalIP=({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
"""\WinternalHost=({src_host}[^=]+?)(\s+\w+=|\s*$)"""
"""\Wmd5=({hash_md5}[^=]+?)(\s+\w+=|\s*$)"""
"""\Wuser_name=({user}[^=]+?)(\s+\w+=|\s*$)"""
""""threat":\{[^\}]*?"name":"({alert_name}[^"]+)"""
""""receivers":.*?"email_address":"({email_address}[^"]+)".*?"direction":"I""""
""""direction":"I".*?"receivers":.*?"email_address":"({email_address}[^"]+)""""
