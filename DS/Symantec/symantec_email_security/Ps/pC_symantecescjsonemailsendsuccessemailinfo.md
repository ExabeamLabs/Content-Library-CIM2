...
Name = symantec-esc-json-email-send-success-emailinfo
   ...
   Fields = [
     ...
     """"headerTo":\["({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))"""",
     """"isOutbound":({direction}[^,]+),""",
     """"senderIp":"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
     """\[\{"fileNameOrURL":"({file_name}({email_attachment}[^\.]+\.({file_ext}[^"]+)))"""

     """exa_json_path=$.emailInfo.mailProcessingStartTime,exa_field_name=time""",
     """exa_json_path=$.emailInfo.headerFrom,exa_regex=^({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))$""",
     ...
     """exa_json_path=$.emailInfo.headerTo,exa_field_name=email_recipients""",
     """exa_json_path=$.emailInfo.headerTo[0],exa_regex=^({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))$""",
     ...
     """exa_json_path=$.emailInfo.filesAndLinks[0].fileNameOrURL,exa_regex=^({file_name}({email_attachment}[^\.]+\.({file_ext}[^"]+)))$"""
   ]
