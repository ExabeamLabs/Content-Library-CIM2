...
Fields = [
  ...
  """REJECT(?:\s[^\s]+){2}\s(-|({dest_ip}(((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))))\s*({dest_port}\d+)?\s+({time}\d{10})"""
  ...
  """REJECT(?:\s[^\s]+){16}\s({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
