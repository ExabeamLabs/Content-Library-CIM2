...
Name = amazon-awsguardduty-json-database-login-success-rdssuccessfullogin
  ...
  Fields = ${AwsGuardDutyParserTemplates.json-aws-guardduty-security-alert-template.Fields} [
    """"userName":"(({aws_email_address}({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({aws_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))"""",
    """exa_regex="userName":"(({aws_email_address}({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({aws_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))"""
  ]
