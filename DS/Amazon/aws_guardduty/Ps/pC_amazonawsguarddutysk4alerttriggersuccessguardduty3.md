...
Conditions = [ """\"source\":\"aws.guardduty\"""", """\"detail-type\":\"GuardDuty Finding\"""" ]
  Fields = [
    ...
    """"remoteIpDetails[^=]+?"ipAddressV4\\?":\\?"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
    ...
    """"userName\\?":\\?"(({aws_email_address}({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))|({aws_account}({user}[\w\.\-\!\#\^\~]{1,40}\$?)))""",
