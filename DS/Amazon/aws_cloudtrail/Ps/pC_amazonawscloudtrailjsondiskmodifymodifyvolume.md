...
Name = amazon-awscloudtrail-json-disk-modify-modifyvolume
  ...
  """"+requestParameters.+?volumeId\\?":\s*\\?"({resource_id}[^"]+?)\\?"""",
  """"+requestParameters.+?volumeType\\?":\s*\\?"({volume_type}[^"]+?)\\?"""",
  """"+responseElements.+?originalMultiAttachEnabled\\?":\s*\\?({old_multiattach}[^",]+?)\\?,""",
  """"+responseElements.+?targetMultiAttachEnabled\\?":\s*\\?({new_multiattach}[^",]+?)\\?,""",
  """"+responseElements.+?originalSize\\?":\s*\\?({old_size}\d+?)\\?,""",
  """"+responseElements.+?targetSize\\?":\s*\\?({new_size}\d+?)\\?,""",
  ]
