...
Name = amazon-awscloudtrail-json-bucket-policy-modify-putbucketpolicy
  ...
  """exa_json_path=$.requestParameters..Sid,exa_field_name=policy_sids""",
  """exa_json_path=$.requestParameters.bucketPolicy,exa_regex=\{[^\[]+({policy_content}"+Statement\\*"+[^\]]+\])""",
  """exa_regex="+Effect\\?":\s*\\?"Allow\\?".+?Principal\\?":\s*\{?\[?({allowed_users}[^\]\}]+)\]?\}?"Action?""",
  """exa_regex="+Effect\\?":\s*\\?"Allow\\?".+?Action\\?":\s*\[?({allowed_permissions}[^\.\]\}]+)\]?.*"Resource?""",
  """exa_regex="+Effect\\?":\s*\\?"Allow\\?".+?Resource\\?":\s*\{?\[?({allowed_resources}[^\]\}]+)\]?\}?""",
  """exa_regex="+Effect\\?":\s*\\?"Allow\\?".+?Condition\\?":\s*\[?({allowed_conditions}[^\.\]\}]+)\]?"""",
  """exa_regex="+Effect\\?":\s*\\?"Deny\\?".+?Principal\\?":\s*\{?\[?({denied_users}[^\]\}]+)\]?\}?"Action?""",
  """exa_regex="+Effect\\?":\s*\\?"Deny\\?".+?Action\\?":\s*\[?({denied_permissions}[^\.\]\}]+)\]?.*"Resource?""",
  """exa_regex="+Effect\\?":\s*\\?"Deny\\?".+?Resource\\?":\s*\{?\[?({denied_resources}[^\]\}]+)\]?\}?""",
