...
Name = amazon-awscloudtrail-json-file-copy-copyobject
  ...
  """exa_regex=requestParameters.+?key\\?":\s*\\?"({file_path}(({file_dir}.*?)[\\\/]*)({file_name}[^\\\/"]+?(\.({file_ext}[^"\\\/]+))?))\s*""""
 ...
 """exa_json_path=$..bytesTransferredOut,exa_field_name=bytes_out""",
  """exa_json_path=$..resources[*],exa_regex=Object.+?(?:ARN|arn)\\?":\s*\\?"({src_file_arn}[^"]+?)\\?""""
  """exa_json_path=$..resources[*],exa_regex=Bucket.+?(?:ARN|arn)\\?":\s*\\?"({src_bucket_arn}[^"]+?)\\?""""
  """exa_json_path=$..resources[*],exa_regex=Object.+?Object.+?(?:ARN|arn)\\?":\s*\\?"({dest_file_arn}[^"]+?)\\?""""
   """exa_json_path=$..resources[*],exa_regex=Bucket.+?Bucket.+?(?:ARN|arn)\\?":\s*\\?"({dest_bucket_arn}[^"]+?)\\?""""
  ]
