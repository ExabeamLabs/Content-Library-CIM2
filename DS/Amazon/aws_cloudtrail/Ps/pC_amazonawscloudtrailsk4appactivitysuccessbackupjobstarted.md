...
```Java
{
Name = amazon-awscloudtrail-sk4-app-activity-success-backupjobstarted
  # TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """"eventType":"AwsServiceEvent"""", """eventName":"BackupJobStarted"""" ]
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"backupJobId\\?":\s*\\?"({job_id}[^"]+?)\\?"""",
    """"backupVaultName\\?":\s*\\?"({vault_name}[^"]+?)\\?"""",
  ]
