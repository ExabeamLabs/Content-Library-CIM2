...
Name = amazon-awscloudtrail-json-snapshot-create-awsapicall
  ...
  """"+requestParameters.+?({source_resource_type}volume|[Ii]nstance)Id\\?":\s*\\?"({source_resource}[^",]+?)\\?"""",
  """"+requestParameters.+?[Dd]escription\\?":\s*\\?"({description}[^"]+?)\\?"""",
  """"+responseElements.+?snapshotId\\?":\s*\\?"({resource_id}[^"]+?)\\?"""",
  """"+responseElements.+?ownerId\\?":\s*\\?"({owner_id}[^"]+?)\\?"""",
  ]
