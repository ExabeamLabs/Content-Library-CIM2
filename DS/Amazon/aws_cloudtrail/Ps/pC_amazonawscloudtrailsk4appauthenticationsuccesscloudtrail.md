...
Name = amazon-awscloudtrail-sk4-app-authentication-success-cloudtrail
  ...
  Fields = [
    ...
    """"+userIdentity.+?AssumedRole.+?principalId\\?"+\s*:\s*\\?"+?[A-Z\d]{1,50}:(({aws_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({aws_user}[\w\.\-]{1,40}\$?))\\?"+\s*[,\]\}]""",
    """"userName"+\s*:\s*"+?(|({aws_email_address}({email_address}[^@"]+@[^@".]+\.[^@"]+))|({aws_user}({user}[\w\.\-]{1,40}\$?)))"+\s*[,\]\}]""",
