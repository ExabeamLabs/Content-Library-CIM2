...
```Java
{
Name = amazon-awscloudtrail-json-policy-create-success-createpolicy
  #TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":""", """"CreatePolicy"""" ]
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
  """exa_json_path=$.requestParameters.policyDocument,exa_field_name=policy_content"""
  """exa_json_path=$.requestParameters.policyDocument,exa_regex="+Effect\\?":\s*\\?"Allow\\?".+?Action\\?":\s*\[?({allowed_permissions}[^\.\]\}]+)\]?.*\s*"+Resource?"""
  """exa_json_path=$.requestParameters.policyDocument,exa_regex="+Effect\\?":\s*\\?"Allow\\?".+?Resource\\?":\s*\[?({allowed_resources}[^\.\]\}]+)\]?"""
  """exa_json_path=$.requestParameters.policyDocument,exa_regex="+Effect\\?":\s*\\?"Deny\\?".+?Resource\\?":\s*\[?({denied_resources}[^\.\]\}]+)\]?""",
  """exa_json_path=$.requestParameters.policyDocument,exa_regex="+Effect\\?":\s*\\?"Deny\\?".+?Action\\?":\s*\[?({denied_permissions}[^\.\]\}]+)\]?.*\s*"+Resource?"""
  """exa_json_path=$.errorMessage,exa_regex=Policy.*?name=({policy_name}[^"),]+)"""
  """exa_json_path=$.responseElements.policy.policyName,exa_field_name=policy_name"""
  """exa_json_path=$.responseElements.policy.policyId,exa_field_name=policy_id"""
  """exa_json_path=$.responseElements.policy.arn,exa_field_name=policy_arn"""
...
aws-cloudtrail-json = {
    ...
    Fields = [
      ...
      ""","userIdentity\\?":\{[^\}]+"AssumedRole\\?"[^\}]+"principalId\\?":\s*\\?"[A-Z\d]{1,25}:({aws_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))\\?"\s*[,\]\}]""",
