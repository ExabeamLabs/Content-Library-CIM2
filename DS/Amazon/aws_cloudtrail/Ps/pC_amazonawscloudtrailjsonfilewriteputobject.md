...
Name = amazon-awscloudtrail-json-file-write-putobject
  ...
  """exa_json_path=$.requestParameters.key,exa_regex=({file_path}(({file_dir}[^\"]+?)[\\\/]+?)?({file_name}[^"\\\/]+?(\.({file_ext}[^\.\"\\\/]+))?)?)\\?$"""
...
${AwsParserTemplates.aws-cloudtrail-json}{
  ...
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
      """exa_json_path=$.requestParameters,exa_regex=({source_resource_type}[Ss]napshot|[Ii]nstance)Id\\?":\s*\\?"({source_resource}[^",]+?)\\?""""
