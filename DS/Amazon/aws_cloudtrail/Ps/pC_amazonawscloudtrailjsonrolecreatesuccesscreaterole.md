...
```Java
{
Name = amazon-awscloudtrail-json-role-create-success-createrole
  #TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":""", """"CreateRole"""" ]
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
  """exa_json_path=$.requestParameters.assumeRolePolicyDocument,exa_regex="+Effect\\?":\s*\\?"Allow\\?".+?AssumeRole.+?Principal\\?":\s*\[?\{?({allowed_users}[^\]\}]+)\]?\}?""",
  """exa_json_path=$.requestParameters.assumeRolePolicyDocument,exa_regex="+Effect\\?":\s*\\?"Deny\\?".+?AssumeRole.+?Principal\\?":\s*\[?\{?({denied_users}[^\]\}]+)\]?\}?""",
  """exa_json_path=$.responseElements.role.roleName,exa_field_name=identities""",
  """exa_json_path=$.responseElements.role.roleName,exa_field_name=role""",
  """exa_json_path=$.responseElements.role.roleId,exa_field_name=role_id""",
  """exa_json_path=$.responseElements.role.arn,exa_field_name=role_arn""",
