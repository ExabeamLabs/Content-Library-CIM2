...
Name = amazon-awscloudtrail-json-role-create-success-createrole
  ...
  """"+assumeRolePolicyDocument\\?".+?"+Effect\\?":\s*\\?"Allow\\?".+?AssumeRole.+?Principal\\?":\s*\[?\{?({allowed_users}[^\]\}]+)\]?\}?""",
  """"+assumeRolePolicyDocument\\?".+?"+Effect\\?":\s*\\?"Deny\\?".+?AssumeRole.+?Principal\\?":\s*\[?\{?({denied_users}[^\]\}]+)\]?\}?""",
  """"+responseElements":\{"role":\{("[^,]+,)*"roleName\\?":\s*\\?"({identity}[^"]+?)\\?"""",
  """"+responseElements":\{"role":\{("[^,]+,)*"roleId\\?":\s*\\?"({identity_id}[^"]+?)\\?"""",
  """"+responseElements":\{"role":\{("[^,]+,)*"arn\\?":\s*\\?"({identity_arn}[^"]+?)\\?"""",
  """\"roleName\":\"({role_name}[^\"]+)""""
  ]
