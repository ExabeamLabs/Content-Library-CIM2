...
Name = amazon-awscloudtrail-json-bucket-accessblock-modify-awsapicall
  ...
  """"+requestParameters.+?RestrictPublicBuckets\\?":\s*\\?({restrict_public_buckets}[^",]+?),""",
  """"+requestParameters.+?BlockPublicPolicy\\?":\s*\\?({block_public_policy}[^",]+?),""",
  """"+requestParameters.+?BlockPublicAcls\\?":\s*\\?({block_public_acls}[^",]+?),""",
  """"+requestParameters.+?IgnorePublicAcls\\?":\s*\\?({ignore_public_acls}[^",]+?),""",
