...
```Java
{
Name = amazon-awscloudtrail-json-endpoint-modify-instanceattribute
  Vendor = Amazon
  ...
  """"+requestParameters[^\}]+instanceId\\?":\s*\\?"({resource_id}[^"]+?)\\?"""",
  """"+requestParameters[^\}]+userData\\?":\s*\\?"({userdata}[^"]+?)\\?"""",
  """exa_json_path=$..instanceId,exa_field_name=resource_id"""
  """exa_json_path=$..userData,exa_field_name=userdata"""
  ]
