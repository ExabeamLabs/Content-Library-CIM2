...
```Java
{
Name = amazon-awscloudtrail-json-bucket-permission-modify-putobjectacl
  Vendor = Amazon
  ...
  """exa_json_path=$..AccessControlList,exa_field_name=acl_content""",
  """exa_json_path=$..AccessControlList.Grant[0].Grantee.['xsi:type'],exa_field_name=allowed_user_types""",
  """exa_json_path=$..AccessControlList.Grant[0].Grantee.ID,exa_field_name=allowed_ids""",
  """exa_json_path=$..AccessControlList.Grant[1].Grantee.URI,exa_field_name=allowed_uris""",
  """exa_json_path=$..AccessControlList.Grant[1].Permission,exa_field_name=allowed_permissions""",
  """exa_json_path=$..Owner.ID,exa_field_name=owner_id""",
  """exa_json_path=$..requestParameters.key,exa_field_name=file_path"""
  """exa_regex=resources.+?Bucket.+?(?:ARN|arn)\\?":\s*\\?"({bucket_arn}[^"]+?)\\?"""",
  """exa_regex=resources.+?Object.+?(?:ARN|arn)\\?":\s*\\?"({file_arn}[^"]+?)\\?""""
  ]
