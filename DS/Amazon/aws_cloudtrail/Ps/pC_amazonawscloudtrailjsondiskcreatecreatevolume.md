...
Name = amazon-awscloudtrail-json-disk-create-createvolume
  ...
  """"+requestParameters.+?size\\?":\s*\\?({volume_size}[^",]+?)\\?,""",
  """"+requestParameters.+?zone\\?":\s*\\?"({volume_zone}[^"]+?)\\?"""",
  """"+requestParameters.+?volumeType\\?":\s*\\?"({volume_type}[^"]+?)\\?"""",
  """"+requestParameters.+?snapshotId\\?":\s*\\?"({source_resource}[^"]+?)\\?"""",
  """"+responseElements.+?volumeId\\?":\s*\\?"({resource_id}[^"]+?)\\?"""",
  """"key":"Name","value":"({disk_name}[^"]+)"(,|\}|\])"""
  ]
