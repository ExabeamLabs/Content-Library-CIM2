...
```Java
{
Name = vbcorp-v-kv-alert-trigger-success-vbcorp-1
  ...
  Fields = [
    ...
    """\shost=({host}[^\s]+)""",
    """(?:ユーザ名|ユーザ吁):?\s*(|({user}[\w\.\-]{1,40}\$?))\s*IP""",
    """IPアドレス:\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s*""",
