...
Fields = [
  ...
  """usrName =(({domain}[^\\=]+)(\\)+)?(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s+\w+=""",
  ...
  """\sReason=(|({result_reason}[\s\S]*?))\s*(ExtraDetails|PvwaDetails|\[ORIGINAL REASON:|\|SESSLOGS)""",
  """\sAction=({action}[^=]+?)\s+\w+=""",
]
