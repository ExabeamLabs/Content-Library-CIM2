...
"""\ssuser="?(({domain}[^\\=]+)[\\\/]+)?(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))(\s+\w+=|")"""
"""\sduser="?(({dest_domain}[^\\=]+)[\\\/]+)?(({dest_email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({account}({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?)))(\s+\w+=|")"""
"""cs2="*({safe_value}[^="]+?)"*\s+\w+="""
"""\sdst="?({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
"""\sdhost=?(({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?|({dest_host}[\w\-\.]+?))(\s+\w+=|\s*$|")"""
"""\sduser="?(({dest_domain}[^\\=]+)[\\\/]+)?({account}({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
