...
Name = varonis-dsp-kv-alert-trigger-success-alerttriggerd
  ...
  Fields = [
    ...
    """\|\sActing Object SAM Account Name:\s(({user_sid}S\-(\d+\-){6}\d+)|({user}[^\|\s]+?))\s\|""",
    """\|\sDevice IP address:\s({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))\s\|""",
