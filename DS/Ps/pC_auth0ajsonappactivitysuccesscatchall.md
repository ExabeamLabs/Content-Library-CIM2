...
${BlueCoatParserTemplates.bluecoat-proxy}{
  ...
  Fields = ${BlueCoatParserTemplates.bluecoat-proxy.Fields}[
    ...
    """({time}\d\d\d\d-\d\d-\d\d\s\d\d:\d\d:\d\d)\s\d+\s({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s*({src_port}\d+)?\s+({http_response_code}\d+)\s({proxy_action}[^\s]+)\s({bytes_out}\d+)\s({bytes_in}[^\s]+)\s(unknown|({method}[^\s]+))\s({protocol}[^\s]+)\s({web_domain}[^\s]+)\s({dest_port}\d+)\s({uri_path}[^\s]+)\s({uri_query}[^\s]+)\s({user}[^\s]+)\s[^"]+\\?"?({user_agent}[^"]+?)\\?"\s({action}DENIED)\s\\?"((none|({categories}({category}[^\\";]+)[^"]*?))\\?")"""
