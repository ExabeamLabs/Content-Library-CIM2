...
Name = "symantec-bcpa-mix-http-session-connect"
  ...
  Fields = ${BlueCoatParserTemplates.bluecoat-proxy.Fields}[
    ...
    """({host}[\w\-\.]+)\s\d\d\d\d\-\d\d-\d\d\s\d\d:\d\d:\d\d\s(\d+\s)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s(-|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s(\S+\s*){6}"""
