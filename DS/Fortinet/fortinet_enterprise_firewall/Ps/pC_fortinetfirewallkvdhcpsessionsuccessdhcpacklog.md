...
Name = fortinet-firewall-kv-dhcp-session-success-dhcpacklog
  ...
  Conditions = [ """ logver=""", """ logdesc="""", """ dhcp_msg="""", """DHCP Ack log""" ]
  Fields = [
    """\Wdate=({time}\d\d\d\d\-\d\d\-\d\d time\=\d\d:\d\d:\d\d)""",
    """\w+ \d+ \d\d:\d\d:\d\d ({host}[\w\-.]+)""",
    """\Wip=({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
    """\Whostname="({dest_host}[\w\-.]+)"""",
    """\Whostname="({user}[\w\.\-\!\#\^\~]{1,40}\$?)"""",
    """\Wtz="?({tz}[+-]\d+)"""
  ]
