...
Name = "fortinet-fortigate-kv-network-traffic-logid"
  ...
  Conditions = [  """ devname=""", """ devid=""", """ logid=""", """ level=""" ]
  Fields = [
...
# logtype is removed
    ...
    """\slogid=\\?"?({event_id}[^\s"\\]+)""",
    """\slevel=\\?"?({severity}[^=]+?)\\?"?\s+(\w+=|$)""",
    """\sdevname=\\?"*(::ffff:)?({host}[\w\-.]+)""",
    """\sdevid=\\?"*({device_id}[^"\s\\]+)""",
    """\saction=\\?"?({action}[^=]+?)\\?"?\s+(\w+=|$)""",
    """\spolicyid=({policy_id}\d+)""",
    """\sdstip=\\?"*(::ffff:)?({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?\\?""",
    """\ssrcip="*(::ffff:)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\\?""",
...
# poluuid is removed
    """\suser=\\?"?\s*(N\/A|(?:host\/({src_host}[^"\\]+))|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@({domain}[^="]+))?)\s*\\?"?\s+(\w+=|$)""",
# crscore is removed
# crlevel is removed
# craction is removed
