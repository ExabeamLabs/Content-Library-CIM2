...
Name = fortinet-fortigate-kv-configuration-modify-success-objcfg
  ...
  Fields = [
    ...
    """\sdevid="({devid}[^"]+?)"""",
    """\spri="({severity}[^"]+)"""",
    """\suserfrom="(\w+\()?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
    """\smsg="({additional_info}[^"]+?)"""",
    """\sdesc="({event_name}[^"]+)"""",
    """\suser="(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|(ANONYMOUS|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(\@({domain}[^"]+))?))"""",
    """\sadom="({domain}[^"])"""",
    """operation="({operation}[^"]+)"""",
    """performed_on=({object}[^"]+)"""",
