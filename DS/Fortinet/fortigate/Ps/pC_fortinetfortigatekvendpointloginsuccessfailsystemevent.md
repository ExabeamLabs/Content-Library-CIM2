...
```Java
{
Name = fortinet-fortigate-kv-endpoint-login-success-fail-systemevent
   ParserVersion = v1.0.0
   Conditions = [ """CEF:""", """|Fortinet|FortiGate-""", """|system event login|""" ]
...
fortinet-fortigate-cef-network-traffic-info {
 Vendor = Fortinet
  ...
  Fields = [ 
    ...
    """\Wspt=({src_port}\d+)""",
    """\Wdhost=(|({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?|({dest_host}[\w\-.]+?))(\s+[\w\.]+=|\s*$)""",
