...
```Java
{
Name = fortinet-fortiweb-cef-alert-trigger-success-attack
Vendor = Fortinet
Product = Fortiweb Web Application Firewall
TimeFormat = [ "yyyy-MM-dd-HH:mm:ss"]
ParserVersion = "v1.0.0"
Conditions = [ """CEF:""" , """|attack|""", """|alert|""", """|Fortinet|FortiWeb|""" ]
Fields = [
    """deviceCustomDate1=({time}\d\d\d\d-\d\d-\d\d-\d\d:\d\d:\d\d)"""
    """\Wapp=({network_app}[^=]+)(\s+[\w\.]+=|\s*$)""",
    """CEF:([^\|]+\|){4}({attack_id}[^\|]+)\|"""
    """CEF:([^\|]+\|){4}({event_code}[^\|]+)\|({alert_name}({event_name}[^\|]+))\|({alert_type}[^\|]+)\|"""
    """\Wact=(|({action}.+?))(\s+[\w\.]+=|\s*$)"""
    """\WdeviceExternalId=({device_id}[^=]+)\s+\w+="""
    """deviceProcessName =({process_name}[^\$]+?)\s*\w+="""
    """\ssourceServiceName =({alert_source}[^\s]+)"""
    """proto="*({protocol}[^\s"]+)"""
