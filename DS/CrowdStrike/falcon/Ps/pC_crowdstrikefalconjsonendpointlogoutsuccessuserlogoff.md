...
Name = crowdstrike-falcon-json-endpoint-logout-success-userlogoff
  ...
  Fields = [
    ...
    """"aip":\s*"({aip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))"""",
    """"aid":"({aid}[^"]+)"""",
    """"LogonDomain":"({domain}[^"]+)"""",
    """"AuthenticationPackage":"({auth_package}[^"]+)"""",
    """"UserSid":"({user_sid}[^"]+)""",
    """"UserPrincipal":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)(?<!(local))(?<!(loc))(?<!(localdomain))"""",
    """"event_platform":"({os}[^"]+)"""",
    ...
    """exa_json_path=$.UserPrincipal,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)(?<!(local))(?<!(loc))(?<!(localdomain))""",
    """exa_regex="UserPrincipal":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)(?<!(local))(?<!(loc))(?<!(localdomain))"""",
