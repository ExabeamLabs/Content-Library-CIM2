...
Name = "crowdstrike-falcon-json-alert-trigger-success-identityprotection"
  ...
  Fields = [
    ...
    """exa_json_path=$.event.UserName,exa_regex=(({full_name}({first_name}[^\s"\\\/]+)\s({last_name}[^"\\\/]+))|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|((({domain}[^\\\s]+)\\)?({user}[\w\.\-\!\#\^\~]{1,40}\$?)))("|$)""",
    """exa_regex="UserName":"(({full_name}({first_name}[^\s"\\\/]+)\s({last_name}[^"\\\/]+))|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|((({domain}[^\\\s]+)\\)?({user}[\w\.\-\!\#\^\~]{1,40}\$?)))"""",
    ...
    """"UserName":\s*"(({full_name}({first_name}[^\s"\\\/]+)\s({last_name}[^"\\\/]+))|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|((({domain}[^\\\s]+)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?)))"""",
    """"Category":\s*"({category}[^"]+)"""",
