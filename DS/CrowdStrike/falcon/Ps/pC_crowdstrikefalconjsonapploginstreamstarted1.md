...
```Java
{
Name = "crowdstrike-falcon-json-app-login-streamstarted-1"
  ExtractionType = json
  Vendor = "CrowdStrike"
  Product = "Falcon"
  ParserVersion = "v1.0.0"
  TimeFormat = "epoch"
  Conditions = [""""service":""", """"crowdstrike"""", """"type":""", """"AuthActivityAuditEvent"""", """"name":""", """"streamStarted""""]
  Fields = [
    ...
    """exa_json_path=$.attributes.usr.id,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?)|({user_id}api-client-id:[^"]+))$""",
