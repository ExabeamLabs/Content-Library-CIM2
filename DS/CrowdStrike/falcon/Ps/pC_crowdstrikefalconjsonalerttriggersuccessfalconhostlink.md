...
""""RemoteAddress\":\s*\"({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
"""\\*\"DetectDescription\\*\":\s*\\*\"({alert_name}[^\"]+)"""
""""DetectName\":\s*\"({alert_name}[^\"]+)"""
""""Technique\":\s*\"({alert_type}[^\"]+)"""
""""LocalAddress\":\s*\"({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
""""DetectId\"+:\s*\"+({alert_id}[^\"]+)\""""
