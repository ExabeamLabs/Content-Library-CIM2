...
Name = crowdstrike-falcon-sk4-endpoint-login-userloginfail
  ...
  Fields = [
    ...
    """"UserName":\s*"(-|\/+|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({full_name}({first_name}[^\s"@]+)\s({last_name}[^"]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))""""
    """"UserSid":\s*"({user_sid}[^"]+)"""",
    ...
    """"cid":"({cid}[^"]+)"""
    """"event_platform":"({os}[^"]+)""""
    """"LocalAddressIP4":"({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""",
    """"ComputerName":"({host}[\w\-\.]+)""""
    """"ComputerName":"({dest_host}[\w\-\.]+)""""
    """exa_json_path=$.ProcessStartTime,exa_field_name=time""",
    """exa_json_path=$.UserName,exa_regex=^(-|\/+|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({full_name}({first_name}[^\s"@]+)\s({last_name}[^"]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(@({domain}[^\"]+))?)$""",
    """exa_json_path=$..timestamp,exa_field_name=time""",
    """exa_json_path=$.raw-event.timestamp,exa_field_name=time""",
    """exa_json_path=$..UserName,exa_regex=(-|\/+|[^@]+\/({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({full_name}({first_name}[^\s"@]+)\s({last_name}[^"]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))""",
