...
Fields = [
  ...
  """exa_json_path=$.event.Tactics,exa_field_name=alert_type""",
	...
	"""exa_json_path=$.event.Users,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|>]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))(?<!local)""",
	"""exa_json_path=$.event.Name,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|>]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))(?<!local)""",
