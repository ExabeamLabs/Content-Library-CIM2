...
```Java
{
Name = crowdstrike-falcon-json-app-activity-success-scriptcontrolscantelemetry
    ParserVersion = "v1.0.0"
    ExtractionType = json
    Conditions = [ """"event_simpleName":"ScriptControlScanTelemetry"""", """"destinationServiceName":"CrowdStrike"""" ]
	Fields = ${CrowdStrikeParsersTemplates.crowdstrike-file-operations.Fields}[
		...
		""""ParentCommandLine":"({parent_process_command_line}.+?)","\w+"""",
		""""GrandparentCommandLine":"({grandparent_command_line}.+?)","\w+"""",
...
""""name":"({event_name}[^"]+)""""
""""UserName":"(({full_name}({first_name}[^\s"]+)\s({last_name}[^"]+))|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))""""
