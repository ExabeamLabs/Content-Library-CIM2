...
Name = crowdstrike-falcon-json-endpoint-notification-success-scheduledreport
  ...
  Fields = ${DLCrowdStrikeParserTemplates.json-crowdstrike-alert-1.Fields} [
    ...
    """exa_json_path=$.event.ReportName,exa_field_name=additional_info""",
    """exa_regex="(?i)UserId":\s*"(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-]{1,40}\$?))""""
...
# customer_id is removed
    ...
    """"RuleName":"({rule}[^"]+)""",
    """"LocalAddressIP(4|6)":\s*"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))",.+?"LocalPort":\s*"({src_port}\d+)".+?"ConnectionDirection":"0"""",
    """"LocalAddressIP(4|6)":\s*"({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))",.+?"LocalPort":"({dest_port}\d+)".+?"ConnectionDirection":"1"""",
    """"RemoteAddressIP(4|6)":\s*"({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))",.+?"RemotePort":"({dest_port}\d+)".+?"ConnectionDirection":"0"""",
    """"RemoteAddressIP(4|6)":\s*"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))",.+?"RemotePort":\s*"({src_port}\d+)".+?"ConnectionDirection":"1"""",
