...
```Java
{
Name = "abnormalsecurity-as-json-email-receive-success-spam"
ExtractionType = json
ParserVersion = "v1.0.0"
Vendor = "Abnormal Security"
Product = "Abnormal Security"
TimeFormat = "yyyy-MM-dd'T'HH:mm:ssZ"
Conditions = [""""abxMessageId":""",""""abxPortalUrl":""",""""attackType":"""",""""Spam",""",""""attackedParty":"""]
Fields = [
...
"""exa_json_path=$..fromAddress,exa_regex=^(|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))$"""
"""exa_json_path=$..toAddresses,exa_regex=^(|({email_recipients}({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))[^\"]*))$"""
