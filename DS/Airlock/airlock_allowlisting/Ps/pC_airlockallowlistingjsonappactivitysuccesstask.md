...
```Java
{
Name = airlock-allowlisting-json-app-activity-success-task
  Vendor = Airlock
  ...
  Fields = [
    ...
    """exa_json_path=$.description,exa_regex=(\s+from\s({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)""",
    """exa_json_path=$.description,exa_regex=(\sUser:\s({result}Invalid login))"""
    """exa_json_path=$.description,exa_regex=(\sUser:.+?\s({result}logged in))"""
    """exa_json_path=$.description,exa_regex=^(User\s*({operation}[^\s]+[^$]*on\s({group_name}[^$]*)\sgroup)$)"""
    """exa_json_path=$.description,exa_regex=^(New user\s({operation}[^:]+):\s({new_user_name}.+)$)"""
    """exa_json_path=$.description,exa_regex=^(User\s({operation}[^\s]+)\sthe\s({old_value}.+)\sgroup to\s({new_value}.+)$)"""
  ]
