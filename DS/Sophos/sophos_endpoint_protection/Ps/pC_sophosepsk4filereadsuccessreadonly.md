...
```Java
{
Name = sophos-ep-sk4-file-read-success-readonly
  Conditions = [ """"type":"Event::Endpoint::Device::ReadOnly"""", """"group":"PERIPHERALS"""", """"severity":"""", """"endpoint_type":""" ]
  Fields=${SophosParsersTemplates.cef-sophos-security-alert-1.Fields}[
    """"type":"({alert_name}[^"]+)"""",
    """"group":"({alert_type}[^"]+)"""",
    """"name":"({additional_info}({operation}[^:]+)[^"]+)""""
  ]
