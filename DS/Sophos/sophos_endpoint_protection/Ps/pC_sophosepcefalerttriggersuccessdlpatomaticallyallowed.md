...
```Java
{
Name = sophos-ep-cef-alert-trigger-success-dlpatomaticallyallowed
...
cef-sophos-dlp-alert = {
  ...
  Fields = [
    ...
    """CEF:([^\|]*\|){5}({alert_name}[^:.\|]+).+?Username:\s*(({domain}[^\\]+)\\+)?({user}[^\s\\]+)\s+Rule names:\s*′({rule}[^′]+).+?Data Control action:\s*({result}[^\s]+)\s+File type:\s*({file_type}.+?)\s+File size:\s*({bytes}\d+)\s+({additional_info}.+?Destination path:\s*({target}.+?)\s+Destination type:[^\|]*)""",
