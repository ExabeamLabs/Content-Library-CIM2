...
```Java
{
Name = sophos-xgfirewall-kv-app-login-success-sfw
  Vendor = Sophos
  ...
  Fields = [
    ...
    """\Wsrc_ip=({src_ip}((([0-9a-fA-F.:]{1,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""", 
    """\Wuser_name="(({email_address}[^\s@"]+@[^\s@"]+)|({user}[^\s@"]+))""",
