...
Name = envoy-e-json-http-session-http
  ...
  Fields = [
    ...
    """"downstreamLocalAddress":[^\}]+?"address":"({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))""",
    """"downstreamLocalAddress":[^\}]+?"portValue":({src_port}\d+)""",
    """"upstreamRemoteAddress":[^\}]+?"portValue":({dest_port}\d+)""",
    """"upstreamRemoteAddress":[^\}]+?"address":"({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))""",
