...
```Java
{
Name = "commvault-threatwise-str-traps-catchall"
  Vendor = "Commvault"
...
Fields = [
  ...
  """({event_category}traps)\[({session_id}\d+)\]({protocol}.+?)\s+\:\s+({service_name}.+?)\s+\:\s+({event_name}.+?)\s+\:\s+({interface}.+?)\s+\:\s+({os}.+?)\s+\:\s+\d+\|({additional_info}.+?)\s+\:\s+\d+""",
  """({event_category}traps)\[({session_id}\d+)\]({protocol}.+?)\s\:\s+({service_name}.+?)\s+\:\s+\d+\.\d+\s+\:""",
