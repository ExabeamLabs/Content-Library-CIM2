...
```Java
{
Name = hp-arubacpm-kv-endpoint-authentication-success-machineauthenticated
  Conditions = [ """ LEEF:""", """|Aruba Networks|ClearPass|""", """ cat=Insight Logs""", """|[Machine Authenticated] """, """Auth.Login-Status=""", """ Auth.Roles=""" ]
  Fields=${HPEParsersTemplates.leef-aruba-clearpass-events.Fields}[
    """({event_name}Machine Authenticated)"""
  ]
...
leef-aruba-clearpass-events = {
  ...
  Fields = [
    ...
    """\s(Auth\.|RADIUS\.Acct\-)NAS-IP-Address=({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?\s""",
