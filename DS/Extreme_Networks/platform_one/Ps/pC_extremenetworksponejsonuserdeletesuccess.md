...
```Java
{
Name = extremenetworks-pone-json-user-delete-success
  ParserVersion = "v1.0.0"
  Conditions = [ """"operation_name":"user.delete"""", """"operation_detail":""", """"timestamp":""", """"operation_context":{""", """"category":""" ]
  Fields = ${ExtremeNetworksParserTemplates.json-extremenetworks-pone-event.Fields} [
    """exa_json_path=$.operation_context.usernames,exa_field_name=removed_users""",
  ]
...
json-extremenetworks-pone-event = {
  ...
  Fields = [
    ...
    """exa_json_path=$.email,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))""",
    ...
    """exa_json_path=$.username,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({full_name}\w+(\s+\w+)+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))""",
