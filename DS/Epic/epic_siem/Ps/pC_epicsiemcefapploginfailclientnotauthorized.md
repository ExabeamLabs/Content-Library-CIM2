...
```Java
{
Name = epic-siem-cef-app-login-fail-clientnotauthorized
  ParserVersion = "v1.0.0"
  Product = Epic SIEM
  Conditions = [ """CEF:""", """|Epic|Security-SIEM|""", """|CLIENT_NOT_AUTHORIZED|""" ]
  Fields = ${EpicParsersTemplates.cef-epic-app-activity.Fields} [
    """suser=([^\^]+)?\^({full_name}[^\^]+)\^({user}[\w\.\-\!\#\^\~]{1,40}\$?)?"""
    """CLNTPEERIP=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
