...
```Java
{
Name = epic-siem-cef-endpoint-command-success-fndoscmdinvoke
  ParserVersion = "v1.0.0"
  Product = Epic SIEM
  Conditions = [ """CEF:""", """|Epic|Security-SIEM|""", """|FND_OS_CMD_INVOKE|""" ]
  Fields = ${EpicParsersTemplates.cef-epic-app-activity.Fields} [
    """DBUSERNAME=({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.-])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
    """OSUSR=({user}[\w\.\-\!\#\^\~]{1,40}\$?)"""
    """OSSHELLCMD=({command}.+?)(?=\s+\w+=|$)"""
  ]
