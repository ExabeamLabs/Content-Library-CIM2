...
```Java
{
Name = manageengine-adauditplus-json-app-activity-success-301
  ParserVersion = "v1.0.0"
  Conditions = [ """"EVENT_NUMBER":"301"""", """"ACTIVITY":"""", """"ACTIVITY_TYPE":"""" ]
  Fields = ${ADAuditParserTemplates.ad-audit-plus-json-events.Fields}[
    """exa_json_path=$.ACTOR_UPN,exa_regex=(({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))""",
    """exa_json_path=$.TARGET0_UPN,exa_regex=(({dest_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?))""",
  ]
