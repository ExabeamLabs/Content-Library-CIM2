...
```Java
{
Name = manageengine-adauditplus-json-group-member-remove-success-4757
  ParserVersion = "v1.0.0"
  Conditions = [ """"EVENT_NUMBER":"4757"""", """"A member was removed from a security-enabled universal group."""" ]
  Fields = ${ADAuditParserTemplates.ad-audit-plus-json-events.Fields}[
    """exa_json_path=$.MEMBER_NAME,exa_field_name=member"""
    """exa_json_path=$.GROUP_SCOPE,exa_field_name=group_info"""
    """exa_json_path=$.GROUP_TYPE,exa_field_name=group_type"""
    """exa_json_path=$.MEMBER_SID,exa_field_name=member_id"""
  ]
...
ad-audit-plus-json-events = {
  ...
  Fields = [
    ...
    """exa_json_path=$.TARGET0_UPN,exa_regex=(({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-]{1,40}\$?))""",
