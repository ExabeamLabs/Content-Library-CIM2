...
```Java
{
Name = citrix-cvapps-json-file-download-success-filedownload
  ParserVersion = v1.0.0
  Product =  Citrix Virtual Apps
  Conditions = [ """"occurrence_event_type":"File.Download"""", """"entity_type":"user"""", """"product":"Citrix Virtual Apps and Desktops"""" ]
  Fields = ${CitrixParsersTemplates.citrix-json-virtual-apps.Fields} [
    """exa_json_path=$.file_download_file_name,exa_field_name=file_name""",
    """exa_json_path=$.file_download_file_path,exa_field_name=file_dir"""
  ]
...
citrix-json-virtual-apps {
    ...
    Fields = [
      ...
      """exa_json_path=$.entity_id,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?)""",
      """exa_json_path=$.session_user_name,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?)""",
