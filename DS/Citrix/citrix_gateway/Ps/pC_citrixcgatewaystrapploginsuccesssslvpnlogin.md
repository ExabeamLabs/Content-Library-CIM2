...
Name = "citrix-cgateway-str-app-login-success-sslvpnlogin"
  ...
  Fields = [
    ...
    """User (({email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|((({domain}[^\\\/]+?)(?:\\|\/))?({user}[\w\.\-\!\#\^\~]{1,40}\$?))) - Client_ip ({host}({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)"""
    """dvchost=({host}[^\s]+)"""
