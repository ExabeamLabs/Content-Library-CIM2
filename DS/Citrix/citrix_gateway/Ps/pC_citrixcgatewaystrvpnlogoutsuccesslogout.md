...
Name = citrix-cgateway-str-vpn-logout-success-logout
  ...
  Fields = [
    ...
    """({time}\d\d\/\d\d\/\d\d\d\d:\d\d:\d\d:\d\d)\s+(\w\w\w)?\s+({host}(({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?|({dest_host}[^\s]+)))""",
    """User ({email_address}[^@\s]+@[^@\s]+) - Client_ip ({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
