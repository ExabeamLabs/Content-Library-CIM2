...
"""\sUser\s*(({email_address}[^@\-\s]+@[^\.\-\s]+\.[^\-\s]+)|(({domain}[^\\\s\-]+)\\+)?({user}[^\-\s]+))\s*\-""",
"""({event_name}SSLVPN \w+)"""
"""\sSessionId:\s*({session_id}\d+)"""
"""\sSource\s*({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})):({src_port}\d+)"""
...
"""\sDestination\s*({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})):({dest_port}\d+)"""
