...
Name = trendmicro-tippingpoint-str-alert-trigger-success-ip
  ...
  Fields = ${TippingPointParserTemplates.tippingpoint-sms-alert-template.Fields} [
    ...
    """00000001-0001-0001-0001-00000\d+\s+.+?\s+\d+\s+ip(\s+[^\s]+){6}\s+({vlan_id}\d+)""",
    """00000001-0001-0001-0001-00000\d+\s+.+?\s+\d+\s+ip(\s+[^\s]+){7}\s+({host}[^\s]+)""",
    """00000001-0001-0001-0001-00000\d+\s+.+?\s+\d+\s+ip(\s+[^\s]+){9}\s+({time}\d{13})""",
    """00000001-0001-0001-0001-00000\d+\s+.+?\s+\d+\s+ip(\s+[^\s]+){10}\s+({alert_id}\d+)""",
    """ip\s+({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s+({src_port}\d+)\s+({dest_ip}[a-fA-F\d.:]+)\s+({dest_port}\d+)"""
  ]
...
tippingpoint-sms-alert-template = {
    ...
    Fields = [
          ...
          """\s+({event_id}[^\s]+)\s+00000001-0001-0001-0001-00000""",
