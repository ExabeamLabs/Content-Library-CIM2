...
Name = trendmicro-tippingpoint-str-alert-trigger-success-smb
  ...
  Fields = ${TippingPointParserTemplates.tippingpoint-sms-alert-template.Fields} [
    ...
    """smb\s+({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s+({src_port}\d+)\s+({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s+({dest_port}\d+)"""
  ]
