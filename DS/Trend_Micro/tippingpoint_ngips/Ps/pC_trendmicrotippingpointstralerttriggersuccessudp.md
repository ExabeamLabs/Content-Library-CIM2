...
```Java
{
Name = trendmicro-tippingpoint-str-alert-trigger-success-udp
  ...
  Fields = ${TippingPointParserTemplates.tippingpoint-sms-alert-template.Fields} [
    ...
    """udp\s+({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s+({src_port}\d+)\s+({dest_ip}[a-fA-F\d.:]+)\s+({dest_port}\d+)"""
  ]
