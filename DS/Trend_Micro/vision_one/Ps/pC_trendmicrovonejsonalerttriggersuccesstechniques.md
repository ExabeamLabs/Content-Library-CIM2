...
Name = trendmicro-vone-json-alert-trigger-success-techniques
  ...
  Fields = [
    ...
    """exa_json_path=$.entityName,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))"""
    """exa_json_path=$..highlightedObjects[?(@.type == 'email_sender')].value,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))"""
