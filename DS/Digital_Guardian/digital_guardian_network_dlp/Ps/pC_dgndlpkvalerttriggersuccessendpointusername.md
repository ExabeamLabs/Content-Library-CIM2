...
"""\s\[id=\"({alert_id}[^\"]+)"""
"""\s\[endpoint_user_name=\"(({domain}[^\"]+)[\\\/]+)?({user}[\w\.\-]{1,40}\$?)"""
"""\s\[ip_addr=\"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
"""\s\[protocol_device_taisyou=\"({alert_type}[^\"]+)"""
"""\s\[policy_rule=\"({alert_name}[^\"]+)"""
"""\s\[syadan=\"({action}[^\"]+)"""
"""\s\[jyudaido=\"({alert_severity}[^":]+)"""
"""\s\[soushinsya=\"(N\/A|({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)"""
"""\s\[taisyou_ip=\"(N\/A|({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?)"""
