...
```Java
{
Name = goanywhere-gamft-kv-file-download-success-download-1
  ...
  Fields = ${GoAnywhereParsersTemplates.goanywhere-events-2.Fields}[
     """GoACHfile_path="({file_path}[^"]*\/({file_name}[^"]*?(\.({file_ext}[^"\.]+))?))"""",
     """"({operation}Download)""""
  ]
...
goanywhere-events-2 = {
    ...
    Fields = [
      ...
      """GoACHremote_ip="({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""",
      """GoACHlocal_ip="({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""",
