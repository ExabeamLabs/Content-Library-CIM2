...
```Java
{
Name = pan-gp-json-app-authentication-success-configuration
  Vendor = Palo Alto Networks
  Product = GlobalProtect
  TimeFormat = ["yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ", "yyyy-MM-dd'T'HH:mm:ss.SSSSSSSSSZ"]
  ExtractionType = json
  Conditions = [ """"Stage":""", """LogType":"GLOBALPROTECT""", """EventIDValue":""", """EventStatus""", """"AuthMethod":""" ]
  Fields = [
  ...
  """exa_json_path=$.AuthMethod,exa_field_name=auth_method,exa_match_expr=!Contains(toLower($.AuthMethod),"null")"""
  """exa_json_path=$.Description,exa_regex=({additional_info}[^"]+:\s*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})))"""
  ]
