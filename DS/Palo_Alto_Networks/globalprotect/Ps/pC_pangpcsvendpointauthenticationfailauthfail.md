...
""""failed authentication for user '(localhost|none|system|\.{3}|({src_ip}(\d{1,3}\.){3}\d{1,3}|([A-Fa-f0-9%\.]*:[A-Fa-f0-9%\.:]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?)@({domain}[^@.,']+(\.lan|\.local)?)|({email_address}[^@',]+@[^\.']+\.[^']+)|({=user}[^']+))'""",
