...
```Java
{
Name = pan-wildfire-json-alert-trigger-success-wildfire
  ParserVersion = "v1.0.0"
  Conditions = [ """"LogType":"THREAT"""", """"Subtype":"wildfire"""" ]
  #Removed the fields section from here as template block contains all the fields
...
cef-palo-alto-networks-firewall = {
  ...
  """exa_json_path=$.SourceUserName,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|(({domain}[^\\\s,]+)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
  ...
  """exa_json_path=$.event.SourceUserName,exa_regex=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|(({domain}[^\\\s,]+)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
  ...
  """exa_regex=((?:1969-[^,]+?)|({time}\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d\.\d+[\+-]\d+:\d+))"""
    ...
    """exa_json_path=$.ThreatID,exa_field_name=alert_name"""
