...
```Java
{
Name = pan-prismaaccess-leef-endpoint-authentication-fail-authfail
 Conditions = [ """LEEF:""", """|Palo Alto Networks|""", """|Prisma Access|""", """ cat=system""", """SubType=auth""", """|auth-fail|""" ]
 ParserVersion = "v1.0.0"
  Fields = ${PaloAltoParsersTemplates.leef-pan-events.Fields}[
    """EventComponent=({auth_method}[^=]+?)\s\w+=""",
    """EventDescription=failed authentication for user\s'({user}[\w\.\-\!\#\^\~]{1,40}\$?)'""",
    """EventDescription=[^=]+?From:\s({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
