...
```Java
{
Name = pan-prismaaccess-leef-endpoint-authentication-success-signvalidated
 Conditions = [ """LEEF:""", """|Palo Alto Networks|""", """|Prisma Access|""", """ cat=system""", """SubType=auth""", """|saml-signature-validated|""" ]
 ParserVersion = "v1.0.0"
  Fields = ${PaloAltoParsersTemplates.leef-pan-events.Fields}[
    """EventComponent=({auth_method}[^=]+?)\s\w+=""",
    """EEventDescription=[^=]*?for user\s'((({user}[\w\.\-]{1,40}\$?)@({domain}[^']+))|(pre-logon|((({=domain}[^\\']+)\\)?({=user}[^']+))))'""",
