...
```Java
{
Name = pan-prismaaccess-leef-endpoint-authentication-success-authsuccess
 Conditions = [ """LEEF:""", """|Palo Alto Networks|""", """|Prisma Access|""", """ cat=system""", """SubType=auth""", """|auth-success|""" ]
 ParserVersion = "v1.0.0"
  Fields = ${PaloAltoParsersTemplates.leef-pan-events.Fields}[
    """EventComponent=({auth_method}[^=]+?)\s\w+=""",
    """user\s*'((({user}[\w\.\-]{1,40}\$?)@({domain}[^']+))|(pre-logon|((({=domain}[^\\']+)\\)?({=user}[^']+))))'"""
