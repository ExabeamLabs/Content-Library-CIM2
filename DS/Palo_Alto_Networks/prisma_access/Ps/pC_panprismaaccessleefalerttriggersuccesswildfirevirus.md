...
```Java
{
Name = pan-prismaaccess-leef-alert-trigger-success-wildfirevirus
 Conditions = [ """LEEF:""", """|Palo Alto Networks|""", """|Prisma Access|""", """ cat=threat""", """SubType=wildfire-virus""" ]
 Fields = ${PaloAltoParsersTemplates.leef-pan-events.Fields}[
    """\sSubType=({alert_type}[^=]+?)\s\w+=""", 
    """\sThreatCategory=({threat_category}[^=]+?)\s\w+=""",
    """\scat=({alert_name}[^=]+?)\s\w+=""",
    """VendorSeverity=({alert_severity}[^\s]+)"""
 ]
