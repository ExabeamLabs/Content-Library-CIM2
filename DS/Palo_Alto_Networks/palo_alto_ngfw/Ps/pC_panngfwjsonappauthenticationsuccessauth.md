...
```Java
{
Name = pan-ngfw-json-app-authentication-success-auth
    Conditions = [ """"LogType":"AUTH"""", """"DeviceSN":"""", """"Subtype":""""]
    Fields = ${PaloAltoParsersTemplates.paloalto-vpn.Fields}[
      """exa_json_path=$.AuthenticationPolicy,exa_field_name=policy_name"""
      """exa_json_path=$.AuthenticationDescription,exa_regex=\s?'({description}[^=]+)'"""
      """exa_json_path=$.AuthEvent,exa_field_name=action"""
      """exa_regex=auth_session_id\s'({session_id}[^']+)'"""
      """exa_json_path=$.NormalizeUser,exa_regex=(null|(({domain}[^\\\s]+)\\+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
    ]
