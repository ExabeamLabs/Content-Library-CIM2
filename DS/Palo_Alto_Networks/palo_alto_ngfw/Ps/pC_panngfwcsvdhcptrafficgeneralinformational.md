...
Name = pan-ngfw-csv-dhcp-traffic-generalinformational
  ...
  Fields = [
    ...
    """,general,informational,"+({event_name}DHCP lease)\s+started ip\s+({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s+-->\s+mac\s+({src_mac}[^\s]+)\s+-\s+hostname\s+({src_host}[\w.-]+)?,\s+interface\s+({interface_name}[^"]+)""",
