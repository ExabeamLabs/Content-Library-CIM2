...
Fields = ${PaloAltoParsersTemplates.pan-csv-threat.Fields}[
  ...
  """,THREAT,data,([^,]*,){26}(("*[^"]*")|[^,]*),([^,]*,){27}({device_name}[\w\-\.]+)(,|$)"""
  """,THREAT,data,([^,]*,){8}(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|(|({domain}[^\\,]+))\\?(|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))(,|$)"""
  """,THREAT,data,([^,]*,){7}(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|(|({domain}[^\\,]+))\\?(|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))(,|$)"""
