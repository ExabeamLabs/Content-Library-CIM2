...
paloalto-firewall = {
   ...
   Fields = [
     ...
     """TRAFFIC,([^,]*,){7}({rule}[^,]+?)\s*,""",
     """,TRAFFIC,([^,]*,){8}\s*(|\d|(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|((?:({domain}({src_domain}[^\s,\\]+))\\)?({src_user}({user}[\w\.\-\!\#\^\~]{1,40}\$?))))),""",
     """,TRAFFIC,([^,]*,){9}\s*(|(?:({dest_domain}[^\s,\\]+)\\)?(|\d|({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({dest_user}[^\s,]+))),""",
