...
Name = pan-ngfw-csv-network-traffic-success-connection
  ...
  TimeFormat = ["yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ","yyyy/MM/dd HH:mm:ss"]
  Conditions = [
""",THREAT,url,"""
  ]
  Fields = [
    ...
    """THREAT,url,((|[^,]+),){2}({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?,({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?,({src_translated_ip}[A-Fa-f\d:.]+),({dest_translated_ip}[A-Fa-f\d:.]+),"""
