...
""""When authenticating user '(localhost|none|system|\.{3}|({user}[\w\.\-\!\#\^\~]{1,40}\$?)@({domain}[^@.,']+(\.lan|\.local)?)|({email_address}[^@',]+@[^\.']+\.[^']+)|({=user}[^']+))' from '(({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?|({src_host}[\w\-.]+))', ({failure_reason}[^.,]+)\.""",
