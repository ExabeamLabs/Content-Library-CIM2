...
Name = pan-aperture-sk4-alert-trigger-success-policyviolation
    ...
    Fields = [
      ...
      """\Witem_type\s*=\s*"user"(\s*\w+\s*=\s*"[^"]*")*\s*item_name\s*=\s*"(Unknown|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))"""",
      """\Witem_name\s*=\s*"(Unknown|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))"(\s*\w+\s*=\s*"[^"]*")*\s*item_type\s*=\s*"user"""",
      """\Wcloud_app_instance\s*=\s*"({alert_type}[^"]+)"""",
      """\Waction_taken\s*=\s*"({additional_info}[^"]+)"""",
      """"policy_rule_name":"({alert_name}[^"]+)""",
      """"item_type":"({item_type}[^"]+)""",
      """"item_type":"user".*?"item_name":"(Unknown|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))""",
      """"item_name":"(Unknown|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))".*?"item_type":"user"""",
      """"cloud_app_instance":"({alert_type}[^"]+)""",
      """"action_taken":"({additional_info}[^"]+)""",
      """"item_creator":"(|({item_creator}[^"]+))"""",
      """"item_creator_email":"(|Unknown|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))"""",
