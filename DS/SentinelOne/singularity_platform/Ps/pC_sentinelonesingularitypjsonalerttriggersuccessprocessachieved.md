...
""""(agentIp|externalIp)"+:\s*"+({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
""""(agentOsType|osName)"+:\s*"+({os}[^"]+)"""
""""fileDisplayName"+:\s*"+({file_name}[^"]+)"""
""""filePath"+:\s*"+({malware_url}[^"]+)"""
""""domain"+:\s*"+({domain}[^",]+)"""
""""uuid"+:\s*"+({user_uid}[^"]+)"""
""""inet":\s*\["({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"]"""
