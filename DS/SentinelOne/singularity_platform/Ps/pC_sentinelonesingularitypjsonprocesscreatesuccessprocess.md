...
Name = sentinelone-singularityp-json-process-create-success-process
  ...
  Fields = ${SentinelOneParsersTemplates.json-sentinelone-edr-events.Fields} [
    ...
    """"src.process.user":"*((NT AUTHORITY|NT-AUTORITÄT|({domain}[^\\"]+))[\\\/]+)?(SYSTEM|NETWORK SERVICE|LOCAL SERVICE|Système|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""",
    ...
    """tgt.process.user":"*((NT AUTHORITY|NT-AUTORITÄT|({domain}[^\\"]+))[\\\/]+)?(SYSTEM|NETWORK SERVICE|LOCAL SERVICE|Système|({user}[\w\.\-\!\#\^\~]{1,40}\$?)|({full_name}[^"]+))""""
    ...
    """exa_regex="tgt.process.user":"*((NT AUTHORITY|NT-AUTORITÄT|({domain}[^\\"]+))[\\\/]+)?(SYSTEM|NETWORK SERVICE|LOCAL SERVICE|Système|({user}[\w\.\-\!\#\^\~]{1,40}\$?)|({full_name}[^"]+))""""
  ]
