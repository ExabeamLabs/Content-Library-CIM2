...
Name = sentinelone-singularityp-json-alert-trigger-success-dnsresolved
  ...
  Conditions = [ """"dataSource.name":"SentinelOne"""", """"event.category":"dns"""", """"event.type":"DNS Resolved"""" ]
  Fields = ${SentinelOneParsersTemplates.json-sentinelone-edr-events.Fields} [
    ...
    """exa_json_path=$..['event.dns.response'],exa_field_name=dns_response""",
    """exa_json_path=$..['event.dns.request'],exa_regex=(type:\s*({dns_query_type}[\w\s\(\)]*?))?\s*({dns_query}\S+?)($|")""",
  ]
