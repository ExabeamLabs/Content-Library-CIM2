...
```Java
{
Name = "sentinelone-singularityp-json-peripheral_device-enable-connected-5126"
Vendor = "SentinelOne"
...
"""exa_json_path=$..creator,exa_regex=^(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+?))|({full_name}[^\s"]+\s[^"]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))$"""
