...
Name = sentinelone-singularityp-mix-process-close-processexit
  ...
  Conditions = [ """timestamp {""", """millisecondsSinceEpoch: """, """trueContext {""", """isRedirectedCommandProcessor: """, """processExit {""", """path:""" ]
  Fields = ${DLSentinelOneParsersTemplates.sentinelone-activity.Fields} [
    """({event_name}processExit)""",
    """parent.*?path:\s*\\?"+\s*({parent_process_path}({parent_process_dir}[^@]+?)[\\\/]*({parent_process_name}[^"\\\/]+))\\*".*commandLine:\s*\\?"+\s*({parent_process_command_line}.+?)"\\n?""",
