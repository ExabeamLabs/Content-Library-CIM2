...
```Java
{
Name = pingidentity-pi-cef-app-login-sso-idp
...
cef-ping-events-skyformation = {
  ...
  Fields = [
    ...
    """suser=(anonymous|(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))""",
...
s-ping-events = {
  ...
  Fields = [
    ...
    """exa_json_path=$.duid,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
