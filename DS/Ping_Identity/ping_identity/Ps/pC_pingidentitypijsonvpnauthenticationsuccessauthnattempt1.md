...
```Java
{
Name = "pingidentity-pi-json-vpn-authentication-success-authnattempt-1"
Conditions = [
...
s-ping-events = {
  ...
  Fields = [
    ...
    """"duid":\s*"(({domain}[^"@\\\/]+)[\\\/]+)?({user}[\w\.\-]{1,40}\$?)"""",
    """"duid":\s*"({email_address}[^"\s@]+@[^"\s@]+)""""
    """"SRCIP":\s*"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
    """"remoteAddr":\s*"({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
    """"Status":\s*"({result}[^"]+)""",
    """"Protocol":\s*"({protocol}[^"]+)""",
    """"PingHost":\s*"({host}[^"]+)""",
