...
```Java
{
Name = pingidentity-pi-json-app-activity-success-deviceadded
  ParserVersion = "v1.0.0"
  Conditions = [ """New authentication device added""", """source":"PINGID""", """fromAddress":""", """actors""" ]
...
json-ping-events = {
  ...
  Fields = [
    ...
    """exa_regex="actors":[^}]+?"name":"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"[^}]+?"type":"user"""",
    """exa_regex="actors":[^}]+?"type":"user"[^}]+?"name":"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))""""
  json-ping-events = {
  Vendor = Ping Identity
  Product = Ping Identity
  ExtractionType = json
