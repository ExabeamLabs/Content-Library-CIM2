...
```Java
{
Name = pingidentity-pingone-json-user-role-revoke-success-roleassignmentdeleted
  ParserVersion = "v1.0.0"
  Conditions = [  """"type":"ROLE_ASSIGNMENT.DELETED"""", """"actors":""", """"action":""", """"resources":""", """"internalCorrelation":""" ]
...
pingone-events = {
   ...
   Fields = [
      ...
      """exa_regex="user":.+?"name":"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""",
      """exa_regex="resources":.+?"name":"(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?))""""
   pingone-events = {
   Vendor = Ping Identity
   Product = PingOne
   TimeFormat = "yyyy-MM-dd'T'HH:mm:ss.SSSZ"
