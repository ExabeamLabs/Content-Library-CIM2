...
Name = rsa-securid-kv-vpn-logout-success-sessionremoved
  ...
  Fields = [
    ...
    """\sREMOTE_IP="({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
    """\sSESSION_ID="({session_id}[^"]+)""",
    """\sREASON="({result_reason}[^"]+)""",
    """({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?(\s+\S+){2}\s+USER_SESSION_REMOVED_TIMEOUT"""
  ]
