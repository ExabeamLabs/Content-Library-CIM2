...
Fields = [
  ...
  """exa_json_path=$.event_type_description,exa_field_name=file_type"""
  ...
  """exa_json_path=$..content_name,exa_regex="({src_file_name}[^"]+?(\.({src_file_ext}[^"\\\/\s\d\.]+))?)""""
  """exa_json_path=$..content_link,exa_field_name=directory_uri"""
  """exa_json_path=$..folder_name,exa_regex="({src_file_name}[^"]+?(\.({src_file_ext}[^"\\\/\s\d\.]+))?)""""
  """exa_regex="base_name":\s*"({src_file_name}[^"]+?({src_file_ext}\.[^"\\\/\s\d\.]+)?)""""
  ...
  """exa_json_path=$.target_users.email,exa_field_name=object"""
