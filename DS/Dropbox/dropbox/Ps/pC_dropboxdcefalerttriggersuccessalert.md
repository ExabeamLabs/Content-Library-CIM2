...
```Java
{
Name = dropbox-d-cef-alert-trigger-success-alert
    ParserVersion = "v1.0.0"
    Conditions =  [""""team_member_id":"dbmid""", """.tag""", """"admin_alerting"}""" ]
    Fields = ${DropboxParsersTemplates.cef-dropbox-activity.Fields}[
      """"assets":\s*\[[^\]]*?"display_name":\s*"({object}[^",]+)"""",
      """"alert_severity":\{"\.tag":"({alert_severity}[^"]+)""""
    ]
...
cef-dropbox-activity = {
  ...
  Fields = [
    ...
    """"actor":\s*[^\}]*?"email":\s*"(?:N\/A|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))"""",
    ...
    """"participants":\[\{"\.tag":"user","user":\s*[^\}]*?"email":"(?:N\/A|({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))""""
