...
Name = mcafee-wg-csv-http-session-3
    ...
    Fields = [
        ...
        """\[[^\]]+\] "({user}[\w\.\-\!\#\^\~]{1,40}\$?)" ({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))? ({http_response_code}\d+)\s\"\w+\s""",
        """\s\"({user_agent}[^"]*)\"\s(\"+|\"\S+\")\s+\"\d+\"\s({bytes_in}[\d]+)\s(\"+|\"\S+\")\s""",
        """({bytes_out}[^ ]+)\s\"({user_agent}[^"]*)\"\s(\"+|\"\S+\")\s+\"\d+\"\s({bytes_in}[\d]+)\s(\"+|\"\S+\")\s""",
        """"({mime}[^"]*)"\s({bytes_in}[^ ]+)\s\"({user_agent}[^"]*)\"\s(\"+|\"\S+\")\s+\"\d+\"\s({bytes_out}[\d]+)\s(\"+|\"\S+\")\s""",
        """\s"({category}[^"]*)"\s"[\w\s]+"\s"({mime}[^"]*)"\s({bytes_in}[^ ]+)\s\"({user_agent}[^"]*)\"\s(\"+|\"\S+\")\s+\"\d+\"\s({bytes_out}[\d]+)\s(\"+|\"\S+\")\s""",
        """\[[^\]]+\]\s*"({user}[\w\.\-\!\#\^\~]{1,40}\$?)"\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?\s*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?\s*({http_response_code}\d+)\s\"\w+(\s\w+\:\/\/|\s)({web_domain}[\w\-\.]+)(\/|\s|\:)"""
    ]
