...
```Java
{
Name = skyhighsecurity-swg-cef-app-logout-success-usertimedout
  Conditions = [ """ mwg:""", """ CEF:""", """|Skyhigh Security|WebGateway|""", """ Action=USER_TIMED_OUT """ ]
...
cef-skyhigh_security-secure_web_gateway = {
  ...
  Fields = [
    ...
    """\sUser=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s""",
    """\sAction=({action}[^\s]+)\s"""
