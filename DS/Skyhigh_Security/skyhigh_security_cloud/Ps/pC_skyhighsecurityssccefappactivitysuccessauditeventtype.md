...
```Java
{
Name = skyhighsecurity-ssc-cef-app-activity-success-auditeventtype
  Vendor = Skyhigh Security
  Product = Skyhigh Security Cloud
  TimeFormat = "MMM dd yyyy HH:mm:ss.SSS z"
  Conditions = [  """CEF:0""" ,  """|Skyhigh Security|""", """Dashboard Audit Logs""", """auditEventType""" ]
  Fields = [
    ...
    """userInfoUserId=({user_info}[^\s]+)\s*"""
    """dvc=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
    """tenantId=({tenant_id}[^\s]+)\s+\w+="""
    """description=({description}[^"]+)\s+eventInfo="""
