...
Fields = [
  ...
  """({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\/({src_port}\d+)\W+({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\/({dest_port}\d+)"""
  """ for ({protocol}[^\s]+) protocol and service ({service_name}[^\s]+) application ({app}[^\s]+) by rule ({rule_id}[^\s]+)"""
  """attack:.+?action=(NONE|({action}[^\s,]+))"""
