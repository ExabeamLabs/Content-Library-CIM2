...
Name = cisco-secureemail-json-email-send-receive-esamid
  ...
  Fields = [
    ...
    """exa_json_path=$.ESAFriendlyFrom,exa_regex=^({src_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({src_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))$""",
    """exa_json_path=$.suser,exa_regex=^({src_email_address}([A-Za-z0-9]+[!#$%&'+\/=?^_`~.\-])*[A-Za-z0-9]+@({src_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))$""",
