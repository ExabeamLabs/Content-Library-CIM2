...
Name = "cisco-ise-kv-endpoint-authentication-accounting"
  ...
  Fields = [
    ...
    """User(-)?Name =(?:({user_type}host)/)?(({domain}[^\\\s\/;,@]+)\\+)?(([a-fA-F\d]{2}[-:]){5}[a-fA-F\d]{2}|({user}[^,;@\s\\\/]+))(;|,|\s)"""
...
# acct_output_octets is removed
    ...
    """Called-Station-ID=(::ffff:)?(({dest_mac}([a-fA-F\d]{2}[-:]){5}[a-fA-F\d]{2})|({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?)"""
