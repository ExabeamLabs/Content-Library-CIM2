...
Fields = [
  ...
  """, User-?Name =((host\/)({src_host}[^,]+)|(?!(host\/))((({domain}[^\\\/,\s@]+)[\\\/]+)?(([a-fA-f\d]{1,2}(\-|:)){5}[a-fA-f\d]{1,2}|({user}[^\\\/\s,@]+)))),"""  
  ...
  """, Framed-IP-Address=({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
  """, NAS-Identifier=({dest_host}[\w\-.]+)"""
  """, Called-Station-ID=(({dest_mac}([a-fA-F\d]{2}[-:]){5}[a-fA-F\d]{2})|({dest_host}[\w\-.]+))(:({ssid}[^,:]+)?),"""
  """, Calling-Station-ID=(({src_mac}([a-fA-F\d]{2}[-:]){5}[a-fA-F\d]{2})|({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)"""
