...
Name = cisco-asa-str-network-traffic-fail-313005
  ...
  Fields = [
    ...
    """\ssrc\s*({src_interface}\w+):((::ffff:)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})(:({src_port}\d+))?|([A-Fa-f0-9%.]*:[A-Fa-f0-9%.:]+(th0)?))|(::ffff:)?({src_host}[^\s]+?))((\(({domain}[^\\]+)\\({email_address}[^@]+@[^.]+\.[^)]+)|({user}[^\\]+?)\))?\))?""",
