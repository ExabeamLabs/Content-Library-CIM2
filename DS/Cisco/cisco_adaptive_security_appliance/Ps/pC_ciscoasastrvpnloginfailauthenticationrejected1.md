...
Name = cisco-asa-str-vpn-login-fail-authentication-rejected-1
  ...
  Fields = [
    ...
    """server\s*=\s*(::ffff:)?(({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?|({dest_host}\S+))""",
    """reason\s*=\s*({failure_reason}[^:=]+?)\s*:""",
    """user\s*=\s*(?:|\*+|({email_address}[^@:]+@[^\.]+\.[^:]+)|({user}[^@:]+)@({domain}[^:\.@]+)|(({=domain}[^\\\/:]+)[\\\/]+)?({=user}[^:]+))\s+:""",
