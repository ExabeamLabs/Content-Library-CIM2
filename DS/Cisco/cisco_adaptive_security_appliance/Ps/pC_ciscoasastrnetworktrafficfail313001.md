...
Name = cisco-asa-str-network-traffic-fail-313001
  ...
  Fields = [
    ...
    """\sfrom\s*((::ffff:)?({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})(:({src_port}\d+))?|([A-Fa-f0-9%.]*:[A-Fa-f0-9%.:]+(th0)?))|(::ffff:)?({src_host}[^\s]+?))\s*on interface\s({src_interface}\w+)"""
  ]
