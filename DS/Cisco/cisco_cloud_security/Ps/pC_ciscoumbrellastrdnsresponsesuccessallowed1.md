...
cisco-dns-response = {
  ...
  Fields = [
    """({time}\d\d\d\d\-\d\d\-\d\d \d\d:\d\d:\d\d)",("[^"]*?",|[^,]*?,){2}"(({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)","(|({src_translated_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_translated_port}\d+))?)",(|("[^"]*",))"(|({result}[^",]+))","({dns_query_type}[^"]+)","({dns_response_code}NOERROR|NXDOMAIN|SERVFAIL|REFUSED)","({dns_query}[^"]+)",("[^"]*?",|[^,]*?,)"(|({categories}({category}[^,"]+)[^"]*))""""
    ...
    """({time}\d\d\d\d-\d\d-\d\d\s\d\d:\d\d:\d\d)","[^"]+","[^,]+,({host}[\w\-.]+)"?,"[^"]+","({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?","({result}Allowed)","({dns_query_type}[^"]+)","({dns_response_code}NOERROR|NXDOMAIN|SERVFAIL|REFUSED)","({dns_query}[^"]+)","(|({categories}({category}[^,"]+)[^"]*))""""
    """({time}\d\d\d\d-\d\d-\d\d\s\d\d:\d\d:\d\d)(",("[^"]*"),"([^,]*,)?)({full_name}[^\(]+?)\s\(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^"]+\.[^"]+)\),({host}[\w.-]+)\s*(\(\d+\))?","[^"]+","({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?","({result}[^"]+)","({dns_query_type}[^"]+)","({dns_response_code}NOERROR|NXDOMAIN|SERVFAIL|REFUSED)","({dns_query}[^"]+)",""",
