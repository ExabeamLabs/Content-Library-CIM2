...
Name = cisco-umbrella-csv-http-session-proxy
  ...
  Fields = [
    ...
    """({time}\d\d\d\d-\d\d-\d\d\s\d\d:\d\d:\d\d)",("[^"]*",){4}"(|({mime}[^",]+))","({action}[^",]+)","({url}[^"]*?)","(|({referrer}[^"]+))","\s*({user_agent}[^"]+)?","({http_response_code}\d+)","({bytes_in}\d+)?","({bytes_out}\d+)?","[^"]*","(|({hash_sha256}[^"]+))","({categories}({category}[^,"]+)?(\s*,[^"]*?)?)(?:",)?",("[^"]*",){7}"(({src_host}[\w\-\.]+),)?({full_name}[^\(]+?)\s\(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^"]+\.[^"]+)\)(,({=src_host}[\w\-\.]+))?","({identity_type}(Anyconnect Roaming Client,AD Users|Roaming Computers,AD Users|AD Users,Roaming Computers))","({method}[^"]+)"""
  ]
