...
Name = cisco-fp-str-alert-trigger-success-impact
  ...
  Fields = [
    ...
    """\{\w+\}\s*({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\:+({src_port}\d+) \((unknown|({src_country}[^\)]+))""",
    """->\s*({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\:+({dest_port}\d+) \((unknown|({dest_country}[^\)]+))""",
