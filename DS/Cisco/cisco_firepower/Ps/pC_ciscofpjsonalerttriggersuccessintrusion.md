...
""""initiator_ip":"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""""
""""initiator_port":({src_port}\d+)"""
""""intrusion_policy":"({policy_name}[^"]+)""""
""""protocol":"({protocol}[^"]+)""""
""""responder_country_code":"({dest_country}[^"]+)""""
""""responder_ip":"({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""""
