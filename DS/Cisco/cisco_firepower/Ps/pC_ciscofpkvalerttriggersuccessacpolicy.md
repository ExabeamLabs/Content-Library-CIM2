...
Fields = [
  ...
  """\sSrcIP:\s*({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
  """\sDstIP:\s*({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
  """\sSrcPort:\s*({src_port}\d+)"""
  """\sDstPort:\s*({dest_port}\d+)"""
  """\sProtocol:\s*({protocol}[^,]+?)(,|\s*$)"""
  """\sUser:\s*(Unknown|No Authentication Required|({user}[^,]+?))(,|\s*$)"""
