...
Name = cisco-fp-kv-alert-trigger-success-correlationevent
  ...
  Fields = [
    ...
    """Connection Type:\s*({alert_type}.+?) (0.0.0.0|({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?)\:({=src_port}\d+) \((unknown|({src_country}[^\)]+))\) -> (0.0.0.0|({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?)\:({=dest_port}\d+) \((unknown|({dest_country}[^\)]+))\) \(({protocol}[^\)]+)\)""",
