...
Name = "cisco-fp-cef-network-traffic-connection-stats"
  ...
  Fields = [
    ...
    """cs2=({rule}[^\s]+)"""
    """cs5Label=({category}[^\s]+)"""
    """dpt=({dest_port}\d+)\s*dst=({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({=dest_port}\d+))?"""
    """request=({url}[^\s]+)"""
    """spt=({src_port}\d+)\s*src=({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({=src_port}\d+))?"""
