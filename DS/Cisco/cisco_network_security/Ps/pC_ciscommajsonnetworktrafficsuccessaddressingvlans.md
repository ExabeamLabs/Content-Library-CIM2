...
Name = cisco-mma-json-network-traffic-success-addressingvlans
  ...
  Fields = [
    ...
    """exa_json_path=$.adminName,exa_regex=^(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-]{1,40}\$?))$""",
    """exa_json_path=$.adminEmail,exa_regex=^(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-]{1,40}\$?))$""",
