...
```Java
{
Name = cisco-secureendpoint-mix-alert-trigger-success-threatdetected
  ...
  Fields=${CiscoParsersTemplates.s-cisco-amp-alert.Fields}[
    ...
    """exa_json_path=$..external_ip,exa_regex=({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
    """exa_json_path=$..network_addresses[*],exa_regex="ip":\s*"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
    """exa_json_path=$..trajectory,exa_field_name=additional_info"""
    """exa_json_path=$..parent.identity.sha256,exa_field_name=hash_sha256"""
    """exa_json_path=$..parent.identity.sha1,exa_field_name=hash_sha1"""
    ...
    """exa_json_path=$.event.file.parent.file_name,exa_field_name=file_name"""
  ]
DupFields = ${CiscoParsersTemplates.s-cisco-amp-alert.DupFields}[ "file_path->malware_file_name", "event_name->alert_type", "alert_type->category" ]
