...
Name = cisco-cii-json-alert-trigger-success-checkid
  ...
  Fields = [
    ...
    """exa_json_path=$.detail.usersFailing[:1],exa_regex=^(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-]{1,40}\$?))$""",
    """exa_json_path=$.detail.login,exa_regex=^(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-]{1,40}\$?))$""",
