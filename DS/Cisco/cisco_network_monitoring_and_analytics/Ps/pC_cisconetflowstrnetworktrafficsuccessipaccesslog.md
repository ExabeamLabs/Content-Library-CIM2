...
Name = "cisco-netflow-str-network-traffic-success-ipaccesslog"
  ...
  Fields = [
    ...
    """\slist\s\S+\s({result}\S+)\s(|({protocol}[^\.:]+)\s)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s*(\(({src_port}\d{1,5})\))?(\s(|({src_interface}\S+)))?(->\s({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(\(({dest_port}\d{1,5})\)|\s\([^\)]+\))?,\s)?\d+\spacket"""
