...
Name = cisco-se-cef-email-send-receive-success-suser
  ...
  Conditions = [ """CEF""" , """|Cisco|""", """Email Security Virtual Appliance""", """duser=""" ]
  Fields = [ 
    ...
    """suser=(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?)@({domain}[^\s]+))""",
    """\sduser=({email_recipients}({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))[^\s]*)\s+(\w+=|$)""",
