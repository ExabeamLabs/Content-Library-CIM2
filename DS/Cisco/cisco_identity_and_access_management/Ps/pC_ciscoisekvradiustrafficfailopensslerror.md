...
```Java
{
Name = cisco-ise-kv-radius-traffic-fail-opensslerror
  ParserVersion = "v1.0.0"
  Vendor = Cisco
  Product = Cisco Identity and Access Management
  TimeFormat = "MMM dd HH:mm:ss"
  Conditions = [""" CISE_RADIUS_Diagnostics ""","""OpenSSLError"""]
  Fields = ${CiscoParsersTemplates.s-nac-logon.Fields}[
    """(({time}\w+ \d+ \d+:\d+:\d+)\s+)""",
    """({host}[\w\-.]+)\s({category}CISE_RADIUS_Diagnostics)""",
    """SelectedAccessService=({policy_name}[^,]+),\s\w+=""",
    """OpenSSLErrorMessage=({additional_info}[^,]+),\s\w+=""",
    """EapAuthentication=({auth_method}[^,]+),\s\w+=""",
...
s-nac-logon = {
  ...
  Fields = [
    ...
    """User-?Name =(([a-fA-F\d]{2}[-:]){5}[a-fA-F\d]{2}|host\/({src_host}[\w\-.]+)|(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\s*(,|;)"""
    """User-?Name =((?!(host\/[\w\-\.]+))({domain}[^,;]+?)[\\\/]+)?(([a-fA-F\d]{2}[-:]){5}[a-fA-F\d]{2}|(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))|host\/({src_host}[\w\-.]+)),"""
    """, UserName =host\/[^.]+\.({domain}[^,]+),\s*NAS-IP-Address"""
    """, User-?Name =((host\/)({src_host}[^,]+)|((::ffff:)?(?!(host\/)))?(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|((([a-fA-F\d]{2}[-:]){5}[a-fA-F\d]{2})|(({domain}[^\\\/,]+)[\\\/]+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?))))"""    
