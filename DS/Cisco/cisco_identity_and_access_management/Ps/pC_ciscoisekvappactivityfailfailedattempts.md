...
Name = cisco-ise-kv-app-activity-fail-failedattempts
  ...
  Fields = [
    ...
    """User(-)?Name =(({domain}[^\\,]+)\\+)?(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))>?,"""
...
# usertype is removed
    ...
    """(=\d+)?[\=,](({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))(?<!local)(?<!loc)(?<!prd)(?<!localdomain)|({user}[\w\.\-\!\#\^\~]{1,40}\$?)(?:@({domain}[^,"]+)))("|,)\s""",
