...
Name = netskope-sc-json-alert-trigger-success-compromised-1
  ...
  Conditions = [ """"alert_type":"Compromised Credential"""", """"alert":"yes"""", """"type":"""", """"alert_name":"""" ]
   Fields = ${NetskopeParsersTemplates.json-netskope-alert.Fields}[
    ...
    """exa_json_path=$.matched_username,exa_regex=({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""
  ]
...
json-netskope-alert = {
  ...
  Fields = [
    ...
    """"protocol":\s*"({protocol}[^"]+)"""",
    """"user":\s*"(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""",
