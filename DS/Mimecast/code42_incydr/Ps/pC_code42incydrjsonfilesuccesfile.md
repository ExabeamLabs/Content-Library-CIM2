...
Name = code42-incydr-json-file-succes-file
  ...
  Fields = [
    ...
    """"destination":\{.*?"user":\{.*?"email":\[?"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"""",
    """"destination":\{.*?"category":"({dest_group}[^"]+)"""",
    """"subject":"({email_subject}[^"]+)""""
    """"email":\{"sender":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""""
    """"email":\{"recipients":\[({email_recipients}"({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))[^\]]*?)"+\]"""
    """"file":\{"name":"({email_attachment}[^"]+)".*?"email":\{"sender":""""
    """"repositoryEmail":"({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))""""
    ...
    """exa_json_path=$..file.category,exa_field_name=file_type""",
    """exa_json_path=$..file,exa_regex="originalName":"({src_file_name}[^"\.]+(\.({src_file_ext}[^"]+))?)"""",
    """exa_json_path=$..file,exa_regex="sizeInBytes":(({bytes}\d+))""",
    """exa_json_path=$..source.name,exa_field_name=src_host,exa_match_expr=Contains($..source.category,"Device")""",
    """exa_json_path=$..source.ip,exa_regex=({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""",
    """exa_json_path=$..source.domain,exa_field_name=domain""",
    """exa_json_path=$..source.removableMedia.busType,exa_field_name=device_type""",
    """exa_json_path=$..source.removableMedia.mediaName,exa_field_name=device_name""",
    """exa_json_path=$..source.removableMedia.serialNumber,exa_field_name=removable_media_serial_number""",
    """exa_json_path=$..application-action,exa_field_name=operation""",
    """exa_json_path=$..file.changeType,exa_field_name=operation,exa_match_expr=!InList(toLower($..file.changeType),"none")""",
    """exa_json_path=$..file.hash.md5,exa_field_name=hash_md5""",
    """exa_json_path=$..file.hash.sha256,exa_field_name=hash_sha256""",
    """exa_json_path=$..git.repositoryUri,exa_field_name=repository_name""",
    """exa_json_path=$..destination.printerName,exa_field_name=printer_name"""
    """exa_json_path=$..destination.printJobName,exa_field_name=object"""
    """exa_json_path=$..risk.severity,exa_field_name=alert_severity""",
    """exa_json_path=$..source.operatingSystem,exa_field_name=os""",
    """exa_json_path=$..file.category,exa_field_name=file_category""",
    """exa_json_path=$..file.mimeTypeByBytes,exa_field_name=mime""",
    ...
    """exa_json_path=$..destination.user.email,exa_regex=({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)""",
