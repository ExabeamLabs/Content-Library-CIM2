...
```Java
{
Name = mimecast-seg-json-email-eventtype
  Vendor = Mimecast
  Product = Mimecast Secure Email Gateway
  ParserVersion = v1.0.0
  ExtractionType = json
  TimeFormat = "epoch"
  Conditions = [ """destinationServiceName =Mimecast Email Security""", """"senderEnvelope":""", """"eventType":""", """"subject":""" ]
  Fields = [
    """exa_json_path=$.timestamp,exa_regex=({time}\d{13})""",
    """exa_json_path=$.action,exa_field_name=action""",
    """exa_json_path=$.eventType,exa_field_name=operation""",
    """exa_json_path=$.senderEnvelope,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""",
    ...
    """exa_json_path=$.recipients,exa_regex=({email_recipients}(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+),?)?[^"]*)""",
    ...
    """exa_json_path=$.rejectionType,exa_field_name=failure_reason""",
    """exa_json_path=$.rejectionCode,exa_field_name=failure_code""",
    ...
    """exa_json_path=$.fileMime,exa_field_name=mime""",
