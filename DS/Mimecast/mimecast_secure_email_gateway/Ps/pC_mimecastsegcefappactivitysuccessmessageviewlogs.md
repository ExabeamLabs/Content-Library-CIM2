...
Fields = [
  ...
  """"viewer":"\s*(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)(?<!local)(?<!loc)))""""
  """({app}Mimecast Email Security)"""
  """({operation}Archive Message View Logs)"""
  """"subject":"({email_subject}[^"]+?)""""
  """"to":"(({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({target}[^"]+)")""",
  """"source":"({log_source}[^"]+?)""""
  """"from":"\s*(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)(?<!local)(?<!loc)))""""
