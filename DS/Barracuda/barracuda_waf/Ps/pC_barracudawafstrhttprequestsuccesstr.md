...
# service_ip is removed
      """ barracuda TR ([^\s]+\s+){2}({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s+({src_port}\d+)""",
...
# cache_hit is removed
      ...
      """ barracuda TR ([^\s]+\s+){4}("[^"]+"\s+){2}([^\s]+\s+){9}(?:-|({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?)""",
