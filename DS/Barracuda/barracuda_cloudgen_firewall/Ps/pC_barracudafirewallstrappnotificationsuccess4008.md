...
Name = barracuda-firewall-str-app-notification-success-4008
  ...
  Fields = ${BarracudaParserTemplates.barracuda-network-info.Fields} [
    """({event_name}UDP\/Source connection limit \(\d+\) exceeded) \(\d+\) for ({protocol}[^\s]+)\s({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})):({src_port}\d+)[^@]+?->\s({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})):({dest_port}\d+)""",
  ]
