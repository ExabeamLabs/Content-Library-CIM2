...
Name = accellion-kw-kv-email-send-success-withfiles
  ...
  Fields = [
    ...
    """({email_address}[^@\s]+@[^\s]+)\s+id=[^,]+,\s*({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?,\s*Activity:""",
    """\sSubject:\s*"*\s*({email_subject}[^"]+)\s*"*""",
    """\sTo:\s*({email_recipients}.+?)\s*with files \[({email_attachments}.+?({file_ext}\w+))\]""",
