...
Name = accellion-kw-json-app-activity-success-networksettings
  ...
  Fields = ${KiteWorksParsersTemplates.accelion-kite-app.Fields}[
    """proxy_ip\\"+.+to\s+\\"+({proxy_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))"""
...
accelion-kite-app = {
  ...
  Fields = [
    ...
    """"+user_ip"+:\s*"+({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"+""",
    """"+application"+:\s*"+({app}[^"]+)"+""",
    """"+app_host"+:\s*"+({host}[^"]+)"+""",
    """"+event"+:\s*"+({access}[^"]+)"+""",
