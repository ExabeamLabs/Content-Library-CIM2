...
Name = unix-unix-str-user-switch-success-accountswitch-1
  ...
  Fields = [
    ...
    """(T|\s)\d\d:\d\d:\d\d(\.?\S+)?\s+(::ffff:)?({host}[\w\.\-]+)?\s*({event_code}su)(\[\w+\])?:\s+\(to\s+({dest_user}({account}[^)]+))\)\s+(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s+on""",
    """(T|\s)\d\d:\d\d:\d\d(\.?\S+)?\s+(::ffff:)?({host}[\w\.\-]+)?\s*\[({severity}[^\]]+)\]\s+({event_code}su):\s+\(to\s+({dest_user}({account}[^)]+))\)\s+(({email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s+on""",
