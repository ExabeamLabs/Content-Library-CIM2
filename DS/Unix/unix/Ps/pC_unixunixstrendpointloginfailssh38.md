...
Name = "unix-unix-str-endpoint-login-fail-ssh38"
  ...
  Fields = [
    ...
    """({failure_reason}Invalid user) ({user}[\w\.\-\!\#\^\~]{1,40}\$?) from (::ffff:)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
    """input_userauth_request: ({failure_reason}invalid user) ({user}[\w\.\-\!\#\^\~]{1,40}\$?)"""
    """({protocol}ssh(d)?)"""
    """failed login attempt for ({user}[\w\.\-\!\#\^\~]{1,40}\$?) from (::ffff:)?({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
