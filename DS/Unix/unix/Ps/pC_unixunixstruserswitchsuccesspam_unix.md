...
"""session opened for user"""
  ...
  Fields = [
"""({time}\w+\s+\d+\s+\d+:\d+:\d+)\s+({dest_host}({host}[\w\-.]+))\s"""
"""({time}\w+\s+\d+\s+\d+:\d+:\d+)?\s+({dest_host}({host}[\w\-.]+))?\s*sudo:"""
...
"""({time}(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2}\s+20\d{2}\s+\d{1,2}:\d{1,2}:\d{1,2})""",
