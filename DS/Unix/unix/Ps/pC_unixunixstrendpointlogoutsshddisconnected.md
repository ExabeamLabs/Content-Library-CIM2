...
Name = "unix-unix-str-endpoint-logout-sshddisconnected"
  ...
  Fields = [
    ...
    """({time}\w+\s+\d\d\s\d\d:\d\d:\d\d) (::ffff:)?(\d+|({host}[\w\-\.]+))?(\s\w+)?\s*sshd""",
    """Disconnected from user (({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})) port ({src_port}\d+)""",
