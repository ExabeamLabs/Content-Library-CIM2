...
Name = infoblox-bddi-str-dhcp-traffic-success-dhcpd
  ...
  Fields = [
    """({host}\S+) dhcpd\[""",
    """({event_name}DHCPINFORM) from ({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))? via ({dest_interface}[^\s"]+)\sTransID\s({transaction_id}\w+)""",
