...
Name = "unix-unix-kv-endpoint-login-sshdauth"
  ...
  Fields = [
    ...
    """({event_code}ssh)"""
    """\srhost=(?:({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?|({src_host}[\w\-.]+))"""
    """\suser=(({domain}[^\\=\s\"]+?)\\+)?(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)|({user}[\w\.\-\!\#\^\~]{1,40}\$?))"""
    """sshd\[({login_id}\d+)"""
