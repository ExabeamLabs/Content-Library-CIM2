...
""" nrcpts="""
  ...
  Fields = [
    ...
    """\s+from=<?({sender}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)>?""",
    """\ssize=({bytes}\d+)""",
    """\snrcpts=({num_recipients}\d+)""",
    """\smsgid=<({return_path}[^>]+)>""",
    """\sproto=({protocol}[^,]+)""",
    """from=({user}[^,@\s]+),""",
    """\srelay=(::ffff:)?({dest_host}[\w\-.]+)\s*\[(::ffff:)?({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?""",
  ]
