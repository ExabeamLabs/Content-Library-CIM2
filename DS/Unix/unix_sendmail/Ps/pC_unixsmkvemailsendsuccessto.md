...
Fields = [
  ...
  """\sto=<({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))>"""
  """\sto=({email_recipients}.+?)(,\s+\[more\])?(,\s+\w+=|\s*$)"""
  """\smailer=({protocol}[^,]+)"""
  """\srelay=({dest_host}[\w\-.]+)\s*\[({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
  """\sdsn=({result}[^,]+)"""
]
