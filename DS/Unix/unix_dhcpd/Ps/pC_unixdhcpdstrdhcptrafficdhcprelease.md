...
Name = unix-dhcpd-str-dhcp-traffic-dhcprelease
  ...
  Fields = ${DHCPDParsersTemplates.dhcpd-events.Fields}[
    """DHCPRELEASE of ({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))? from ({dest_mac}[A-Fa-f:\d.]+)\s*(\(({dest_host}[\w\-.]+)\))? via ({dest_interface}[^\s"]+)""",
  ]
