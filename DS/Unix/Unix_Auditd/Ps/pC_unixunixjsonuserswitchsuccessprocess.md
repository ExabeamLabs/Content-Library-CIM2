...
""""event":\{.*?\"event_category\":\"(|({event_subtype}[^\"]+))\""""
""""event":\{.*?\"result\":\"(|({result}[^\"]+))\""""
...

{
  ...
  Product = Unix
  TimeFormat = "yyyy-MM-dd'T'HH:mm:ss.SSSZ"
  Conditions = ["""logstash-auditbeat""", """"process"""", """"category":"audit-rule""""]
  Fields = [
    """"@timestamp":"({time}[^"]+)""",
    """"name_map":\{.*?"uid":"(|({user}[^"]+))"""",
    """"actor":\{[^\}]+?"secondary":"(|({user}[^"]+))""""
...
{
Name = "ftp-f-file-delete-success-200"
Vendor = "FTP"
Product = "FTP"
...
"""\WeventId=({event_id}\d+)"""
"""\Wcs4=({process_id}\d+)"""
"""\sdproc=({auth_process}({process_dir}[^\s]*?[\\\/]+)?({process_name}[^\s\\\/]+))\s+\w+="""
"""categoryOutcome=\/({action}[^\s]+)"""
"""src=({src_ip}[^\s]+)"""
"""dst=({dest_ip}[^\s]+)"""
"""spt=({src_port}\d+)"""
"""dpt=({dest_port}\d+)"""
