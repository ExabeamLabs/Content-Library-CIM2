...
Name = zscaler-ia-cef-alert-trigger-success-zscalernssweblogdlpdictionaries
  ...
  Fields = [
    ...
    """\sapp=({protocol}[^=]+?)\s*(\w+=|$)""",
    """request=({url}[^\s]+?)\s+(\w+=|$)""",
    """request=(\w+:\/{2})?[^\/]+({uri_path}\/[^?\s]+)(\?\S+)?\s+(\w+=|$)""",
    """request=[^=|?]+({uri_query}\?[^\s]+)\s""",
    """request=(?:[^:?]+:\/+)?(({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?|({web_domain}[^\/:\s]+))""",
