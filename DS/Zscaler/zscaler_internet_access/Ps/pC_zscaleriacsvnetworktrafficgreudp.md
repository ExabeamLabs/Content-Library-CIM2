...
```Java
{
Name = zscaler-ia-csv-network-traffic-greudp
  ParserVersion = "v1.0.0"
  Conditions = [ ""","Default Firewall Filtering Rule",""", ""","GRE",""" ]
...
zscaler-ia-csv-events-1 = {
    ...
    Fields = [
      ...
      """\d\s\d\d:\d\d:\d\d\s\d\d\d\d"(("[^"]+")?,){8}"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))""""
      """\d\s\d\d:\d\d:\d\d\s\d\d\d\d"(("[^"]+")?,){9}"({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))""""
