...
zscaler-ia-csv-events-2 = {
    ...
    Fields = [
      ...
      """\d\d:\d\d:\d\d \d\d\d\d",("[^"]*",){13}"(8.8.8.8|({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4})))(:({dest_port}\d+))?""""
      """ \d\d:\d\d:\d\d \d\d\d\d",("[^"]*",){12}"({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?""""
      """ \d\d:\d\d:\d\d \d\d\d\d",("[^"]*",){14}"({categories}[^"]+)""""
      """ \d\d:\d\d:\d\d \d\d\d\d",("[^"]*",){15}"(na|({src_host}[^"]+))""""   
