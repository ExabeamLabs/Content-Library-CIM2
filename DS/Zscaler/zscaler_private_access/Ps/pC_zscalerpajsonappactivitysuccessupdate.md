...
```Java
{
Name = zscaler-pa-json-app-activity-success-update
  ParserVersion = "v1.0.0"
  Conditions = [ """"AuditOperationType":"Update"""", """"User":"""", """"ObjectType":""" ]
  Fields = ${DLZscalerParsersTemplates.zscaler-audit-events.Fields} [
    """exa_json_path=$.AuditOperationType,exa_field_name=operation"""
  ]
...
zscaler-audit-events = {
    ...
    Fields = [
      ...
      """exa_json_path=$.User,exa_regex=(({email_address}[^"@]+@({email_domain}[^".]+\.[^"]+))|({user}ZPA System User|[\w\.\-\!\#\^\~]{1,40}\$?)|({full_name}[^",]+))"""
