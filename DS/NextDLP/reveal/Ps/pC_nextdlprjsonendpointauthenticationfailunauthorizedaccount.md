...
```Java
{
Name = nextdlp-r-json-endpoint-authentication-fail-unauthorizedaccount
  Vendor = NextDLP
  Product = Reveal
  ...
  Fields = [
    ...
    """"destination_ip":\s*\["({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))"\]""",
   ...
   """size (({bytes}\d+(\.\d+)?)({bytes_unit}(?i)kb|mb|gb))"""
    """"anonymised_description":\s*".*?status code:\s*({failure_reason}[^"]+)""""
    ]
