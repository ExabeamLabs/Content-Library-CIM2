...
Name = "exabeam-search-kv-alert-trigger-success-rulealerts"
    ...
    Fields = [
      ...
      """(?:\W|")src(_ip)?"*(:|=)"*\s*({src_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s*(?:\||"|\s+\w+=)""",
      """(?:\W|")dest_ip"*(:|=)"*\s*({dest_ip}((([0-9a-fA-F.]{0,4}):{1,2}){1,7}([0-9a-fA-F]){0,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))\s*(?:\||"|\s+\w+=)""",
