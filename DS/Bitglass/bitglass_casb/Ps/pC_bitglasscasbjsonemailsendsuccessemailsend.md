...
Name = bitglass-casb-json-email-send-success-emailsend
  ...
  Fields = [
    ...
    """exa_json_path=$.emailfrom,exa_regex=({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""",
    """exa_json_path=$.emailto,exa_field_name=email_recipients""",
    """exa_json_path=$.emailto,exa_regex=({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))""",
