...
Name = checkpoint-ngfw-kv-email-receive-mtainbound
  ...
  Conditions = [ """ CheckPoint """, """product:"MTA"""", """ifdir:"inbound"""", """email_status:"""" ]
  Fields = [
    """\Wtime:"({time}\d{10})"""",
    """\W({host}[\w\-.]+) CheckPoint""",
    ...
    """\Worigin:"({origin_ip}[^"]+)""",
    """\Wifdir:"({direction}[^"]+)""",
    """\Wfrom:"({src_email_address}([A-Za-z0-9]+[!#$%&'+-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)"""",
    """\Wto:"({email_recipients}({dest_email_address}([A-Za-z0-9]+[!#$%&'+\-\/=?^_`~])*[A-Za-z0-9]+@[^\]\s"\\,\|]+.[^\]\s"\\,\|]+)[^"]*?)""""
