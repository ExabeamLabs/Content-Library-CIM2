...
```Java
{
Name = "checkpoint-ngfw-cef-network-traffic-success-accept"
  ...
  Fields = [
    ...
    """\W(s|d)user=((CheckPoint|({last_name}[^\s]+))\s+,?(Firewall|({first_name}[^\s]+))\s+)(\(\w+\)\s+)?\((({email_address}[^\s@]+@[^\)]+)|checkpointfw|({user}[\w\.\-\!\#\^\~]{1,40}\$?))\)\s*\w+="""
