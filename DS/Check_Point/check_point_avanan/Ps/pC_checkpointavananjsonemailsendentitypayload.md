...
```Java
{
Name = checkpoint-avanan-json-email-send-entitypayload
  Vendor = Check Point
  Product = Check Point Avanan
  TimeFormat = "yyyy-MM-dd'T'HH:mm:ssZ"
  ExtractionType = json
  Conditions = [ """"entityInfo":""", """"entityPayload":""", """"fromEmail":""", """"isOutgoing":true""" ]
  Fields = [
    """exa_json_path=$.entityPayload.received,exa_field_name=time""",
    """exa_json_path=$.entityPayload.fromEmail,exa_regex=^({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))$""",
    ...
    """exa_json_path=$.entityPayload.recipients,exa_regex=^\[?({email_recipients}"?({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+)"?.*?)\s*\]?$""",
