...
```Java
{
Name = checkpoint-sd-kv-app-activity-success-inbound
  ParserVersion = "v1.0.0"
  Product = SmartDefense
  Conditions = [ """ CheckPoint """, """; originsicname:"""", """; ifdir:"inbound";""", """; product:"SmartDefense"""" ]
...
checkpoint-firewall-1 = {
  ...
  Fields = [
    ...
    """\Waction:"({event_name}[^"]+)""",
    """\Wsrc_user_name:"(({user}[\w\.\-\!\#\^\~]{1,40}\$?)|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))\s*"""",
    """\Wdst_user_name:"(({dest_user}[\w\.\-\!\#\^\~]{1,40}\$?)|({dest_email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+)))\s*"""",
    """\Wuser:"({last_name}[^,"]+),\s*({first_name}[\w\s]+\S)\s*\(({account}[^"]+?)\)""",
    """\Wuser:"({first_name}[\w\s]+[^\s,\(])\s+({last_name}[^\s,\(]+)\s*\(({account}[^"]+?)\)""",
    """\Wuser:"({last_name}[^,"\(]+),\s*({first_name}[\w\s\-]+\S)\s*(\([^\)]+?\)[^"]+?)?\(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))\)"""
    """\Wuser:"({first_name}[\w\s\-]+[^\s,\(])\s+({last_name}[^\s,\(]+)\s*(\([^\)]+?\)[^"]+?)?\(({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))\)"""
    """\W(received_bytes|client_inbound_bytes):"({bytes_in}\d+)""",
    """\W(sent_bytes|client_outbound_bytes):"({bytes_out}\d+)""",
    """\Wifname:"({interface_name}[^"]+)""",
    """\W(user|src_user_name|dst_user_name):"(?:[^_"\s]+_)?(-|({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|((({domain}[^\s]+?)[\\]+)?({user}[\w\.\-\!\#\^\~]{1,40}\$?)))\s*""""
...
"""\Wuser:\"+({user}[\w\.\-]{1,40}\$?)\s*\""""
"""\Wuser:\"+({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+.[^\]\s"\\,;\|]+))\s*\""""
"""\Wsrc_user_name:\"+({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+.[^\]\s"\\,;\|]+))\s*\""""
"""\Wdst_user_name:\"+({email_address}[A-Za-z0-9!#$%&'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+.[^\]\s"\\,;\|]+))\s*\""""
"""\Wuser:\"+({last_name}[^,\"]+),\s*({first_name}[\w\s]+\S)\s*\(({account}[^\"]+?)\)"""
"""\Wuser:\"+({first_name}[\w\s]+[^\s,\(])\s+({last_name}[^\s,\(]+)\s*\(({account}[^\"]+?)\)"""
"""\Wuser:\"+({last_name}[^,\"\(]+),\s*({first_name}[\w\s]+\S)\s*\([^\)]+?\)[^\"]+?\(({user}[\w\.\-]{1,40}\$?)(@({domain}[^\"\)]+?))?\)"""
"""\Wuser:\"+({first_name}[\w\s]+[^\s,\(])\s+({last_name}[^\s,\(]+)\s*\([^\)]+?\)[^\"]+?\(({user}[\w\.\-]{1,40}\$?)(@({domain}[^\"\)]+?))?\)"""
