...
Conditions = [
  """"proxyUserName":"""
  """"authenticationType":"""
  """destinationServiceName =Workday"""
  """"authenticationFailureMessage":"""
]
Fields = [
  """({time}\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d)\.\d+Z\s+[\w\-.]+\s+"""
  """authenticationFailureMessage"+:"+({failure_reason}[^"]+)"""
  """userName":"(Invalid Authentication|({email_address}[^@"]+@[^"]+)|({user}[^"]+))"""
  """signonIPAddress"+:"+({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
