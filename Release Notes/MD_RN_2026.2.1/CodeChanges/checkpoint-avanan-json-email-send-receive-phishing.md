...
| edit_regex_field | email_subject |  | ['"description_text\\*":\\*"[^"\']+?\'({email_subject}[^"\']+)\'', '("|\')(entity_)?payload\\*("|\'):\{[^\}]+?"subject\\*":\\*"\s*({email_subject}[^"\\]+?)\s*\\*"', '("|\')subject\\*("|\'):\s*("|\')({email_subject}[^\\"]+?)\s*\\*("|\')'] |
| edit_regex_field | full_name |  | ['"saas_info\\*":\[\{[^\}]+?"full_name\\*":\\*"({full_name}[^\\"]+)\\*"', '("|\')saas_info\\*("|\'):\{[^\}]+?"full_name\\*":\\*"({full_name}[^\\"]+)\\*"'] |
| edit_regex_field | result |  | ['"is_deleted\\*":({result}[^,]+)', '("|\')is_quarantined\\*("|\'):\s*({result}[^,]+)'] |
