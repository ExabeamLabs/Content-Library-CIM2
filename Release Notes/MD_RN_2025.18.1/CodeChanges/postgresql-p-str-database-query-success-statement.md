# Code Changes for postgresql-p-str-database-query-success-statement (Parser)

| Code Change | Field Name | Before | After |
|-------------|------------|--------|-------|
| changed | Conditions |  | [']', 'statement:'] |
| edit_regex_field | db_name |  | ['\):({user}[\w\.\-\!\#\^\~]{1,40}\$?)@({db_name}[^:]+):[^:]+.+?statement:', 'db=({db_name}[^,]+)'] |
| edit_regex_field | db_query |  | ['\Wstatement:\s*({db_query}[^;]+)'] |
...
| edit_regex_field | user |  | ['\):({user}[\w\.\-\!\#\^\~]{1,40}\$?)@({db_name}[^:]+):[^:]+.+?statement:', 'user=({user}[\w\.\-\!\#\^\~]{1,40}\$?)'] |