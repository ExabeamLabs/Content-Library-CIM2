# Code Changes for microsoft-azuremon-sk4-app-activity-policy (Parser)
...
| edit_regex_field | additional_info |  | ['"description":\s*"({failure_reason}({additional_info}[^"]+))', '"message":\s*"({failure_reason}({additional_info}[^"]+))', '"policyDefinitionId\\?":\s*\\?"({additional_info}[^"]+)"', 'exa_regex=message":\s*"({failure_reason}({additional_info}[^"]+))', 'exa_regex=policyDefinitionId\\?":\s*\\?"({additional_info}[^"]+)"'] |
