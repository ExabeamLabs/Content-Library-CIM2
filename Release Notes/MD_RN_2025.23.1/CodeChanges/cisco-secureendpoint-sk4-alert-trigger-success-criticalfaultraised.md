# Code Changes for cisco-secureendpoint-sk4-alert-trigger-success-criticalfaultraised (Parser)
...
| edit_regex_field | alert_name |  | ['"detection":\s*"(|({alert_name}[^"]+?))"', '"event_type":"({alert_name}[^"]+)"', 'dpriv=({alert_name}[^=]+?)\s\w+=', 'event_type":\s*"({alert_name}[^"]+)'] |
