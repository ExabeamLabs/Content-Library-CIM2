# Code Changes for checkpoint-ngfw-kv-network-traffic-outbound-1 (Parser)
...
| edit_regex_field | action |  | ['\Waction:"({action}[^"]+)', '\Wstormagentaction:"({event_name}({action}[^"]+))'] |
| added_regex_field | event_name |  | ['\Waction:"({event_name}[^"]+)', '\Wstormagentaction:"({event_name}({action}[^"]+))'] |
