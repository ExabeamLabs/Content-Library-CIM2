# Code Changes for cisco-secureendpoint-sk4-alert-trigger-success-cloudioc (Parser)
...
| edit_regex_field | alert_name |  | ['"detection":\s*"(|({alert_name}[^"]+?))"', '"short_description":"({alert_name}[^"]+)', 'dpriv=({alert_name}[^=]+?)\s\w+=', 'event_type":\s*"({alert_name}[^"]+)', 'exa_regex="short_description":"({alert_name}[^"]+)'] |
