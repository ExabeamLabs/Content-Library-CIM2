# Code Changes for microsoft-evsecurity-kv-endpoint-success-successfullylogin-2 (Parser)
...
| edit_regex_field | host |  | ['(?i)(((audit|success)( |_)(success|audit)))\s*(\s|\t|,|#\d+|<[^>]+>)\s*({dest_host}({host}[^=]+?))\s*(\s|\t|,|#\d+|<[^>]+>)\s*', '(?i)<\d+>\s*\w+\s+\d+\s+\d+:\d+:\d+\s+(am|pm|\d{4}|({dest_host}({host}[\w.\-]+)))\s'] |
| edit_regex_field | src_host_windows |  | ['Workstation Name(:|=)\s*(-|[A-Fa-f:\d.]+|({src_host_windows}({src_host}[^\s;]+)))[\s;]*Source Network Address(:|=)"'] |
| added_regex_field | dest_host |  | ['(?i)(((audit|success)( |_)(success|audit)))\s*(\s|\t|,|#\d+|<[^>]+>)\s*({dest_host}({host}[^=]+?))\s*(\s|\t|,|#\d+|<[^>]+>)\s*', '(?i)<\d+>\s*\w+\s+\d+\s+\d+:\d+:\d+\s+(am|pm|\d{4}|({dest_host}({host}[\w.\-]+)))\s'] |
