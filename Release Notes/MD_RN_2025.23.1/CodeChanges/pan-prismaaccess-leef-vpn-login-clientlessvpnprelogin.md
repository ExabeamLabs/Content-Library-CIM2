# Code Changes for pan-prismaaccess-leef-vpn-login-clientlessvpnprelogin (Parser)
...
| edit_regex_field | result |  | ['EventStatus=({action}({result}[^\s=]+))', 'EventStatus=({result}[^\s]+)', '\sAction=({action}({result}[^=]+?))\s\w+='] |
