...
|-------------|------------|--------|-------|
| edit_regex_field | domain |  | ['"AccountDomain":\s*"({domain}[^"\s]+)"', '"InitiatingProcessAccountDomain":\s*"(-|({domain}[^"]+))"', 'exa_json_path=$..InitiatingProcessAccountDomain,exa_regex=(-|({domain}[^$"]+))$'] |
