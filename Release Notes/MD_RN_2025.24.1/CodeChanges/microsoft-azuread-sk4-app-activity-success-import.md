...
| edit_regex_field | category |  | ['({event_name}({category}ProvisioningManagement))', 'Category":\s*"({event_name}({category}[^"]+))'] |
| edit_regex_field | resource |  | ['"Resource":\s*"({object}({resource}[^"]+))'] |
| added_regex_field | event_name |  | ['({event_name}({category}ProvisioningManagement))', 'Category":\s*"({event_name}({category}[^"]+))'] |
