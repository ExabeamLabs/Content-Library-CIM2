...
| edit_regex_field | user |  | ['exa_json_path=$.actor,exa_regex=(System|User|({user}[\w\.\-\!\#\^\~]{1,40}\$?))', 'exa_json_path=$.username,exa_regex=(({email_address}[A-Za-z0-9!#$%&\'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,\|]+\.[^\]\s"\\,\|]+))|({user}[\w\.\-\!\#\^\~]{1,40}\$?))'] |