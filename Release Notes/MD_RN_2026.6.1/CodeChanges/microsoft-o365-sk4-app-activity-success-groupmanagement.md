# Code Changes for microsoft-o365-sk4-app-activity-success-groupmanagement (Parser)
...
| edit_regex_field | dest_user |  | ['targetResources":\[\{[^\}]+?userPrincipalName":"(({dest_email_address}[A-Za-z0-9!#$%&\'+\/=?^_`~.-]+@({dest_email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|({dest_user}[^"]+))"'] |
