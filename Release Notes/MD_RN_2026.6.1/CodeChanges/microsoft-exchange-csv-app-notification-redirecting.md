# Code Changes for microsoft-exchange-csv-app-notification-redirecting (Parser)
...
| edit_regex_field | direction |  | [',[<\s]*({email_address}[A-Za-z0-9!#$%&\'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))(?<!local)(?<!loc)(?<!localdomain)(?<!localhost)[\s>]*,([^,]*,){2}({direction}Incoming|Originating),'] |
