# Code Changes for ipswitch-moveittransfer-str-app-logout-dmzsignoff (Parser)
...
| edit_regex_field | user |  | ['User\s\'(({email_address}[A-Za-z0-9!#$%&\'+\/=?^_`~.-]+@({email_domain}[^\]\s"\\,;\|]+\.[^\]\s"\\,;\|]+))|Automation|({full_name}[^\']+))?\'\s\(({user}[\w\.\-\!\#\^\~]{1,40}\$?)?\)', '\sUsername:\s*({user}[\w\.\-\!\#\^\~]{1,40}\$?)'] |