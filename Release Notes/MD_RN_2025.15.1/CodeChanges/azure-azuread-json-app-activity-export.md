# Code Changes for azure-azuread-json-app-activity-export (Parser)
...
| edit_regex_field | operation | ['"activityDisplayName":\s*"({operation}({event_name}[^"]+))"', '"operationType":\s*"({operation}[^"]+)"'] | ['"activityDisplayName":\s*"({operation}({event_name}[^"]+))"', '"operationType":\s*"({operation}[^"]+)"', 'activityDisplayName":"({operation}[^"]+)'] |
