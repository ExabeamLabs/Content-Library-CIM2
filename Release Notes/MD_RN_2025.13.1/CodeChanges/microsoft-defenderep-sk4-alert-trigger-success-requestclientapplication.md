...
| edit_regex_field | process_name |  | ['"InitiatingProcessFolderPath":\s*"(({process_path}({process_dir}[^"]+?[\\\/]+)({process_name}[^"\\\/]+(\.[a-zA-Z]+)))|({=process_dir}[^"]+))"', 'FileName":\s*"({process_name}[^"]+)', 'InitiatingProcessFileName"+:\s*"+({process_name}[\w\.\-]+)"'] |
