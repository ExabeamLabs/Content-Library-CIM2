...
```Java
{
Name = mcafee-es-process-create-fail-executiondenied
  Vendor = McAfee
  ...
  Fields = [
    ...
    """<SCORparent_process_name>({parent_process}({parent_process_dir}[^<>]*?[\\\/]+)?({parent_process_name}[^\\\/<>]+))</SCORparent_process_name>""",
