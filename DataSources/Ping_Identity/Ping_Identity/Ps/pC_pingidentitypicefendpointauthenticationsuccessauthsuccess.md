...
DupFields = [ "account->dest_user", "account_domain->dest_domain" ]
Conditions = [
...

{
  ...
  Conditions = [ """|SMB|""","""|OPEN|""" ]
  Fields = [
    """({time}\d+-\d+-\d+T\d+:\d+:\d+(([\+\-]\d+:\d+)|Z))\s+({host}[\w\-.]+)\s+([^\[\s]*)?\[[^\]]*\]:?\s+({user_sid}[^\s\|]+)\|({user_uid}[^\|]*)\|({server_name}[^\|]+)\|({zone_id}[^\|]*)\|({src_ip}[A-Fa-f:\d.]+)\|({protocol}[^\|]*)\|({access}OPEN)\|({result}[^\|\s]*)\|({desire_access}[^\|]*)\|({file_type}[^\|]*)\|({create_result}[^\|]*)\|(|({inode}[^\|]*))\|(|({src_file_path}(|({file_dir}[^"\|]*?))[\\\/]*({src_file_name}[^\\\/"\|]+?(\.({src_file_ext}[^\\\.\s"\|\/\d]+))?)))\s*""",
