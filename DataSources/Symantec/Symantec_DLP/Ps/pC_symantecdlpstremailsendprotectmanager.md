...
Fields = [
  ...
  """({host}[\w.\-]+)\s+({alert_name}[^\s\^]+)\^\^({alert_id}\d+)\^\^({additional_info}[^\^]+)\^\^[^\^]*\^\^({email_recipients}({dest_email_address}[^\^,]+)[^\^]*)\^\^({email_address}[^\^]+)\^\^({alert_severity}[^\^]+)\^\^({email_subject}[^\^]+)\^\^(N/A|({object}[^\^]+))\^\^([^\^]*\^\^){9}({protocol}[^\^]+?)\s*(\^|$)"""
  ...
  """\d\d:\d\d:\d\d\s+[\w.\-]+\s+([^\^]*?\^\^){5}({email_address}[^\^]+)"""
