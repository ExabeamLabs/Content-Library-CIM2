...
Fields = [
  ...
  """"ResourceId":"({src_file_path}({file_dir}(?:[^";]+)?[\/;])?({src_file_name}[^\/";]+))""""
  """"Resource":"({src_file_name}[^"]+)""""
  """"id_s":"({src_file_path}({file_dir}(?:[^";]+)?[\/;])?({src_file_name}[^\/";]+)?)""""
  """"SourceSystem":"({app}[^"]+)""""
...
Fields = [
  ...
  """\Wc6a1=\s*({src_ip}[A-Fa-f:\d.]+)"""
  """\Wmsg=({additional_info}.*?)\s+(\w+=|$)"""
  """\Wmsg=(.+?):\s*({file_type}[^\s]+)\s+({src_file_path}({file_dir}[^=]+?)[\\\/]+(|({src_file_name}[^\\\/]*?(\.({src_file_ext}[^\\\/:\s\.]+))?)))(\s*(with|folder|to file|;)\s+.*?)?\s+(?:$|\w+=)"""
]
